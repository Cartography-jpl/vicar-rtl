...
float *covar;
{
	...
	status =  IBISColumnWrite(ibis,&npix1,stats[unit].npixcol,recnum,1);
	if (status < 0) return status;
	status =  IBISColumnWrite(ibis,&nband,stats[unit].nbandcol,recnum,1);
	if (status < 0) return status;
...
void F77_FUNC_(istat_file_open, ISTAT_FILE_OPEN)
( int *unit, char *mode, int *nclasses,
...
void F77_FUNC_(istat_signal, ISTAT_SIGNAL)
( unit,status,abendflag)
int *unit;
int *status;
int *abendflag;
{
...
void F77_FUNC_(istat_file_close, ISTAT_FILE_CLOSE)
( unit,status)
int *unit;
int *status;
{
...
void F77_FUNC_(istat_file_info, ISTAT_FILE_INFO)
(unit,nclasses,nbands,ibis) 
int *unit;
int *nclasses;
int *nbands;
int *ibis;
...
void F77_FUNC_(istat_record_write, ISTAT_RECORD_WRITE)
(int *unit, int *recnum, char *name,
...
void F77_FUNC_(istat_record_read, ISTAT_RECORD_READ)
(int *unit, int *recnum, char *name, int *npix,
...
int F77_FUNC_(istat_record_find, ISTAT_RECORD_FIND)
(int *unit, char *classname, ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
   char c_name[33];
