...
$! VPACK Version 1.9, Thursday, May 31, 2007, 10:27:22
$!
 ...
 *----------------------------------------------------------------*/
u_long mssvr_readRequestCode (int sd)
{
 u_long req_code;
 ...
 *	2. send request code (4bytes u_long value).
...
/*--------------------------------------------------------------------------*/
int msclt_read_long (int sd, long *data)
{
 XDR    xdrs;
 char   buf[REQ_CODE_LEN];
...
/*--------------------------------------------------------------------------*/
int msclt_write_long (int sd, long data)
{
 XDR    xdrs;
 char   buf[REQ_CODE_LEN];
...
/*--------------------------------------------------------------------------*/
int msclt_read_u_long (int sd, u_long *data)
{
 XDR    xdrs;
 char   buf[REQ_CODE_LEN];
...
/*--------------------------------------------------------------------------*/
int msclt_write_u_long (int sd, u_long data)
{
 XDR    xdrs;
 char   buf[REQ_CODE_LEN];
 ...
 * return 0 ==> ERROR.
 *--------------------------------------------------------------------------*/
u_long msclt_read_resp_code (int sd)
{
 XDR    xdrs;
 char   buf[REQ_CODE_LEN];
 u_long resp = 0;
 ...
 *
 *--------------------------------------------------------------------------*/
int msclt_getspice (u_long req_code, msUserRequestStruct *req,
		msCkStruct *ckdata, msSpkStruct *spkdata)
{
 int	sd;
 u_long resp_code;
 ...
 *
 *--------------------------------------------------------------------------*/
int msclt_putspice (u_long req_code, msCkStruct *ckdata)
{
 int	sd;
 u_long	resp_code;
 char	svr_mesg[SVR_MESG_BUF_LEN];
...
int msclt_send_text_file (int sd, char *tname)
{
 ...
 else filesize = (u_long) statbuf.st_size;
 ...
 *--------------------------------------------------------------------------*/
int msclt_receive_kernel (u_long req_code, char *kname, char *local_dir)
{
 int sd;
 u_long resp_code;
...
/*--------------------------------------------------------------------------*/
int msclt_put_kernel (u_long req_code, char *kname)
{
 int sd;
...
char *item;
{
   ...
   status = FTN_NAME(xms_bodfnd) (&body, item, &i );
   return status;
}
...
double *values;
{
   ...
   FTN_NAME(xms_bodvar) (&body, item, &i, dim, values);
}
...
int *status;   		/*output*/
{
   ...
   FTN_NAME(xms_ckgp) (&inst,&sclkdp,&tol,ref,&i,cmat,clkout,status);
}
...
int *status;   		/*output*/
{
   ...
   FTN_NAME(xms_ckgpav) (&inst,&sclkdp,&tol,ref,&i,cmat,av,clkout,status);
 }
...
void *sum; /*input*/
{
   ...
   FTN_NAME(xms_dafbna) (&handle, sum, name, &i);
}
...
int *handle;
{
   ...
   FTN_NAME(xms_dafopr) (fname, &i, handle);
}
...
int *handle;
{
   ...
   FTN_NAME(xms_dafopw) (fname, &i, handle);
}
