 ...
 *     subname is the name of the fortran subroutine to be called
 ...
 *
 * In stacka_big the fortran subroutine name is in   char (*subr)()
 *
 ...
 *----------------------------------------------------------------
 * USAGE NOTES: 
 *    Any routines that may be called by stacka_big must be modified to
 *    accept "numargs" as the first parameter. numargs = total number of
 *    arguments other than numargs itself.
 ...
 *    Sep  3, 2015 - Ray Bambery - fixes for MacOSX gcc 4.8.4
 *    Jul  1, 2016 - Ray Bambery - fixed compiler warnings
 *                  improved error messages, put cast on malloc 
 */
#include <stdint.h>
#include <stdlib.h>
#include <stdio.h>
...
/* declare prototype for all other systems*/
...
//  int byte_size = 1;       /* size of one byte used for calloc    */
  ...
  int64_t len[25] = { 0 };             /* temporary array to hold lengths     */
...
//    printf ("len[0] = %ld  %ld   %ld\n",len[0],len[1],len[2] );
  for(i=0;i<*n;i++) {
    ...
    if (aary[i] == NULL) {
      sprintf(msg,"STACKA_big: malloc - insufficient memory for argument %d - %ld bytes",i,len[i]);
      zvmessage(msg,"");
      zabend();
    }
/*      else {
      sprintf(msg,"STACKA_big: malloc - argument %d - %ld bytes",i,len[i]);
      zvmessage(msg,"");
    }
*/
  }
