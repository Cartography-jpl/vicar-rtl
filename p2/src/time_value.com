...
$! VPACK Version 1.9, Wednesday, September 11, 2002, 16:16:58
$!
				...
				int *min, int *sec, int *msec)
{
	*year = 0; *doy = 0; *hr = 0; *min = 0; *sec = 0; *msec = 0;
        ...
        while (1) {
           if (*p == '-' || *p == 'T' || *p == ':' || *p == '.' || *p == '\0') {
              switch(ct) {
                ...
                case 1:
                   ...
                   *doy = atoi(tmp);
                   ...
                   if (((*year % 4) == 0) && ((*year % 100) != 0) || ((*year % 400) == 0)) {
                      if (*doy > 366) {
                         cerr << "ERROR: DOY cannot be larger than 366!!!\n";
                         exit(0);
                      }
                   }
                   else { /* if year is not leapyear, doy can be 0-365 */
                      if (*doy > 365) {
                         cerr << "ERROR: DOY cannot be larger than 365!!!\n";
                         exit(0);
                      }
                   }
                   ct++;
                ...
                case 2:
                   ...
                   *hr = atoi(tmp);
                   if (*hr > 24) {
                      cerr << "ERROR: HR cannot be larger than 24!!!\n";
                      exit(0);
                   }
                   else
                      ct++;
                ...
                case 3:
                   ...
                   *min = atoi(tmp);
                   if (*min > 60) {
                      cerr << "ERROR: MIN cannot be larger than 60!!!\n";
                      exit(0);
                   }
                   else
                      ct++;
		   ...
		   *sec = atoi(tmp);
                   if (*sec > 60) {
                      cerr << "ERROR: SEC cannot be larger than 60!!!\n";
                      exit(0);
                   }
                   else
		      ct++;
...
int ScetCoeff::load_coeff(char *filename)
{
   ...
   if (!ct) {
      cout << "No Records found in Coefficient File" << endl;
      return -1;
   }
   reset();
