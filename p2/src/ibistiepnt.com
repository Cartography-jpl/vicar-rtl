...
$! VPACK Version 1.9, Tuesday, May 12, 1998, 14:47:35
$!
...
/*   Now put pertinent parameters in Property label  */
    ...
    status = zladd( unit, "PROPERTY","NUMBER_OF_IMAGES",&no_imgs,
                          "FORMAT", "INT", "PROPERTY","TIEPOINT",0);
    if (status != 1){
   ...
   for (i = 0; i < no_imgs; i++) {
       ...
       sprintf(string, "IMAGE_%1d_NAME\0", i+1);
       sprintf(picnam, "%s\0", imgsnam[i]);
...
/* ************************************************** Change 8.Dez.97 by F.Wewel */    
        ...
        for (j = 0 ; j < cnt; j++) {
             ...
             for (k = 0; k < *Niqual; k++) {
                  ...
                  index = k + 3;
                  if (u_list[j] == index) {
                      sprintf (iqual_unit[k], "%s\0", unit_nam[i]);
                      }
                  }
/* ************************************************** Change 8.Dez.97 by F.Wewel */    
             for (k = 0; k < *Ngqual; k++) {
                  ...
                  if (u_list[j] == index) {
                      sprintf (gqual_unit[k], "%s\0", unit_nam[i]);
                      }
                  }
             }
...
/* ------------------------------------ get tne names of the image files */
   for (i = 0; i < *Nimages; i++) {
       ...
       sprintf(type, "IMAGE_%1d_NAME\0", i+1);
       status = zlget(unit, "HISTORY", type, imgsnam[i], "");
       if (status < 0) {
...
/* ------------------------------------------------
                                           general qualifier */
   sprintf (genqlf_nam [0], "id-number\0");
   sprintf (genqlf_frmt[0], "FULL\0");
   sprintf (genqlf_unit[0], "none\0");
   
   sprintf (genqlf_nam [1], "specification\0");
   sprintf (genqlf_frmt[1], "A10\0");
   sprintf (genqlf_unit[1], "XX\0");
  
   slen0 = atoi(genqlf_frmt[1] + 1) + 1;


   sprintf (genqlf_nam [2], "quality level\0");
   sprintf (genqlf_frmt[2], "FULL\0");
   sprintf (genqlf_unit[2], "Pixel\0");
  

   sprintf (genqlf_nam [3], "class\0");
   sprintf (genqlf_frmt[3], "A1\0");
   sprintf (genqlf_unit[3], "none\0");
  

/* ------------------------------------------------
                                           image qualifier */
   sprintf (imgqlf_nam [0], "cross correlation\0");
   sprintf (imgqlf_frmt[0], "REAL\0");
   sprintf (imgqlf_unit[0], "none\0");
   
   sprintf (imgqlf_nam [1], "lsm correlation\0");
   sprintf (imgqlf_frmt[1], "REAL\0");
   sprintf (imgqlf_unit[1], "0\0");
  
   no_irqlf = 2;

   sprintf (imgqlf_nam [2], "point class\0");
   sprintf (imgqlf_frmt[2], "FULL\0");
   sprintf (imgqlf_unit[2], "none\0");

   sprintf (imgqlf_nam [3], "descriptor\0");
   sprintf (imgqlf_frmt[3], "A4\0");
   sprintf (imgqlf_unit[3], "abc\0");
...
/* ------------------------------------------------
                                              any image name */
   for (i = 0; i < no_imgs; i++) {
        sprintf (&imgs_fnam[i][0], "image_%3.3d.byte\0", i+100);
        }
...
/* ------------------------------------------------
                                           general qualifier */
   sprintf (genqlf_nam [0], "\0");
   sprintf (genqlf_frmt[0], "\0");
   sprintf (genqlf_unit[0], "\0");
   
   sprintf (genqlf_nam [1], "\0");
   sprintf (genqlf_frmt[1], "\0");
   sprintf (genqlf_unit[1], "\0");
  
   sprintf (genqlf_nam [2], "\0");
   sprintf (genqlf_frmt[2], "\0");
   sprintf (genqlf_unit[2], "\0");
  

   sprintf (genqlf_nam [3], "\0");
   sprintf (genqlf_frmt[3], "\0");
   sprintf (genqlf_unit[3], "\0");

/* ------------------------------------------------
                                           image qualifier */
   sprintf (imgqlf_nam [0], "\0");
   sprintf (imgqlf_frmt[0], "\0");
   sprintf (imgqlf_unit[0], "\0");
   
   sprintf (imgqlf_nam [1], "\0");
   sprintf (imgqlf_frmt[1], "\0");
   sprintf (imgqlf_unit[1], "\0");
  
   sprintf (imgqlf_nam [2], "\0");
   sprintf (imgqlf_frmt[2], "\0");
   sprintf (imgqlf_unit[2], "\0");

   sprintf (imgqlf_nam [3], "\0");
   sprintf (imgqlf_frmt[3], "\0");
   sprintf (imgqlf_unit[3], "\0");
