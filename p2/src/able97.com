...
$! VPACK Version 1.9, Wednesday, January 21, 2004, 16:21:16
$!
...
C-----------ENCODING_TYPE
     ...
     2        'FORMAT','STRING','ULEN', 8,' ')
        ...
        ELSE
C-----------INST_CMPRS_TYPE
	    CALL XLGET(UNIT,'PROPERTY','INST_CMPRS_TYPE',LAB_COMPRSN,ISTAT,
	...
	IF ((PROPERTYNAME .EQ. 'CASSINI-ISS   ') .OR. 
     1      (PROPERTYNAME .EQ. 'CASSINI-ISS2  ')) THEN
C-----------CONVERSION_TYPE
	    CALL XLGET(UNIT,'PROPERTY','CONVERSION_TYPE',LAB_CONVRSN,ISTAT,
     1             'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
...
C-----------DATA_CONVERSION_TYPE
	    CALL XLGET(UNIT,'PROPERTY','DATA_CONVERSION_TYPE',LAB_CONVRSN,
...
C-------OPTICS_TEMPERATURE
	CALL XLGET(UNIT,'PROPERTY','OPTICS_TEMPERATURE',LAB_OPTTEMP,ISTAT,
     1             'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','REAL',' ')
	IF (ISTAT.NE.1) THEN
...
C-------SENSOR_HEAD_ELEC_TEMPERATURE
	IF (PROPERTYNAME .EQ. 'CASSINI-ISS2  ') THEN
	   CALL XLGET(UNIT,'PROPERTY','SENSOR_HEAD_ELEC_TEMP',LAB_SENSORTEMP,
     1		   ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','REAL',' ')
	   IF (ISTAT.NE.1) THEN
...
C-------ANTIBLOOMING_STATE_FLAG
	CALL XLGET(UNIT,'PROPERTY','ANTIBLOOMING_STATE_FLAG',LAB_ANTIBLM,
     1             ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','STRING','ULEN',3,' ')
	IF (ISTAT.NE.1) THEN
...
C-------CALIBRATION_LAMP_STATE_FLAG
	...
	CALL XLGET(UNIT,'PROPERTY','CALIB_LAMP_STATE_FLAG',LAB_CALLAMP,
     ...
     2             'FORMAT','STRING','ULEN', 3,' ')
	    ...
	    END IF
        ELSE
	CALL XLGET(UNIT,'PROPERTY','CALIBRATION_LAMP_STATE_FLAG',LAB_CALLAMP,
     1             ISTAT,'PROPERTY','INSTRUMENT','ERR_ACT',' ',
     2             'FORMAT','STRING','ULEN', 3,' ')
	    IF (ISTAT.NE.1) THEN
...
C-------DARK_CURRENT / EXTENDED_PIXEL_VALUE / DARK_STRIP_MEAN
	IF (PROPERTYNAME .EQ. 'CASSINI-ISS   ') THEN
     ...
     2             'FORMAT','REAL',' ')
	   ...
	   END IF
	ELSEIF (PROPERTYNAME .EQ. 'CASSINI-ISS2  ') THEN
	   CALL XLGET(UNIT,'PROPERTY','EXTENDED_PIXEL_VALUE',LAB_EXTPIXVAL,
     1		   ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','REAL',' ')
	   IF (ISTAT.NE.1) THEN
...
C-------SHUTTER_MODE_ID (NACONLY, WACONLY, BOTSIM)
	   CALL XLGET(UNIT,'PROPERTY','SHUTTER_MODE_ID',LAB_SHUTTERMODE,ISTAT,
     1             'PROPERTY','INSTRUMENT','ERR_ACT',' ',
     2             'FORMAT','STRING','ULEN', 8,' ')
	   IF (ISTAT.NE.1) THEN
		CALL XVMESSAGE('SHUTTER_MODE_ID VALUE MISSING',' ')
...
C-------DELAYED_READOUT_FLAG
	   CALL XLGET(UNIT,'PROPERTY','DELAYED_READOUT_FLAG',LAB_DLYREADOUT,
     1             ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','STRING','ULEN', 3,' ')
	   IF (ISTAT.NE.1) THEN
...
C-------SHUTTER_EVENT_TYPE (NACONLY, WACONLY, BOTSIM)
	   CALL XLGET(UNIT,'PROPERTY','SHUTTER_EVENT_TYPE',LAB_SHUTTERMODE,
     1		   ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2		   'FORMAT','STRING','ULEN', 8, ' ')
           IF (ISTAT.NE.1) THEN
...
C-------PREPARE_CYCLE_INDEX
	   CALL XLGET(UNIT,'PROPERTY','PREPARE_CYCLE_INDEX',LAB_PREPCYCLE,
     1             ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','INT',' ')
	   IF (ISTAT.NE.1) THEN
...
C-------READOUT_CYCLE_INDEX
	   CALL XLGET(UNIT,'PROPERTY','READOUT_CYCLE_INDEX',LAB_READOUTCYCLE,
     1             ISTAT,'PROPERTY',PROPERTYNAME,'ERR_ACT',' ',
     2             'FORMAT','INT',' ')
	   IF (ISTAT.NE.1) THEN
...
C-------MISSING_PACKET_FLAG
	   CALL XLGET(UNIT,'PROPERTY','MISSING_PACKET_FLAG',LAB_MISPACFLG,
     1		   ISTAT,'PROPERTY','TELEMETRY','ERR_ACT',' ',
...
C-------SPACECRAFT_CLOCK_CNT_PARTITION
     ...
     2		   ' ','FORMAT','INT',' ')
           IF (ISTAT.NE.1) THEN
		CALL XVMESSAGE('SPACECRAFT_CLOCK_CNT_PARTITION VALUE MISSING'
     1             ,' ')
		IND=-1
		RETURN
           END IF
...
C-------COMMAND_SEQUENCE_NUMBER
	   CALL XLGET(UNIT,'PROPERTY','COMMAND_SEQUENCE_NUMBER',LAB_COMDSEQNUM,
...
C-------EARTH_RECEIVED_START_TIME
	   CALL XLGET(UNIT,'PROPERTY','EARTH_RECEIVED_START_TIME',LAB_ERTSTART,
     1		   ISTAT,'PROPERTY','TELEMETRY','ERR_ACT',' ',
     2		   'FORMAT','STRING','ULEN', 21, ' ')
           IF (ISTAT.NE.1) THEN
...
C-------EARTH_RECEIVED_STOP_TIME
	   CALL XLGET(UNIT,'PROPERTY','EARTH_RECEIVED_STOP_TIME',LAB_ERTSTOP,
     1		   ISTAT,'PROPERTY','TELEMETRY','ERR_ACT',' ',
     2		   'FORMAT','STRING','ULEN', 21, ' ')
           IF (ISTAT.NE.1) THEN
...
FTN_NAME(xable97)( ind, &unit, &lab->sclk, 
