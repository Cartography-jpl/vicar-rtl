...
$! VPACK Version 1.9, Wednesday, February 28, 2001, 15:29:30
$!
...
void TpMatchManager::readPointsIbis(char *filename)
{
    ...
    cout << "Reading tiepoint file " << _ibisFilename << endl;

    status = zvunit(&unit, "tp_file_1",  1, "u_name", _ibisFilename, 0);
    if (status != OK) {
...
void TpMatchManager::writePointsIbis(char *filename)
{
    ...
    status = zvunit(&unit, "tp_file_2", 1, "u_name", _ibisFilename, 0);
    if (status != OK) {
        theErrorManager->process(Error, "zvunit", "Cant process file");
	return;
    }
