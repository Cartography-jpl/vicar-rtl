...
$! VPACK Version 1.9, Wednesday, April 26, 2000, 16:13:30
$!
...
{ long	LeapYear,
  ...
  if (Julian=(Time[8] == 'T'))		// Auto-detects Julian Format
     sscanf(Time,"%4d-%3dT%2d:%2d:%2d.%3d",
            &Year,&Day,&Hour,&Minute,&Second,&Milli);
  else sscanf(Time,"%4d-%2d-%2dT%2d:%2d:%2d.%3d",
              &Year,&Month,&Day,&Hour,&Minute,&Second,&Milli);
  ...
  /**/
  if (Year < _epochYear)
  { sprintf(_errorMessage,"Supplied year is less than the EPOCH (%d vs %d)",
            Year,_epochYear);
    return RTN_INVLD_ARG;
  } else if (Year >= (_epochYear + MAX_RANGE_YEARS))
  { sprintf(_errorMessage,
            "Supplied year is larger than EPOCH can handle (%d vs %d)",
  ...
  { if ((Day > DAYS_PER_LEAP) || (!LeapYear && Day >DAYS_PER_YEAR))
    { sprintf(_errorMessage,"Day of year out of range: %d",Day);
      return RTN_INVLD_ARG;
    }
  } else
  { if (!Julian && (Month < 1 || Month > 12))
    { sprintf(_errorMessage,"Month is invalid: %d",Month);
      return RTN_INVLD_ARG;
    }
    if (Day > dom[Month])
    { if (Day > (dom[Month]+1) || (!LeapYear && Day > dom[Month]))
      { sprintf(_errorMessage,"Invalid number of days (%d) for month (%d)",
                Day,Month);
        return RTN_INVLD_ARG;
      }
    }
...
{ int	PartialSeconds;
  ...
  if (PartialSeconds < 0 &&
      ((-PartialSeconds) / PS_PER_SECOND + 1) > _seconds)
  { sprintf(_errorMessage,
...
#define MODULE_LIST SfocTime.cc SclkTime.cc ChdoBase.cc Chdo002.cc Chdo010.cc Chdo082.cc Chdo128.cc
