...
$! VPACK Version 1.9, Wednesday, January 24, 2007, 11:40:57
$!
...
ostream &operator<<(ostream &ostr, const TpQualifier &q) 
{
    switch (q._type) {
    case TpFull:
	ostr << q._value.f;
...
void TpQualGroup::setValue(int n, int value)
{
    std::ostrstream outStream;
    outStream << value << '\0';
    _qualifiers[n]->setValue(outStream.str());
}
...
ostream &operator<<(ostream &ostr, const TpQualGroup &qg)
{
    int i;
...
ostream &operator<<(ostream &ostr, const TpQualGroupMgr &m)
{
    for (int i = 0; i < m._numGroups; i++) 
	ostr << *(m._groups[i]) << endl;
