...
$! VPACK Version 1.8, Tuesday, September 30, 1997, 14:40:26
$!
...
/***   :                                         */
/************************************************************************************/
zapdsline( optr, type, labelitem, value, number )
/* int	optr;*/
FILE	*optr;
char	type;
char	labelitem[];
...
int	length,		/* String length variable			*/
	...
	stat,		/* Returned status indicator			*/
...
if ( number < 1 )	/* Check for zero or negative NUMBER parameter	*/
	...
	number = 1;
	zvmessage("*** WARNING: NUMBER of values negative or zero. ",0);
	sprintf(strng,
...
default:
		...
		zvmessage(strng,0);
		zvmessage(" ",0);
		zvmessage("     for label item",0);
		sprintf(strng,"     %s",labelitem);
		zvmessage(strng,0);
		zvmessage(" ",0);
		zvmessage("  No PDS line written.",0);
		zvmessage("************************",0);
		zvmessage("************************",0);
		break;		}
}
