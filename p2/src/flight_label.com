...
$! VPACK Version 1.9, Wednesday, July 01, 1998, 15:40:18
$!
...
int unit;		/* Unit number of file whose label is listed	      */
{
 ...
 char   boom[8],	/* Array of possible boom obstructions		*/
	fltr[8],	/* Array of possible filters			*/
...
int	flags,		/* Flag to determine if file is open		*/
	...
	mod8,		/* Last digit of RIM				*/
	mod10,		/* Eleventh digit of RIM			*/     
	mod91,		/* Ninth and tenth digits of RIM		*/
	nhist,		/* Number of tasks in VICAR label		*/
	rim,		/* First eight digits of RIM (int[100xRIM])	*/
...
phase = 0;
galsosdone=0;
sta[0] = zvget(unit,"FLAGS",&flags, 0);
if((flags & OPEN)==0)
  sta[0] = zvopen(unit,"OP","READ","OPEN_ACT","SA", 0);

sta[0] = zvget(unit,"FORMAT",format, 0);  /* Get input file data format  */
...
nhist = MAX;
strcpy( task,"NONE" );
status = zlhinfo(unit,tasks,instances,&nhist,"ULEN",15,"ERR_ACT","SA", 0);
for(x=0;x<nhist;x++)
 {
   if (!strncmp(tasks[x],"GALSOS",6)) galsosdone=1;
   sta[0] = zlget(unit,"HISTORY","SENSOR",v[0],"HIST",tasks[x], 0);
...
type = NON_COMPRSSD;
sta[0] = zlget(unit,"HISTORY","ENCODING_TYPE",v[0], "HIST", task, 0);
if ( sta[0] == 1 )  { 
...
i=0;
sta[i] = zlget(unit,"HISTORY","SENSOR", v[i],"HIST",task, 0); i_sensor = i; i++;
sta[i] = zlget(unit,"HISTORY","PICNO",  v[i],"HIST",task, 0); i_picno = i; i++;
sta[i] = zlget(unit,"HISTORY","TLMFMT", v[i],"HIST",task, 0); i_tlmfmt = i; i++;
sta[i] = zlget(unit,"HISTORY","TARGET", v[i],"HIST",task, 0); i_target = i; i++;
sta[i] = zlget(unit,"HISTORY","TCA",    v[i],"HIST",task, 0); i_tca  = i; i++; 
sta[i] = zlget(unit,"HISTORY","PA",     v[i],"HIST",task, 0); i_pa = i; i++;
sta[i] = zlget(unit,"HISTORY","BOOM",   v[i],"HIST",task, 0); i_boom = i; i++;
sta[i] = zlget(unit,"HISTORY","CAL",    v[i],"HIST",task, 0);
if (sta[i] != 1 && galsosdone) sta[i] = zlget(unit,"HISTORY","CAL",    v[i],"HIST","GALSOS", 0);
i_cal = i; i++;
sta[i] = zlget(unit,"HISTORY","UBWC",   v[i],"HIST",task, 0); i_ubwc  = i; i++;
sta[i] = zlget(unit,"HISTORY","DC",     v[i],"HIST",task, 0);
if (sta[i] != 1 && galsosdone) sta[i] = zlget(unit,"HISTORY","DC",     v[i],"HIST","GALSOS", 0);
i_dc = i; i++;
sta[i] = zlget(unit,"HISTORY","BLM",    v[i],"HIST",task, 0);
if (sta[i] != 1 && galsosdone) sta[i] = zlget(unit,"HISTORY","BLM",    v[i],"HIST","GALSOS", 0);
i_blm  = i; i++;
sta[i] = zlget(unit,"HISTORY","SO",     v[i],"HIST",task, 0);
if (sta[i] != 1 && galsosdone) sta[i] = zlget(unit,"HISTORY","SO",     v[i],"HIST","GALSOS", 0);
i_so = i; i++;
sta[i] = zlget(unit,"HISTORY","QUANTIZATION_MATRIX_NAME", v[i],"HIST",task,0); i_qm =i; i++;
sta[i] = zlget(unit,"HISTORY","ZIGZAG_PATTERN", v[i],"HIST",task,0); i_zz = i; i++;
sta[i] = zlget(unit,"HISTORY","HUFFMAN_TABLE_NAME", v[i],"HIST",task,0); i_huff = i; i++;
if ( type == OLD_STYLE_LABEL ) {
	sta[i] = zlget(unit,"HISTORY","BARC",   v[i],"HIST",task, 0); i_comp = i; i++;
	sta[i] = zlget(unit,"HISTORY","FIBE", v[i],"HIST",task, 0); i_fibe = i; i++; 
	}
else {
	sta[i] = zlget(unit,"HISTORY","ENCODING_TYPE", v[i],"HIST",task, 0); i_comp = i; i++;
	sta[i] = zlget(unit,"HISTORY","MOFIBE", v[i],"HIST",task, 0); i_fibe = i; i++;	
	}
if( strcmp(format,"HALF")==0 ) {
  	sta[i] = zlget(unit,"HISTORY","EDRTAPE",v[i],"HIST",task, 0); i_edrtape = i; i++;
  	}
else {
  	sta[i] = zlget(unit,"HISTORY","REDRTAPE",v[i],"HIST",task, 0); i_edrtape = i; i++;
  	}
...
i = 0;
sta[i] = zlget(unit,"HISTORY","FILTER", 	&n[i],"HIST",task, 0); i_filter  = i; i++;
sta[i] = zlget(unit,"HISTORY","PARTITION", 	&n[i],"HIST",task, 0); i_partition = i; if (sta[i]==1) phase = PHASEII; i++;
sta[i] = zlget(unit,"HISTORY","RIM",    	&n[i],"HIST",task, 0); i_rim = i; i++;
sta[i] = zlget(unit,"HISTORY","MOD91",  	&n[i],"HIST",task, 0); i_mod91 = i; i++;
sta[i] = zlget(unit,"HISTORY","MOD10",  	&n[i],"HIST",task, 0); i_mod10 = i; i++;
sta[i] = zlget(unit,"HISTORY","MOD8",   	&n[i],"HIST",task, 0); i_mod8 = i; i++;
sta[i] = zlget(unit,"HISTORY","SCETYEAR",	&n[i],"HIST",task, 0); i_scetyear  = i; i++;
sta[i] = zlget(unit,"HISTORY","SCETDAY", 	&n[i],"HIST",task, 0); i_scetday = i; i++; 
sta[i] = zlget(unit,"HISTORY","SCETHOUR",	&n[i],"HIST",task, 0); i_scethour = i; i++;
sta[i] = zlget(unit,"HISTORY","SCETMIN", 	&n[i],"HIST",task, 0); i_scetmin= i; i++;
sta[i] = zlget(unit,"HISTORY","SCETSEC", 	&n[i],"HIST",task, 0); i_scetsec = i; i++;
sta[i] = zlget(unit,"HISTORY","SCETMSEC", 	&n[i],"HIST",task, 0); i_scetmsec = i; i++;
sta[i] = zlget(unit,"HISTORY","GAIN",     	&n[i],"HIST",task, 0); i_gain = i; i++;
sta[i] = zlget(unit,"HISTORY","RATE",     	&n[i],"HIST",task, 0); i_rate = i; i++;
sta[i] = zlget(unit,"HISTORY","QUANTIZATION_STEP_SIZE", &n[i],"HIST",task, 0); i_qstep = i; i++;
sta[i] = zlget(unit,"HISTORY","CUT_OUT_WINDOW", &n[i],"HIST", task, 
		"ELEMENT", 1, 0);i_roi = i; i++;
sta[i] = zlget(unit,"HISTORY","CUT_OUT_WINDOW", &n[i],"HIST", task, 
		"ELEMENT", 2, 0); i++;
sta[i] = zlget(unit,"HISTORY","CUT_OUT_WINDOW", &n[i],"HIST", task, 
		"ELEMENT", 3, 0); i++;
sta[i] = zlget(unit,"HISTORY","CUT_OUT_WINDOW", &n[i],"HIST", task, 
		"ELEMENT", 4, 0); i++;
sta[i] = zlget(unit,"HISTORY","TRUTH_WINDOW",   &n[i],"HIST", task, 
		"ELEMENT", 1, 0);i_tw = i; i++;
sta[i] = zlget(unit,"HISTORY","TRUTH_WINDOW",   &n[i],"HIST", task, 
		"ELEMENT", 2, 0); i++;
sta[i] = zlget(unit,"HISTORY","TRUTH_WINDOW",   &n[i],"HIST", task, 
		"ELEMENT", 3, 0); i++;
sta[i] = zlget(unit,"HISTORY","TRUTH_WINDOW",   &n[i],"HIST", task, 
		"ELEMENT", 4, 0); i++;
if( strcmp(format,"HALF")==0 ) {
   sta[i] = zlget(unit,"HISTORY","EDRFILE", &n[i],"HIST",task, 0); i_edrfile = i; i++;
}
else {
   sta[i] = zlget(unit,"HISTORY","REDRFILE",&n[i],"HIST",task, 0); i_edrfile = i; i++;
}
sta[i] = zlget(unit,"HISTORY","SEQNO",&n[i],"HIST",task,0);i_seqno=i; i++;
sta[i] = zlget(unit,"HISTORY","ICT_DESPIKE_THRESHOLD",&n[i],"HIST",task,0);i_ict_despike_t=i; i++;

sta[i] = zlget(unit,"HISTORY","NSTARS",&n[i],"HIST",task,0);i_nstars=i; i++;
i_star1=i;
strcpy(STARn,"STAR ");
for (x=1; x<= n[i_nstars]; x++)
  ...
  STARn[4]=(char) (48 + x);
   for ( y=1; y<=4; y++){         
           ...
           sta[i] = zlget(unit,"HISTORY",STARn,&n[i],"HIST",task,"ELEMENT",y,0);
           i++;
			}
}
...
i=0;
sta[i] = zlget(unit,"HISTORY","EXP",    &r[i],"HIST",task, 0);  i_exp = i; i++; 
sta[i] = zlget(unit,"HISTORY","TBPPXL", &r[i],"HIST",task, 0);  i_tbppxl = i; i++; 
sta[i] = zlget(unit,"HISTORY","INA",    &r[i], "HIST",task, 0); i_ina = i; i++;
sta[i] = zlget(unit,"HISTORY","TWIST",  &r[i], "HIST",task, 0); i_twist = i; i++;
sta[i] = zlget(unit,"HISTORY","SUNAZ",  &r[i], "HIST",task, 0); i_sunaz = i; i++;
sta[i] = zlget(unit,"HISTORY","HSCL",   &r[i], "HIST",task, 0); i_hscl = i; i++;
sta[i] = zlget(unit,"HISTORY","EMA",    &r[i], "HIST",task, 0); i_ema = i; i++;
sta[i] = zlget(unit,"HISTORY","CONE",   &r[i], "HIST",task, 0); i_cone = i; i++;
sta[i] = zlget(unit,"HISTORY","SMRAZ",  &r[i], "HIST",task, 0); i_smraz = i; i++;
sta[i] = zlget(unit,"HISTORY","SMEAR",  &r[i],"HIST",task, 0); i_smear = i; i++;
sta[i] = zlget(unit,"HISTORY","VSCL",   &r[i],"HIST",task, 0); i_vscl = i; i++;
sta[i] = zlget(unit,"HISTORY","PHA",    &r[i],"HIST",task, 0); i_pha = i; i++;
sta[i] = zlget(unit,"HISTORY","RA",     &r[i],"HIST",task, 0); i_ra = i; i++;
sta[i] = zlget(unit,"HISTORY","SCAZ",   &r[i],"HIST",task, 0); i_scaz = i; i++;
sta[i] = zlget(unit,"HISTORY",lat_rad,  &r[i],"HIST",task, 0); i_latrad = i; i++;
sta[i] = zlget(unit,"HISTORY","PLRANGE",&r[i],"HIST",task, 0); i_plrange = i; i++;
sta[i] = zlget(unit,"HISTORY","HRA",    &r[i],"HIST",task, 0); i_hra = i; i++;
sta[i] = zlget(unit,"HISTORY","DEC",    &r[i],"HIST",task, 0); i_dec = i; i++;
sta[i] = zlget(unit,"HISTORY","NORAZ",  &r[i],"HIST",task, 0); i_noraz = i; i++;
sta[i] = zlget(unit,"HISTORY","LON",    &r[i],"HIST",task, 0);
if (sta[i] != 1) sta[i] = zlget(unit,"HISTORY","LOG",    &r[i],"HIST",task, 0); /*try old style */
i_lon = i; i++;
sta[i] = zlget(unit,"HISTORY","SLRANGE",&r[i],"HIST",task, 0); i_slrange = i; i++;
sta[i] = zlget(unit,"HISTORY","IOF",    &r[i],"HIST",task, 0); i_iof = i; i++;
sta[i] = zlget(unit,"HISTORY","SOLRANGE",&r[i],"HIST",task, 0);i_solrange = i; i++;
sta[i] = zlget(unit,"HISTORY","CNV",    &r[i],"HIST",task, 0); i_cnv = i; i++;
sta[i] = zlget(unit,"HISTORY","COMPRESSION_RATIO", &r[i],"HIST",task, 0); i_compratio = i; i++;
sta[i] = zlget(unit,"HISTORY","MAXIMUM_COMPRESSION_RATIO", &r[i],"HIST",task, 0); i_max_comp = i; i++;
sta[i] = zlget(unit,"HISTORY","MINIMUM_COMPRESSION_RATIO", &r[i],"HIST",task, 0); i_min_comp = i; i++;
sta[i] = zlget(unit,"HISTORY","TPPLNE", &r[i],"HIST",task, 0); i_tpplne = i; i++;

sta[i] = zlget(unit,"HISTORY","ENTRPY",&r[i],"HIST",task, 0); /*try old style */
if (sta[i] != 1) /* new style */
	sta[i] = zlget(unit,"HISTORY","ENTROPY",&r[i],"HIST",task, 0);  i_entropy = i; i++;

sta[i]=zlget(unit,"HISTORY","SUB_SOLAR_LATITUDE",&r[i],"HIST",task,0); i_sub_solar_lat=i; i++;
sta[i]=zlget(unit,"HISTORY","SUB_SOLAR_LONGITUDE",&r[i],"HIST",task,0); i_sub_solar_long=i; i++;
sta[i]=zlget(unit,"HISTORY","SUB_SPACECRAFT_LATITUDE",&r[i],"HIST",task,0); i_sub_spacecraft_lat=i; i++;
sta[i]=zlget(unit,"HISTORY","SUB_SPACECRAFT_LONGITUDE",&r[i],"HIST",task,0); i_sub_spacecraft_long=i; i++;
