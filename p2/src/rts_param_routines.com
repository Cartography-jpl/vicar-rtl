$!****************************************************************************
$!
$! Build proc for MIPL module rts_param_routines
$! VPACK Version 1.8, Monday, March 25, 1996, 15:47:06
$!
$! Execute by entering:		$ @rts_param_routines
$!
...
$!****************************************************************************
$!
$ write sys$output "*** module rts_param_routines ***"
$!
$ Create_Source = ""
$ Create_Repack =""
$ Create_Test = ""
...
$ write sys$output "Invalid argument given to rts_param_routines.com file -- ", primary
...
$!
$Run_Make_File:
$   if F$SEARCH("rts_param_routines.imake") .nes. ""
$   then
$      vimake rts_param_routines
$      purge rts_param_routines.bld
$   else
$      if F$SEARCH("rts_param_routines.bld") .eqs. ""
$      then
$         gosub Imake_File
$         vimake rts_param_routines
$      else
...
$   if (primary .eqs. " ")
$   then
$      @rts_param_routines.bld "STD"
$   else
$      @rts_param_routines.bld "''primary'" "''secondary'"
$   endif
$ Return
$!#############################################################################
$Repack_File:
$ create rts_param_routines.repack
$ DECK/DOLLARS="$ VOKAGLEVE"
$ vpack rts_param_routines.com -
	-s rts_param_routines.c -
	-i rts_param_routines.imake -
	-t tst_rts_param_routines.c tst_rts_param_routines.imake test_param.cnf -
	   test_param.upf readme_rts_param.tst
$ Exit
$ VOKAGLEVE
$ Return
$!#############################################################################
$Source_File:
$ create rts_param_routines.c
$ DECK/DOLLARS="$ VOKAGLEVE"
...
$!#############################################################################
$Imake_File:
$ create rts_param_routines.imake
/******************************************************************************
/*
/*                     IMAKE FILE FOR MODULE rts_param_routines
/*
/*   To Create the build file give the command:
/*
/*		$ vimake rts_param_routines			(VMS)
/*   or
/*		% vimake rts_param_routines			(Unix)
/*
/*****************************************************************************/

/***  Define for whom this file exisits  ***/
#define SUBROUTINE rts_param_routines		/* Only one of these */
/*#define PROGRAM rts_param_routines		/* Only one of these */

/***  List all modules which are used by locally by this module  ***/
#define MODULE_LIST rts_param_routines.c
...
/**********  End of rts_param_routines imake file  **********/
$ Return
$!#############################################################################
$Test_File:
$ create tst_rts_param_routines.c
/**  Copyright (c) 1995, California Institute of Technology		**/
/**  U. S. Government sponsorship under NASA contract is acknowledged	**/
...
{ int	elements;
  ...
  { printf("No files to parse\n\n");
    ...
    printf("\t3rd - UPF file (output) - optional\n");
    exit(0);
  }
  ...
  if (argc > 3) write_param_file(argv[3],root);
...
$!-----------------------------------------------------------------------------
$ create tst_rts_param_routines.imake
/******************************************************************************
/*
/*                     IMAKE FILE FOR MODULE tst_rts_param_routines
/*
/*   To Create the build file give the command:
/*
/*		$ vimake tst_rts_param_routines			(VMS)
/*   or
/*		% vimake tst_rts_param_routines			(Unix)
/*
/*****************************************************************************/

/***  Define for whom this file exisits  ***/
/*#define SUBROUTINE tst_rts_param_routines		/* Only one of these */
#define PROGRAM tst_rts_param_routines		/* Only one of these */
#define TEST

/***  List all modules which are used by locally by this module  ***/
#define MODULE_LIST tst_rts_param_routines.c
...
TestProgram.Path.Rsrc2.ParamType :\
					real
TestProgram.Path.Rsrc2.ParamValid:	: 999.99,2000.0:
TestProgram.Path.Rsrc2.ParamDefault:	666.0
TestProgram.Path.Rsrc2.TclName:		TstReal
TestProgram.Path.Rsrc2.TclForm:		2
!
TestProgram.Path.RsrcA.ParamName: 	Biggy
TestProgram.Path.RsrcA.ParamType :	double
TestProgram.Path.RsrcA.ParamValid:	: 9876543421.99
TestProgram.Path.RsrcA.ParamDefault:	666.0
TestProgram.Path.RsrcA.TclName:		TstBiggy
TestProgram.Path.RsrcA.TclForm:		2
!
...
TestProgram.Path.Rsrc5.ParamValid:	:
  ...
  1) A symbolic command must be defined before the test program can be run.
	...
	CMND :== $DEV:[USA001.DIRECTORY]PROGRAM.EXE
		...
		[USA001.DIRECTORY] is the directory/pathname where the
			test program exists
		PROGRAM.EXE is the test program (TST_RTS_PARAM_ROUTINES.EXE)

     This symbolic command must be redefined every time you log in.

  2) Execute the following command:
	CMND TEST_PARAM.CNF TEST_PARAM.UPF TEST.OUT

For UNIX:

  From the directory where the test program is built, execute the command:
	./tst_rts_param_routines test_param.cnf test_param.upf test.out

This is just a short test which defines a few parameters from the configuration
file, modifies them with a parameter file, writes the current parameter values
out to a new parameter file, and displays information about each parameter.
A more extensive test of the parameter definition capability would be to use
the delivered configuration file for GLLTELEMPROC (glltelemproc.cnf) instead
of TEST_PARAM.CNF.  The parameter file will not match this configuration file
(it will generate some undefined keyword errors ... which it should), but
there will be more parameters defined.
$ Return
$!#############################################################################
