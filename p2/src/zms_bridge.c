...
double	*lambda;
{
   F77_FUNC(bodeul, BODEUL) 
( &body, &et, ra, dec, w, lambda );
}
...
char *item;
{
   ...
   status = F77_FUNC(xms_bodfnd, XMS_BODFND) 
...
double *values;
{
   ...
   F77_FUNC(xms_bodvar, XMS_BODVAR) (&body, item, &i, dim, values);
}
...
int *status;   		/*output*/
{
   ...
   F77_FUNC(xms_ckgp, XMS_CKGP) 
(&inst,&sclkdp,&tol,ref,&i,cmat,clkout,status);
}
...
int *status;   		/*output*/
{
   ...
   F77_FUNC(xms_ckgpav, XMS_CKGPAV) 
(&inst,&sclkdp,&tol,ref,&i,cmat,av,clkout,status);
 }
...
void *sum; /*input*/
{
   ...
   F77_FUNC_(xms_dafbna, XMS_DAFBNA) 
(&handle, sum, name, &i);
}
...
int *status;     /*outpu*/
{
   F77_FUNC_(xms_daffna, XMS_DAFFNA) 
(status);
}
...
int *handle;
{
   ...
   F77_FUNC_(xms_dafopr, XMS_DAFOPR) 
(fname, &i, handle);
}
...
int *handle;
{
   ...
   F77_FUNC_(xms_dafopw, XMS_DAFOPW) 
(fname, &i, handle);
}
...
{
  double dpr();
  double F77_FUNC(dpr, DPR) ();

  return F77_FUNC(dpr, DPR) ();
}
...
{
  double rpd();
  double F77_FUNC(rpd, RPD) ();

  return F77_FUNC(rpd, RPD) ();
}
