$!****************************************************************************
$!
$! Compile+link proc for MIPL program vic1lab
$! VPACK Version 1.2, Tuesday, March 10, 1992, 17:32:18
$!
...
$!****************************************************************************
$!
$ write sys$output "*** Program vic1lab ***"
$!
$ Create_Source = ""
$ Create_Repack =""
$ Create_Test = ""
...
void FTN_NAME(vic1lab)(int *unit, int *status, int *nlabs, char *buf,
...
void FTN_NAME(vic1labx)(unit, status, nlabs, buf, maxlabs)
int *unit;			/* In: unit number */
int *status;			/* Out: status value */
int *nlabs;			/* Out: number of 72-byte labels returned */
...
/* Get the name of the first task */
   ...
   ninst = 1;
   status = zlhinfo(unit, task, &instance, &ninst, 0);
   if (status != SUCCESS)
      return status;
