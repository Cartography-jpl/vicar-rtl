...
$! VPACK Version 1.8, Tuesday, July 15, 1997, 18:07:43
$!
			...
			int *tp_status)
{
    ...
    int *gen_quals_full;
    float *gen_quals_real;
    char *gen_quals_text;
    int *pnt_quals_full;
    float *pnt_quals_real;
    char *pnt_quals_text;
    ...
    status = zvunit(&unit, "dp_1",  1, "u_name", point_file_ibis, 0);
    if (status != OK) {
    ...
    *npoints = 0;
    status = zvunit(&unit, "dp_2",  1, "u_name", point_file_ibis, 0);
    if (status != OK) {
        fprintf(stderr, "Error occurred in zvunit, no changes were made\n");
	*npoints = 0;
        return NULL;
