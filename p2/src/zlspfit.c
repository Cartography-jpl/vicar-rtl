...
int zclfit (int ifit, double *mom, double *umom, double *coef)
{
   ...
   F77_FUNC(clfit, CLFIT)
(&ifit, mom, umom, coef, &ier);
...
{
   F77_FUNC(rmsfit, RMSFIT)
(&mode, &npow, pts, &n, coef, drms, dmax, imax);
}
