...
$! VPACK Version 1.9, Thursday, June 18, 1998, 16:07:02
$!
...
void FTN_NAME(get_seconds)(sec_out)
long *sec_out;
{
  time_t thetime;
  char msg[100];
  ...
  if(TEST_PURPOSE) {
     ...
     zvmessage("*************************************************","");
     sprintf(msg,"Time returned by time function is %u",thetime);
     zvmessage(msg,0);
  ...
  *sec_out = (long) (thetime & Y2K_CORRECTION);
  ...
  if(TEST_PURPOSE) {
     sprintf(msg,"Time after offseted by get_seconds is %u",thetime);
     zvmessage(msg,0);
     sprintf(msg,"Time after corrected by get seconds is %d",*sec_out);
     zvmessage(msg,0);
  }
