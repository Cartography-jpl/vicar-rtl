...
$! VPACK Version 1.9, Wednesday, November 28, 2001, 11:46:36
$!
...
buf_union_typ *buf;
{
  ...
  FTN_NAME(getspice2) (&unit,&provenance,buf->intbuf,&ind);
  return(ind);
}
...
buf_union_typ *buf;	/* output SPICE/SEDR buffer */
{
  ...
  strncpy(&data[24],ztarget,12);
  n = strlen(project);  
  FTN_NAME(xgetspice4) (project,&n,&provenance,data,buf->intbuf,&ind);
  return(ind);
}
 ...
 *========================================================*/
void FTN_NAME(putspice2) (char *source, char *program, int buf[200],
...
int buf[200];
{
  ...
  zccase(userid,1,6);
  strncpy(&buf[10],source,4);
  strncpy(&buf[171],"NONE",4);
  strcpy(&buf[172],purpose);
  strncpy(&buf[173],program,6);
  strcpy(&buf[175],reqnum);
  strncpy(&buf[176],userid,6);
  strncpy(&buf[188],institute,4);
