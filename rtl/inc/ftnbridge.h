...
/* case - it cannot be done automatically.				*/
/*									*/
...
#if SUN3_ARCH+SUN4_ARCH+SUN_SOLARIS_ARCH+ALLIANT_ARCH+DECSTATION_ARCH+MAC_AUX_ARCH+SGI_ARCH+AXP_UNIX_ARCH+ANY_LINUX_ARCH+MAC_OSX_ARCH
#ifdef __STDC__
...
/* All the fortran names have been changed to use the standard
   GNU F77_FUNC and F77_FUNC_ macros. These get defined on the compile
   line. If these aren't defined, then default to the old FTN_NAME macro. */
#ifndef F77_FUNC
#define F77_FUNC(name,NAME) FTN_NAME(name)
#endif
#ifndef F77_FUNC_
#define F77_FUNC_(name,NAME) FTN_NAME(name)
#endif
...
#if SUN3_ARCH + AXP_LINUX_ARCH + X86_LINUX_ARCH + X86_NT_ARCH
