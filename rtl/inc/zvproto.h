...
int zvzinit(			/* DON'T CALL FROM APPLICATION CODE */
		int argc,
		char *argv[]);
void z_exit(int code, char* msg);
void make_upper_case(char *out, char *in);
void make_upper_case_max(char* out, char *in, int max);
...
/* Useful extension in GCC that lets us have the compiler check that
   we end the variable length argument with a NULL. However this is
   only available in GCC version 4.0 or later.
*/
#if __GNUC__ > 3
#define SENTINAL __attribute__((__sentinel__(0)))
#else
#define SENTINAL
#endif
...
void zprnt(int dcode, int n, void* buf, char* title);
