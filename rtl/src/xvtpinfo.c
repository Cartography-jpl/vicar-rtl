...
int F77_FUNC(xvtpinfo, XVTPINFO)
(char *sym_name, char *dev_name, int *tfile, int *trec,
							ZFORSTR_PARAM)
...
int zvtpinfo(char * UNUSED(sym_name), char * dev_name, 
	     int * tfile, int * trec)
{
   ...
   make_upper_case_max(c_sym_name, sym_name, MAX_STRING_SIZE);

/* Find tape index */

   status = i_analyze(c_sym_name, i_tape, i_count, &t_index, &t_file);
   if (status != I_TAPE) {
      dev_name[0] = '\0';
      *tfile = *trec = 0;
   ...
   i_crack(i_tape[t_index], c_sym_name, dev_name);  /* Look up device name */
