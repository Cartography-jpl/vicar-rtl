...
void v2_collect_history_info()
{
   ...
   status = get_one_string_parm("_PROC", 0, &value);	/* Get proc name */
   if (status == SUCCESS) {
   ...
   make_upper_case(history[TASK].value, history[TASK].value);
   ...
   if(pw)
     ...
     value = 0;
#endif
   /* Allow the username to be overriden. This is used in testing when
      we want a fixed username to appear in a history, regardless of
      who is running this command. This wouldn't be used in normal
      VICAR runs. */
   if(getenv("AFIDS_FAKE_TEST_USER"))
     value = getenv("AFIDS_FAKE_TEST_USER");
   if(!value) {
   ...
   value = ctime(&tim);
   /* Allow the datetime to be overriden. This is used in testing when
      we want a fixed username to appear in a history, regardless of
      who is running this command. This wouldn't be used in normal
      VICAR runs. */
   if(getenv("AFIDS_FAKE_TEST_DAT_TIM"))
     value = getenv("AFIDS_FAKE_TEST_DAT_TIM");
