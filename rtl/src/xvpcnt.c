...
#if VMS_OS
void F77_FUNC(xvpcnt, XVPCNT)
(char *key, int *count, int *def, ZFORSTR_PARAM)
/* int *def;	out: Default flag ***VMS OLD COMPAT ONLY!!  DON'T USE!!*** */
#else
void F77_FUNC(xvpcnt, XVPCNT)
(char *key, int *count, ZFORSTR_PARAM)
#endif
...
void XVPCNT(void *UNUSED(key), int * count)
{
   current_call = VPCNT;
   *count = 0;
...
void XVIPCNT(void * UNUSED(key), int *count)
{
   current_call = VIPCNT;
   *count = 0;
   error_handler(NO_UNIT, NO_TAE_SUPPORT);
