...
int v2_initialize_from_label(unit)
int unit;
{
...
/* input default is in the table and no special handling is needed here.     */
   ...
   for (i=0; i<N_UNIT_TABLE_ENTRIES; i++) {
	 ...
	 if (!find_entry(temp, unit_table[i].name, &value, &vallen, &ip)) {
	    if (essential[i]) {
               free(temp);
	       return FAILURE;
            }
	    if (i == HOST)	/* special defaults for non-existent labels */
	       add_lbl_item_value_tbl(unit,i,DEF_HOST_LABEL);
	    if (i == INTFMT)
	       add_lbl_item_value_tbl(unit,i,DEF_INTFMT_LABEL);
	    if (i == REALFMT)
	       add_lbl_item_value_tbl(unit,i,DEF_REALFMT_LABEL);
	 }
	 else			/* add 'value' to current value table */
	    add_lbl_item_value_tbl(unit,i,dequoted(value,vallen));
      }
   }
