...
int open_act_preset_c(unit, nopts, params)
...
va_list *params;			/* optional arguments pointer	    */
{
      ...
      if (s && s[0] != '\0') {
         ...
         if (strlen(s) <= PARAM_NAME_LENGTH) {
            ...
            if (EQUAL(keyword, "OPEN_ACT")) {
               ...
               status = add_str_current_table(value.s, OPEN_ACT,
	       ...
	       if (status != SUCCESS)
	          return UNABLE_TO_STORE_OPTIONAL;
               ...
               status = add_msg_current_table(value.s, OPEN_MES,
					current_table[unit], default_table);
	       if (status != SUCCESS)
	          return UNABLE_TO_STORE_OPTIONAL;
            }
   ...
   } while (s && s[0] != '\0');		/* end main optionals loop */
...
int open_act_preset_for(unit, nopts, nargs, nconst, params)
int unit;				/* Unit number			    */
...
int format_preset_for(unit, nopts, nargs, nconst, params)
int unit;				/* Unit number			    */
int *nopts;				/* Returned # of opt args	    */
int *nargs;				/* Returned # of args overall	    */
int nconst;				/* Input # of constant args	    */
   ...
   do {
      ...
      if (s[0] != ' ' && s[0] != '\0') {
         ...
         if (v2_preset_equals(s, "FORMAT")) {
            ...
            status = add_str_current_table(value.s, LFORMAT,
	    ...
	    format_found = TRUE;
            ...
            status = add_str_current_table(value.s, ERR_ACT,
					label_table[unit], label_default_table);
	    if (status != SUCCESS)
	       return UNABLE_TO_STORE_OPTIONAL;
         }
            ...
            status = add_msg_current_table(value.s, ERR_MESS,
					label_table[unit], label_default_table);
	    if (status != SUCCESS)
	       return UNABLE_TO_STORE_OPTIONAL;
         }
