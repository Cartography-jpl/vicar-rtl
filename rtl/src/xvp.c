...
void F77_FUNC(xvp, XVP)
(char *name, char *value, int *count, ...)
...
/* int *count;		Out: # of values.  0 if not found or default. */
{
   ...
   get_parm_for((struct PARBLK*) &parb, c_name, count, &def, 0, value, 
...
int *count;		/* Out: # of values.  0 if not found or defaulted. */
{
   ...
   status = get_parm_c((struct PARBLK*) &parb, name, value, count, &def, 
		       0, 0, FALSE);
...
/* int *count;		Out: # of values.  0 if not found or default. */
{
   ...
   get_parm_for((struct PARBLK*) &iparb, c_name, count, &def, 0, value, 
		&name, 3, 2, 2, &param, &which, FALSE);
...
int *count;		/* Out: # of values.  0 if not found or defaulted. */
{
   ...
   status = get_parm_c((struct PARBLK*) &iparb, name, value, count, &def, 
		       0, 0, FALSE);
