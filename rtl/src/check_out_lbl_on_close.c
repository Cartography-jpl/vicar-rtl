...
int v2_check_out_lbl_on_close(unit)
int unit;
{
   int i, status;
   ...
   for (i=0; i<nrecs; i++) {		/* write out any full records first */
      status = write_rec((struct bufstate *) CURRENT_IP_VALUE(BUFSTATE), labels, i,0,0, &no_trans);
      labels += recsize;
      if (status != SUCCESS) {
         if (eol_labels != NULL)
            free(eol_labels);
   ...
   if (left != 0) {			/* (probably shouldn't happen) */
      status = write_rec((struct bufstate *) CURRENT_IP_VALUE(BUFSTATE), labels, nrecs, 0, left,
				&no_trans);
      if (status != SUCCESS) {
         if (eol_labels != NULL)
            free(eol_labels);
   ...
   if (eol_labels != NULL) {	/* A second set of labels exists */
      ...
      for (i=0; i<nrecs; i++) {		/* write out any full records first */
         status = write_rec((struct bufstate *) CURRENT_IP_VALUE(BUFSTATE), labels, eol_rec+i, 0, 0,
				&no_trans);
         labels += recsize;
         if (status != SUCCESS) {
            if (eol_labels != NULL)
