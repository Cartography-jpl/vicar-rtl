...
int v2_process_input_file(unit)
int unit;
{
   int status;
...
/* guarantee that ORG from the label matches U_ORG.		*/
	 ...
	 if (!EQUAL(CURRENT_S_VALUE(U_ORG),CURRENT_S_VALUE(ORG)))
      ...
      if (strlen(CURRENT_S_VALUE(I_FORMAT)) != 0)
         add_str_current_table(CURRENT_S_VALUE(I_FORMAT), FORMAT,
...
/* If the user gave U_ORG, then use that value, otherwise default to BSQ */
      ...
      status = add_str_current_table(org_value, ORG, current_table[unit],
							   default_table);
      if (status != SUCCESS)
...
/* interleaved by line), and BIP (band interleaved by pixel).		*/
   ...
   if (strlen(CURRENT_S_VALUE(U_FORMAT))==0)
      add_str_current_table(CURRENT_S_VALUE(FORMAT), U_FORMAT,
				  current_table[unit], default_table);
   ...
   status=check_primary_input(unit);
   if (status != SUCCESS) {
      v2_close_down(unit);
      return status;
   }
