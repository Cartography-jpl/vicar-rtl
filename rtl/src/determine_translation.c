...
typedef int (*fnptr)(void *from, void *to, int len, struct trans *trans);

static fnptr type_rr[N_TYPES][N_TYPES] = {
   { 0,        byte2half,byte2full,byte2real,byte2doub,byte2comp },
   { half2byte,0,        half2full,half2real,half2doub,half2comp },
   { full2byte,full2half,0,        full2real,full2doub,full2comp },
   { real2byte,real2half,real2full,0,        real2doub,real2comp },
   { doub2byte,doub2half,doub2full,doub2real,0,        doub2comp },
   { comp2byte,comp2half,comp2full,comp2real,comp2doub,0         }
};

   /*  low2low,    low2high  */
   /*  high2low,   high2high */

static fnptr byte_rr[N_IHOST][N_IHOST] = {
   { 0,          0           },
   { 0,          0           }
};

static fnptr half_rr[N_IHOST][N_IHOST] = {
   { 0,          trans_swap2 },
   { trans_swap2,0           }
};

static fnptr full_rr[N_IHOST][N_IHOST] = {
   { 0,          trans_swap4 },
   { trans_swap4,0           }
};
...
static fnptr real_rr[N_RHOST][N_RHOST] = {
   { 0,          r_vax2ieee,  r_vax2rieee  },
   { r_ieee2vax, 0,           trans_swap4  },
   { r_rieee2vax,trans_swap4, 0            }
};

static fnptr doub_rr[N_RHOST][N_RHOST] = {
   { 0,          d_vax2ieee,  d_vax2rieee  },
   { d_ieee2vax, 0,           trans_swap8  },
   { d_rieee2vax,trans_swap8, 0            }
};

static fnptr comp_rr[N_RHOST][N_RHOST] = {
   { 0,          c_vax2ieee,  c_vax2rieee  },
   { c_ieee2vax, 0,           c_ieee2rieee },
   { c_rieee2vax,c_rieee2ieee,0            }
};

static int bytes_per_pixel2(int type, int ihost, int rhost);
static int search_table(char *string, struct table table[], int n);

/************************************************************************/

int determine_translation(unit)
int unit;
{
...
int v2_translate_input(stype, dtype, sihost, srhost, trans)
...
struct trans *trans;	/* Out: translation structure */
{
   ...
   make_upper_case_max(temp, sihost, 29);
   ...
   make_upper_case_max(temp, srhost, 29);
   if (EQUAL(temp, "NATIVE") || EQUAL(temp, "LOCAL"))
   ...
   trans->spixsize = bytes_per_pixel2(type_src, ihost_src, rhost_src);
   trans->dpixsize = bytes_per_pixel2(type_dest, ihost_dest, rhost_dest);
   trans->notransamt = 0;
   ...
   if (trans->transfn1 == NULL)		/* no 1st trans, so use 2nd only */
      ...
      trans->midpixsize = bytes_per_pixel2(type_src, ihost_dest, rhost_dest);
   }
...
int v2_translate_output(stype, dtype, dihost, drhost, trans)
...
struct trans *trans;	/* Out: translation structure */
{
   ...
   ihost_src = search_table(NATIVE_INTFMT, ihost_table,
			 sizeof(ihost_table)/sizeof(struct table));
   make_upper_case_max(temp, dihost, 29);
   if (EQUAL(temp, "NATIVE") || EQUAL(temp, "LOCAL"))
      strcpy(temp, NATIVE_INTFMT);
   ...
   rhost_src = search_table(NATIVE_REALFMT, rhost_table,
			 sizeof(rhost_table)/sizeof(struct table));
   make_upper_case_max(temp, drhost, 29);
   if (EQUAL(temp, "NATIVE") || EQUAL(temp, "LOCAL"))
      strcpy(temp, NATIVE_REALFMT);
   ...
   if (trans->transfn1 == NULL)		/* no 1st trans, so use 2nd only */
      ...
      trans->transfn = v2_dual_translation;
      trans->midpixsize = bytes_per_pixel2(type_dest, ihost_src, rhost_src);
   }
...
int bytes_per_pixel(ctype, cihost, crhost, status)
char *ctype;
char *cihost;
char *crhost;
int *status;
{
   ...
   *status = SUCCESS;
   ...
   return bytes_per_pixel2(type, ihost, rhost);
}
