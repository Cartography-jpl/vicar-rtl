...
#define ERROR(x) {error_handler(unit,x); return x;}
...
void *parb;		/* Pointer to start of the par block	*/
{
   ...
   char *buff = 0;		/* ptr to line buf (oldtype)	*/
   char *buf = 0;	       /* ptr to data (inside buff for	old) */
...
/* Check for the existence of a parameter data set */
   ...
   zlget(unit,"system","type",stype,(void *) 0);
...
/* Fill in VARIABLE structure at current_parm */
	 ...
	 if (data_ptr == 0) ERROR(INSUFFICIENT_MEMORY); /* to store data   */
         zvtrans(&real_conv, buf, data_ptr, 
	 ...
	 if (oldtype) {		/* old style needs buffer copied */
	    ...
	    if (header.p_type == PARM_V_INTEGER)
	       zvtrans(&int_conv, buf, data_ptr, header.p_count);
	    else if (header.p_type == PARM_V_REAL)
	       zvtrans(&doub_conv, buf, data_ptr, 
		       header.p_count);
	    else				/* PARM_V_STRING */
...
static int setup_conversions(int unit)
{
   ...
   status = zvtrans_inu(&short_conv, "HALF", "HALF", unit);
   if (status == SUCCESS)
      status = zvtrans_inu(&int_conv, "FULL", "FULL", unit);
   if (status == SUCCESS)
      status = zvtrans_inu(&real_conv, "REAL", "DOUB", unit);
   if (status == SUCCESS)
      status = zvtrans_inu(&doub_conv, "DOUB", "DOUB", unit);
...
static void convert_parm_header(struct PARM_LINE *header)
{
   ...
   zvtrans(&int_conv, &header->version, &itemp, 1);
   header->version = itemp;
   zvtrans(&int_conv, &header->val_len, &itemp, 1);
   header->val_len = itemp;

   zvtrans(&short_conv, &header->p_count, &stemp, 1);
   header->p_count = stemp;
   zvtrans(&short_conv, &header->p_size, &stemp, 1);
   header->p_size = stemp;
}
...
struct PARM_LINE *new_header, char *parm_name)
{
   ...
   zvtrans(&int_conv, &old_header->val_len, 
	   &new_header->val_len, 1);
   zvtrans(&short_conv, &old_header->p_count, 
