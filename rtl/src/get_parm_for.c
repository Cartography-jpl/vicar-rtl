...
void get_parm_for(parblock,name,count,def,maxcnt,value,argptr,nargs,argno,strno,
			params, which, double_flag)
      ...
      case V_STRING:
         ...
         if (*count == 1) {		/* single value, do it the easy way */
            ...
            if (length == 0) { /* Probably VMS pass by ref so pack for XVSPTR */
               pack_xvsptr(v2_sfor2ptr(value), v, *count);	/* grr */
            }
            else {			/* Normal Fortran string array */
               c_value = (char *)malloc(*count * (length+1));
               if (c_value == NULL) {
                  *count = 0;
                  error_handler(NO_UNIT, INSUFFICIENT_MEMORY);
                  return;
               }
...
void get_parm_for(void* UNUSED(parblock),
