...
void F77_FUNC(xvfilename, XVFILENAME)
(char *in_name, char *out_name, int *status,
							ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
   char *c_in_name;
...
int zvfilename(in_name, out_name, out_len)
...
int out_len;
{
   ...
   if (first_call)
      general_initialize();		/* get default filename if needed */
