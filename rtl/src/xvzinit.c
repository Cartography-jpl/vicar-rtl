...
#if RTL_USE_TAE
int F77_FUNC(xzinit, XZINIT)
(struct PARBLK*, int*, int*, int*, int*);
int F77_FUNC(xzsinit, XZSINIT)
(struct PARBLK*, int*, int*, int*, int*);
#endif
...
void F77_FUNC(xvzinit, XVZINIT)
(int *lun, int *flag, int *debug)
{
   int status;
#if RTL_USE_TAE
   ...
   *flag = FALSE;
   ...
   mode = P_CONT;
   F77_FUNC(xzinit, XZINIT)((struct PARBLK*) parb, &parblksize, lun, 
			    &mode, &status);
#endif
#if RTL_USE_SHELL_VIC
   if (status != SUCCESS) {
     in_vicar = 0;
     F77_FUNC(xzsinit,XZINIT)((struct PARBLK*)parb, &parblksize, lun, 
			      &mode, &status);
   }
#endif
