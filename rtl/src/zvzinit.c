...
int zvzinit(argc,argv)
int argc;
char *argv[];
{
#if RTL_USE_TAE
  struct LARGE_PARBLK *parb=NULL;		/* should be struct PARBLK *parb;	*/
   void *file=NULL;
   int lines, cols, type;
   int n, swtch;
   ...
   file = z_init((struct PARBLK*) parb, P_CONT);
#endif
#if RTL_USE_SHELL_VIC
   if (file==NULL) {
     in_vicar=0;		/* not in the TAE - using shell */
     if(zzinit((struct PARBLK*) parb, argc, argv))
