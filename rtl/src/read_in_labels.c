...
int read_in_labels(unit,p,size)
...
/* to allow for expansions of the system label.				     */
   ...
   *p = malloc(*size);
   ...
   for (i=0; i<n_label_recs; i++,q+=recsize) {	/* read in the labels */
      status = read_rec(bufstate, q, i, 0, 0, &no_trans);
      if (status != SUCCESS)
	 return status;
   }
   ...
   if(COMPRESSED) compress_read_in_eol(unit, q, eol_size);
   else
   {
      ...
      for (i=0; i<eol_label_recs; i++,q+=recsize) {	/* read in EOL labels */
         status = read_rec(bufstate, q, rec++, 0, 0, &no_trans);
         if (status != SUCCESS)
	    return status;
      }
   }

   if (eol_label_recs > 0 || (COMPRESSED && CURRENT_I_VALUE(EOL))) /* Cut out 2nd SYSTEM_KEY */
      cut_label_string(lbl2, lbl2+LABEL_SIZE_ITEM, "");
