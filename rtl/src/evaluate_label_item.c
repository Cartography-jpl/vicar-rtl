...
int evaluate_label_item(value,vallen,format,length,nelements,level,strlength)
...
int *strlength;			/* out: length of item treated as a string */
{
   ...
   *level = 0;
   ...
   while (get_label_item(&value, &vallen, &element, level)) {
      (*nelements)++;
      maxlen = MAX(maxlen, (int) strlen(dequoted(element,strlen(element))));
      if (*nelements == 1) {		/* if first element, get the format */
         if (strlen(format) == 0)
            strcpy(format, determine_format(element));
      }
   }
