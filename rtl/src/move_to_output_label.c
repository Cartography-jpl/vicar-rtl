...
int move_to_output_label(unit,buf,size,recbuf)
int unit;
char *buf;
int size;
char *recbuf;
{
   ...
   printf("**in move to output label: %s\n", tstBuf);*/
   ...
   while (size_left > 0) {		/* Keep going until all data gone */
      ...
      if (rem_rec == 0) {		/* If recbuf is full write it out  */
         ...
         status = write_rec((struct bufstate *)CURRENT_IP_VALUE(BUFSTATE), 
			    recbuf, 
			    label_record++, 0, 0, &no_trans);
         if (status != SUCCESS)
            return status;
