...
int v2_del_complex_label_item(value, vallen, start, stop, start_element,
			nelements_to_delete, nelements_present,
			length, level, format, key, newstop)
...
char **newstop;		/* out: pointer to new stop location of label */
{
   ...
   *newstop = stop;
   ...
   status = cut_label_string(start, stop, label_item);
   if (status != SUCCESS) {
      free(valarray);
      free(label_item);
      return status;
