...
void F77_FUNC(xvcmdout, XVCMDOUT)
(char *fcmd, int *status, ZFORSTR_PARAM)
...
/* be okay.								*/
   ...
   status = q_cmd(cmd);
   if (status != SUCCESS) {
      error_handler(NO_UNIT, XVCOMMAND_ERROR);
   ...
   if (sfi) {
      status = IVAL(*sfi, 0);
      if (status != SUCCESS) {
         error_handler(NO_UNIT, XVCOMMAND_FAIL);
         return status;
      }
      return SUCCESS;
   }

   error_handler(NO_UNIT, XVCOMMAND_ERROR);	/* $SFI not found */
   return XVCOMMAND_ERROR;
