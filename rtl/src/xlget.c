...
int zlget(int unit, char *c_type, char *c_key, char *value, ...)	/*$$*/
{
   va_list params;
   int nargs;
   int status;
...
/* Get the constant parameters */	/*$$*/
   ...
   make_upper_case_max(type, c_type, MAX_SHORT_STRING_SIZE);
   make_upper_case_max(key, c_key, MAX_LABEL_KEY_SIZE);
...
/* Call the common routine */
   ...
   retvalue = NULL;
   status = c_xlget(unit,type,key,(char **) &retvalue,&len,&stel,&nel,&format); /*$$*/
   va_end(params);
   if (status != SUCCESS) {
      if (retvalue != NULL)
         free(retvalue);
   ...
   if (EQUAL(format, "STRING")) {
      ...
      if (outlen == 0) {
         if (nel == 1)		/* only one element, use string length */
            ...
            free(retvalue);
            error_handler(unit, IMPROPER_LENGTH);
            return IMPROPER_LENGTH;
         }
      }
      for (i=0; i<nel; i++) {
         strncpy(value+(i*outlen), retvalue+((i+stel)*len), outlen);
         *(value+(i*outlen)+outlen-1) = '\0';  /*Make sure there's a null term*/
      }
   }
   else {
...
void F77_FUNC(xlget, XLGET)
(int *unitp, char *for_type, char *for_key, char *value,
					int *status, ...) 	/*$$*/
{
...
/* FORMAT is required, except on the VAX for compatibility (grr)	  */
   ...
   *status = format_preset_for(unit, &nopts, &nargs, argno, &params);
   if (*status != SUCCESS) {
      error_handler(unit, *status);
      return;
   }
...
/* Call the common routine */
   ...
   retvalue = NULL;
   *status = c_xlget(unit,type,key, (char **) &retvalue,&maxlen,&stel,&nel,&format); /*$$*/
   va_end(params);
   if (*status != SUCCESS) {
      if (retvalue != NULL)
         free(retvalue);
