...
int unit;
{
   ...
   if (CURRENT_I_VALUE(EOL) && COMPRESSED) return compress_get_eol_size(unit);
   ...
   if (CURRENT_I_VALUE(EOL)==TRUE) {
	 ...
	 return NO_MEMORY_FOR_LABEL_PROCESS;
      ...
      for (i=0; i<j; i++) {
	 status = read_rec(bufstate, loc_buf+i*recsize, rec+i, 0, 0, &no_trans);
	 if (status != SUCCESS)
	    return status;
      }
