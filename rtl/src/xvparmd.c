...
void F77_FUNC(xvparmd, XVPARMD)
(char *name, char *value, int *count, int *def,
					int *maxcnt, ...)
...
/* int *maxcnt;		In: max # of values to return.  0 means no limit. */
{
   ...
   get_parm_for((struct PARBLK*) &parb, c_name, count, def, max_count, value, 
...
int zvparmd(name, value, count, def, maxcnt, length)
char *name;		/* In: Name of parameter to get */
...
int length;		/* In: len of each string in value if string array */
{
   ...
   return get_parm_c((struct PARBLK*) &parb, name, value, count, def, 
		     maxcnt, length, TRUE);
...
void F77_FUNC(xviparmd, XVIPARMD)
(char *name, char *value, int *count, int *def,
				int *maxcnt, ...)
/* char *name;		In: Name of parameter to get */
/* char *value;		Out: returned value (may be int,real,or string) */
...
/* int *maxcnt;		In: max # of values to return.  0 means no limit. */
{
   ...
   get_parm_for((struct PARBLK*) &iparb, c_name, count, def, max_count, 
...
int zviparmd(name, value, count, def, maxcnt, length)
char *name;		/* In: Name of parameter to get */
void *value;		/* Out: returned value (may be int,real,or string) */
int *count;		/* Out: # of values returned.  0 if param not found. */
int *def;		/* Out: default flag, 0 if user-given, 1 if default */
...
int length;		/* In: len of each string in value if string array */
{
   ...
   return get_parm_c((struct PARBLK*) &iparb, name, value, count, 
