...
int basic_get_eol_size(int unit)
{
   ...
   if (find_entry(loc_buf, SYSTEM_KEY, &value, &vallen, &ip) != 0)
...
int basic_close(int unit)
{
   ...
   if(EQUAL(CURRENT_S_VALUE(COMPRESS), "BASIC2") && !EQUAL(CURRENT_S_VALUE(OP), "READ"))
   {
	 ...
	 set32BitInt(nBytes, lengths+i*sizeof(int));
      }
   ...
   {
      status = set_basic_label_params(unit);
      if(status != SUCCESS) return status;
                 ...
                 int ns, int wid, int *totBytes)
{
   ...
   *totBytes = pcode1 - encodedBuf;
   ...
   set32BitInt(*totBytes, encodedBuf);
...
/* labels are added.                                 */
/*****************************************************/
int set_basic_label_params(unit)
   int unit;
{
   int status;
   char* label;
   ...
   if (CURRENT_S_VALUE(LABELS) == NULL)
   {
      ...
      status = read_in_labels(unit, &label, &CURRENT_I_VALUE(LBLALLOC));
      if (status != SUCCESS)
         return status;
...
int basic_read_rec(int unit, struct bufstate *state)
{
   ...
   if(LAZY_INDEX(CURRENT_I_VALUE(IMG_REC)-1) == -1) status = lazy_search(unit);
