...
int v2_add_complex_label_item(unit, newvalue, value, vallen, start, stop,
		element, nelements_to_add, newlength, level, format, key)
...

{
   ...
   status = evaluate_label_item(value, vallen, format, &oldlength,
   ...
   status = cut_label_string(start, stop, "");	/* remove old item */
   if (status != SUCCESS) {
      free(valarray);
      free(label_item);
      return status;
