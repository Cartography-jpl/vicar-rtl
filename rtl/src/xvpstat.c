...
void F77_FUNC(xvpstat, XVPSTAT)
(char *key, int *count, int *def, int *maxlen,
...
/* char *type;		out: string indicating type of parameter */
{
   ...
   get_pstat((struct PARBLK*) &parb, c_key, count, def, maxlen, c_type);
...
int zvpstat(key, count, def, maxlen, type)
char *key;		/* in: parameter name. */
int *count;		/* out: actual count of param. If 0, param not found */
int *def;		/* out: default flag.  =1 if defaulted */
int *maxlen;		/* out: max length of any single item (not incl term) */
char *type;		/* out: string indicating type of parameter */
{
   ...
   return get_pstat((struct PARBLK*) &parb, key, count, def, maxlen, type);
...
void F77_FUNC(xvipstat, XVIPSTAT)
(char *key, int *count, int *def, int *maxlen,
						char *type, ZFORSTR_PARAM)
/* char *key;		in: parameter name. */
/* int *count;		out: actual count of param. If 0, param not found */
...
/* char *type;		out: string indicating type of parameter */
{
   ...
   get_pstat((struct PARBLK*) &iparb, c_key, count, def, maxlen, c_type);
...
int zvipstat(key, count, def, maxlen, type)
char *key;		/* in: parameter name. */
int *count;		/* out: actual count of param. If 0, param not found */
int *def;		/* out: default flag.  =1 if defaulted */
int *maxlen;		/* out: max length of any single item (not incl term) */
char *type;		/* out: string indicating type of parameter */
{
   ...
   return get_pstat((struct PARBLK*) &iparb, key, count, def, maxlen, type);
