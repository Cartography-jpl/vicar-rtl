...
int c_xladd(unit, type, key, newvalue, len)
int unit;
char *type;
char *key;
void *newvalue;
int len;			/* len of C array for strings */
{
   ...
   if (CURRENT_S_VALUE(LABELS) == NULL) {
      status = read_in_labels(unit, &p, &CURRENT_I_VALUE(LBLALLOC));
      if (status != SUCCESS)
         return status;
   ...
   if (EQUAL(type, "HISTORY")) {		/* History label */
	 ...
	 /* New property label goes just before history */
         ...
         find_first_hist_item(CURRENT_S_VALUE(LABELS), &p);
         ...
         status=v2_find_property(unit, LABEL_S_VALUE(PROPERTY),
			LABEL_I_VALUE(INSTANCE), &start, &stop, 0);
   ...
   if (status==SUCCESS) {			/* it already exists */
      ...
      if (EQUAL(LABEL_S_VALUE(MODE), "REPLACE")) {
         if (LABEL_I_VALUE(NELEMENTS) == 0)	/* defaulted */
            nelements = -1;
         status = delete_label_item(unit, start, stop, key, value, vallen,
