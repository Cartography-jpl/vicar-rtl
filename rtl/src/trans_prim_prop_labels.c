...
int v2_trans_prim_prop_labels(unit,lblsize,recbuf) 
...
/* there, there must be some internal error.			*/
   ...
   if (find_entry(p, PROPERTY_KEY, &value, &vallen, &loc) == NULL)
      return SUCCESS;			/* No property labels */

   find_entry(p, TASK_KEY, &value, &vallen, &endloc); /* Find end of prop lbl */

   /* Move the property labels to the output label */

   status = move_to_output_label(unit, loc, endloc - loc, recbuf);
   if (status != SUCCESS)
      return status;
