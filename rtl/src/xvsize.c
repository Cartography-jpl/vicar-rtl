...
void zvsize(sl, ss, nl, ns, nli, nsi)
int *sl;		/* Out: starting line (default=1) */
int *ss;		/* Out: starting sample (default=1) */
int *nl;		/* Out: number of output lines (default=nli) */
int *ns;		/* Out: number of output samples (default=nsi) */
...
int *nsi;		/* Out: number of input lines (default=0) */
{
   ...
   *nsi = 0;
   ...
   status = get_one_int_parm("SIZE", 2, &tempnl);
   if (status != SUCCESS)
      tempnl = 0;
   status = get_one_int_parm("SIZE", 3, &tempns);
   if (status != SUCCESS)
      tempns = 0;
   if (tempnl != 0 && tempns != 0) {
      get_one_int_parm("SIZE", 0, sl);
      get_one_int_parm("SIZE", 1, ss);
      *nl = tempnl;
      *ns = tempns;
   }
   else {		/* If NL or NS == 0 in SIZE, assume it is defaulted */
      status = get_one_int_parm("SL", 0, &temp);
      if (temp > 0 && status == SUCCESS)
         *sl = temp;
      status = get_one_int_parm("SS", 0, &temp);
      if (temp > 0 && status == SUCCESS)
         *ss = temp;
      status = get_one_int_parm("NL", 0, &temp);
      if (temp > 0 && status == SUCCESS)
         *nl = temp;
      status = get_one_int_parm("NS", 0, &temp);
      if (temp > 0 && status == SUCCESS)
         *ns = temp;
   }
