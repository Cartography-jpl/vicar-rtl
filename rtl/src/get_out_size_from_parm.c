...
void v2_get_out_size_from_parm(unit) 
int unit;
{
   ...
   status = get_one_int_parm("NL", 0, &x);		/* Get NL param */
   if (status == SUCCESS && x != 0)
      CURRENT_I_VALUE(NL) = x;

   status = get_one_int_parm("NS", 0, &x);		/* Get NS param */
   if (status == SUCCESS && x != 0)
      CURRENT_I_VALUE(NS) = x;

   status = get_one_int_parm("NB", 0, &x);		/* Get NB param */
   if (status == SUCCESS && x != 0)
      CURRENT_I_VALUE(NB) = x;

   for (i=0; i<4; i++) {		/* Get the SIZE parameter */
      status = get_one_int_parm("SIZE", i, &size[i]);
      if (status != SUCCESS)
