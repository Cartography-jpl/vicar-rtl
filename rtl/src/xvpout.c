...
#define ABORT(x) {error_handler(parm_file_unit,x); return x;}
...
void F77_FUNC(xvpout, XVPOUT)
(int *status, char *parm_name, char *parm_value,
...
/* int *count;		In: TCL count of parm			*/
{
   ...
   make_upper_case_max(c_format, pformat, MAX_SHORT_STRING_SIZE);
   ...
   if (strncmp(c_format, "STRING", strlen("STRING")) == 0) {	/* string */
      ...
      if (*status != SUCCESS) {
         error_handler(parm_file_unit, *status);
         return;
      }
...
int zvpout(parm_name, parm_value, parm_format, count, length)
char *parm_name;	/* In: name of parameter to be stored		*/
			...
			/*     multivalued string, or 0.		*/
{
   ...
   make_upper_case_max(format, parm_format, MAX_SHORT_STRING_SIZE);
   make_upper_case_max(name, parm_name, NAMESIZE);
   ...
   status = parm_write(name, header.name_len);
   if (status != SUCCESS)
      return status;
   ...
   if (header.p_type != PARM_V_STRING) {
      status = parm_write(parm_value, header.val_len);
      if (status != SUCCESS)
         return status;
   }
      ...
      p = parm_value;
      for (i=0; i<header.p_count; i++) {
         status = parm_write(p, strlen(p)+1);
         if (status != SUCCESS)
            return status;
         p += header.p_size;
      }
