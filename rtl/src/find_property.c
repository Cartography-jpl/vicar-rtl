...
int v2_find_property(unit, prop, instance, start, stop, inst_out)
				...
				/*      instance not found.  May be NULL. */
{
   ...
   *start = NULL;
   ...
   while (!found) {
      pstop = find_entry(p, PROPERTY_KEY, &value, &vallen, &pstart);
      if (pstop == NULL)
         break;			/* only way out of loop if strlen(task)==0 */
      if (strcmp(dequoted(value,vallen), prop) == 0) {
   ...
   if (pstop == NULL) {		/* No more properties, look for start of hist */
      pstop = find_entry(p, TASK_KEY, &value, &vallen, &pstart);
   }
   *stop = pstart;
