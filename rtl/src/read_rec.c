...
int read_rec(state, buffer, rec, startbyte, nbytes, trans)
...
struct trans *trans;		/* Format translation control struct */
{
   ...
   status = io_complete_check(&state->devstate);
   if (status != SUCCESS)
      return status;
   ...
   if (state->flags & BUF_VARREC) {	/* read a variable length record */
      status = io_complete_check(&state->devstate);  /* wait for prev writes */
      if (status != SUCCESS)
	 return status;
#if VMS_OS
...
/* determines the # of complete records that fit in a buffer.		      */
   ...
   status = io_complete_check(&state->devstate); /* wait for flush to finish */
   if (status != SUCCESS)
      return status;
