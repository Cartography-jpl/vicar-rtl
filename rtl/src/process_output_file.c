...
int v2_process_output_file(unit)
int unit;
{
   int status;
...
/* Copy items from U_NL, etc. to NL, etc. */
   ...
   if (strlen(CURRENT_S_VALUE(U_ORG)) != 0) {
      status = add_str_current_table(CURRENT_S_VALUE(U_ORG), ORG,
...
/* Interleaved by Line), and BIP (Band Interleaved by Pixel).		*/
   ...
   if (EQUAL(CURRENT_S_VALUE(ORG),"BSQ")) {
      ...
      status = add_str_current_table("BSQ", ORG, current_table[unit],
						    default_table);
      if (status != SUCCESS)
         return UNABLE_TO_STORE_OPTIONAL;
...
/* input, VAX-VMS format).						    */
   ...
   if (EQUAL(CURRENT_S_VALUE(BIN_CVT), "ON")) {
      status = add_str_current_table(NATIVE_HOST_LABEL, BHOST,
				current_table[unit], default_table);
      if (status != SUCCESS)
         return UNABLE_TO_STORE_OPTIONAL;
      status = add_str_current_table(NATIVE_INTFMT, BINTFMT,
				current_table[unit], default_table);
      if (status != SUCCESS)
         return UNABLE_TO_STORE_OPTIONAL;
      status = add_str_current_table(NATIVE_REALFMT, BREALFMT,
				current_table[unit], default_table);
      if (status != SUCCESS)
         return UNABLE_TO_STORE_OPTIONAL;
   }
...
/* output format, so set FORMAT.					*/
   ...
   if (strlen(CURRENT_S_VALUE(O_FORMAT)) != 0)
      status = add_str_current_table(CURRENT_S_VALUE(O_FORMAT),FORMAT,
					current_table[unit], default_table);
...
/* U_FORMAT if no primary exists.					*/
   ...
   if (strlen(CURRENT_S_VALUE(U_FORMAT)) == 0) {
      status = add_str_current_table(CURRENT_S_VALUE(FORMAT),U_FORMAT,
					current_table[unit], default_table);
   }

   CURRENT_I_VALUE(PIX_SIZE) = bytes_per_pixel(CURRENT_S_VALUE(FORMAT),
