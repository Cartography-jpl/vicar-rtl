...
int process_optionals_c(int UNUSED(unit), struct UNIT_TABLE* opts_table, 
      ...
      if (s && s[0] != '\0') {
         ...
         if ((opts_table[i].mode == INPUT) ||
            ...
            if (opts_table[i].type == STRING) {
               status = add_str_current_table(value.s, i, currval_table,
						    def_table);
	       if (status != SUCCESS)
	          return UNABLE_TO_STORE_OPTIONAL;
            }
            else if (opts_table[i].type == MESSAGE) {
               status = add_msg_current_table(value.s, i, currval_table,
						     def_table);
	       if (status != SUCCESS)
	          return UNABLE_TO_STORE_OPTIONAL;
            }
...
int process_optionals_for(int UNUSED(unit), struct UNIT_TABLE* opts_table, 
			  int n_entries, VALUE_TABLE* currval_table, 
			  VALUE_TABLE* def_table,
			  int nopts, va_list* params, 
