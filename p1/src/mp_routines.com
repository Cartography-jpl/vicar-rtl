...
$! VPACK Version 1.9, Thursday, January 17, 2002, 12:46:48
$!
...
for ( i=0; i<num; i++ )
	...
	if ( strcmp(keywords[i],mpA_AXIS_RADIUS) == 0 && a_axis == FALSE )
		{
		status = mpGetValues( mp, mpA_AXIS_RADIUS, &radii[0], "" );
		if (status != mpSUCCESS) return status;
		a_axis = TRUE;
		number_of_radii++;
		}
	if ( strcmp(keywords[i],mpB_AXIS_RADIUS) == 0 && b_axis == FALSE )
		{
		status = mpGetValues( mp, mpB_AXIS_RADIUS, &radii[1], "" );
		if (status != mpSUCCESS) return status;
		b_axis = TRUE;
		number_of_radii++;
		}
	if ( strcmp(keywords[i],mpC_AXIS_RADIUS) == 0 && c_axis == FALSE )
		{
		status = mpGetValues( mp, mpC_AXIS_RADIUS, &radii[2], "" );
		if (status != mpSUCCESS) return status;
		c_axis = TRUE;
		number_of_radii++;
		}
...
case 1:		/* One radius value has been set	  	*/
		...
		if ( c_axis == TRUE )
			{
			mpSetValues( mp, mpA_AXIS_RADIUS, radii[2], "" );
			mpSetValues( mp, mpB_AXIS_RADIUS, radii[2], "" );
			}
		if ( b_axis == TRUE )
			{
			mpSetValues( mp, mpA_AXIS_RADIUS, radii[1], "" );
			mpSetValues( mp, mpC_AXIS_RADIUS, radii[1], "" );
			}
		if ( a_axis == TRUE )
			{
			mpSetValues( mp, mpB_AXIS_RADIUS, radii[0], "" );
			mpSetValues( mp, mpC_AXIS_RADIUS, radii[0], "" );
			}
		break;
...
pmp->precalc.cc_matrix = (double *)calloc(i,sizeof(double));
CHECKif( pmp->precalc.cc_matrix == NULL );

pmp->precalc.cp_matrix = (double *)calloc(i,sizeof(double));
CHECKif( pmp->precalc.cp_matrix == NULL );

pmp->precalc.ac_matrix = (double *)calloc(j,sizeof(double));
CHECKif( pmp->precalc.ac_matrix == NULL );

pmp->precalc.ap_matrix = (double *)calloc(NLIMIT,sizeof(double));
CHECKif( pmp->precalc.ap_matrix == NULL );
 ...
 */
  ...
  status = mpGetValues( mp, mpMAP_PROJECTION_TYPE, maptype, "");
  if (status != mpSUCCESS) return status;

  status = mpGetValues( mp, mpCOORDINATE_SYSTEM_NAME, csys, "");
  ...
  status = mpGetValues( mp, mpPOSITIVE_LONGITUDE_DIRECTION, pos_lon, "");
  if (status != mpSUCCESS) return status;
  /* save value of this keyword as it has implications for other
     longitude items: */
  pos_lon_east = 0;
  ...
  status = mpGetValues( mp, mpC_AXIS_RADIUS, &dval, "");
  if (status != mpSUCCESS) return status;
  float_buf[24] = dval;
  status = mpGetValues( mp, mpA_AXIS_RADIUS, &dval, "");
  if (status != mpSUCCESS) return status;
  float_buf[25] = dval;
  status = mpGetValues( mp, mpB_AXIS_RADIUS, &dval, "");
  if (status != mpSUCCESS) return status;
  float_buf[36] = dval;
  ...
  status = mpGetValues( mp, mpBODY_LONG_AXIS, &dval, "");
  if (status != mpSUCCESS) return status;
  float_buf[35] = dval;
  ...
  if (EQUAL( maptype, mpPOINT_PERSPECTIVE)) {	/* PERSPECTIVE is special */
    ...
    status = mpGetValues( mp, mpFOCAL_LENGTH, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[26] = dval;
    mom[5] = dval;

    status = mpGetValues( mp, mpOPT_AXIS_INTERCEPT_LINE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[27] = dval;
    mom[0] = dval;

    status = mpGetValues( mp, mpOPT_AXIS_INTERCEPT_SAMPLE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[28] = dval;
    mom[1] = dval;

    status = mpGetValues( mp, mpFOCAL_PLANE_SCALE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[29] = dval;
    mom[4] = dval;

    status = mpGetValues( mp, mpSUB_SPACECRAFT_LATITUDE, &dval, "");
    if (status != mpSUCCESS) return status;
    if (lattype==0) dval = det2cen(dval);
    float_buf[30] = dval;
    mom[7] = dval;

    status = mpGetValues( mp, mpSUB_SPACECRAFT_LONGITUDE, &dval, "");
    if (status != mpSUCCESS) return status;
    ...
    status = mpGetValues( mp, mpPLANET_CENTER_LINE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[32] = dval;
    mom[2] = dval;

    status = mpGetValues( mp, mpPLANET_CENTER_SAMPLE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[33] = dval;
    mom[3] = dval;

    status = mpGetValues( mp, mpNORTH_ANGLE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[34] = dval;
    mom[8] = dval;

    status = mpGetValues( mp, mpTARGET_CENTER_DISTANCE, &dval, "");
    if (status != mpSUCCESS) {
      status = mpGetValues( mp, mpSPACECRAFT_DISTANCE, &dval, "");
      if (status != mpSUCCESS) return status;
    }
    float_buf[37] = dval;
    mom[9] = dval;
    ...
    momati_c( mom[0], mom[1], mom[2], mom[3], mom[4], mom[5], mom[6],
     mom[7], mom[8], mom[9], &float_buf[0], &float_buf[18] );
      ...
      /* these items are common to all standard projections: */
         ...
         * are to (0,0): */
    status = mpGetValues( mp, mpSAMPLE_PROJECTION_OFFSET, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[0] = dval + 1.0;
    status = mpGetValues( mp, mpLINE_PROJECTION_OFFSET, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[1] = dval + 1.0;

    status = mpGetValues( mp, mpCENTER_LONGITUDE, &dval, "");
    if (status != mpSUCCESS) return status;
    if (pos_lon_east) dval = 360.0 - dval;
    if (dval<0.0) dval += 360.0;
    float_buf[5] = dval;

    status = mpGetValues( mp, mpMAP_SCALE, &dval, "");
    if (status != mpSUCCESS) return status;
    float_buf[6] = dval;
    ...
    if ( EQUAL( maptype, mpOBLIQUE_ORTHOGRAPHIC) ||
	 ...
	 EQUAL( maptype, mpOBLIQUE_SINUSOIDAL) ) {
      status = mpGetValues( mp, mpCARTESIAN_AZIMUTH, &azic, "");
      if (status != mpSUCCESS) azic = 0.0;
      status = mpGetValues( mp, mpSPHERICAL_AZIMUTH, &azis, "");
    ...
    if (EQUAL( maptype, mpLAMBERT) ||
      ...
      status = mpGetValues( mp, mpFIRST_STANDARD_PARALLEL, &dval, "");
      if (status != mpSUCCESS) return status;
      if (lattype==0) dval = det2cen(dval);
      float_buf[3] = dval;
      status = mpGetValues( mp, mpSECOND_STANDARD_PARALLEL, &dval, "");
      if (status != mpSUCCESS) return status;
      if (lattype==0) dval = det2cen(dval);
      float_buf[4] = dval;
    }
	...
	strcmp( maptype, mpHOMALOGRAPHIC) ) {
      status = mpGetValues( mp, mpCENTER_LATITUDE, &dval, "");
      if (status != mpSUCCESS) return status;
...
struct PRECALCULATIONS {
	...
	double	*cc_matrix;			/* Matrices for TRIAXTRAN */
	double	*cp_matrix;
	double	*ac_matrix;
	double	*ap_matrix;
};
...
double a1,b1,c1;
double vmax1,vmax2,vmax3,vmax4,vmin1,norm1,norm2,norm3;
...
double c1,c2,c3,c4,ca,ce,co,sa,se,so;
...
double area,tarea,sphiaut,sum1,autfunc,radsph,tcirc,fsimlon;
int    latlmt,lonlmt;
...
double authcoef1[maxpts],authcoef2[maxpts];

int    nequations,npoints,idum;
double degtorad;
double slon,clon,slat,clat,flon,flat;
double t,t1,t2,t3;
