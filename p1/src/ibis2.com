...
$! VPACK Version 1.9, Monday, May 11, 1998, 13:14:18
$!
...
static int _copy_raw_column(ibis,incolumn,outcolumn)
XIBIS *ibis;
int incolumn;
int outcolumn;
{
...
int IBISColumnGet( ibis_id, name, value, column )
int ibis_id;
char *name;
void *value;
int column;
...
int IBISColumnSet( ibis_id, name, value, column )
int ibis_id;
char *name;
void *value;
int column;
...
int IBISColumnRead(ibis_id,buffer,column,srow,nrows)
int ibis_id;
char *buffer;
int column;
int srow;
...
int IBISColumnWrite(ibis_id,buffer,column,srow,nrows)
int ibis_id;
char *buffer;
int column;
int srow;
...
int IBISColumnClear(ibis_id,column,ncols)
int ibis_id;
int column;
int ncols;
{
...
int IBISColumnNew(ibis_id,column,ncols,fmt)
int ibis_id;
int column;
int ncols;
char *fmt;
...
int IBISColumnDelete(ibis_id,column,ncols)
int ibis_id;
int column;
int ncols;
{
...
int IBISColumnMove(ibis_id,sourcecol,destcol,ncols)
int ibis_id;
int sourcecol;
int destcol;
int ncols;
...
int IBISColumnFind( ibis_id, type, group, column, scol, maxcols )
int ibis_id;
...
void F77_FUNC_(ibis_column_get,IBIS_COLUMN_GET)
(int *ibis_id, char *name, void *value,
	int *column, int *status, ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
...
void FTN_NAME(ibis_column_set)(int *ibis_id, char *name, void *value,
	int *column, int *status, ZFORSTR_PARAM)
{
        ZFORSTR_BLOCK
	char c_name[MAX_VALUE_NAME+1];
...
void FTN_NAME(ibis_column_read)(int *ibis_id, char *buffer, int *column,
			int *srow, int *nrows, int *status, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
   XIBIS *ibis=IBIS_FETCH_ID(*ibis_id);
   char *tempbuf=(char *)0;
...
void FTN_NAME(ibis_column_write)(int *ibis_id, char *buffer,int *column,
			int *srow, int *nrows, int *status, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
   XIBIS *ibis=IBIS_FETCH_ID(*ibis_id);
...
void FTN_NAME(ibis_column_new)(int *ibis_id, int *column, int *ncols,
...
void FTN_NAME(ibis_column_clear)(ibis_id,column,ncols,status)
int *ibis_id;
int *column;
int *ncols;
int *status;
...
void FTN_NAME(ibis_column_delete)(ibis_id,column,ncols,status)
int *ibis_id;
int *column;
int *ncols;
int *status;
...
void FTN_NAME(ibis_column_move)(ibis_id,sourcecol,destcol,ncols,status)
int *ibis_id;
int *sourcecol;
int *destcol;
int *ncols;
...
int FTN_NAME(ibis_column_find)( int *ibis_id, char *type, char *group,
	int *column, int *scol, int *maxcol, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
...
XIBIS *ibis;
{
	 ...
	 */
	status = zvopen( ibis->unit, "op", "read","lab_act","  ", 0 );
	if (status != 1) return (status );
	...
	status = zlget( ibis->unit, "property",IFILE_NC, &val, "property","ibis",   0);
	...
	if (old) /* then we need to determine which IBIS-1 file this is */
	{
		...
		status = zvget( ibis->unit, "type", type,  0);
...
int ibis_id;	/*  XIBIS file unit */
{
	...
	ibis->formats = _i_new_list(_i_free_group);
...
int length; /* length of multvalued string */
{
	...
	switch (_i_keymatch(name,ValueList))
	{
		...
		case VALUE_NC:
			if (ibis->nc != (int)value) /* 64-bit okay rgd 1-99 */
			{
				int nc = (int)value; /* 64-bit okay rgd 1-99 */
				if (is_open)
				{
					CHECK_WRITE( ibis );
					if (nc > MAX_COL)
		...
		case VALUE_NR:
			if (ibis->nr != (int)value) /* 64-bit okay rgd 1-99 */
			{
				int nr = (int)value; /* 64-bit okay rgd 1-99 */
				if (is_open)
				{
					CHECK_WRITE( ibis );
					if (nr>ibis->nr)
						status=IBISRowNew(ibis_id,0,nr-ibis->nr,0);
					else
		...
		case VALUE_PIX_NS:
			...
			if (is_open) return IBIS_FILE_ALREADY_OPENED;
			CHECK_WRITE( ibis );
			ibis->ns = (int)value;	/* 64-bit okay rgd 1-99 */
...
void FTN_NAME(ibis_file_open)( int *vunit, int *xunit_id, char *mode, int *ncol,
...
void FTN_NAME(ibis_file_unit)( int *vunit, int *xunit_id, char *mode, int *ncol,
	 int *nrow, char *format, char *org, int *status, ZFORSTR_PARAM )
#if 0
int *vunit;	/* input: VICAR file unit */
int *xunit_id;	/* output - XIBIS file unit */
...
void FTN_NAME(ibis_file_unit_open)( xunit_id, status )
int *xunit_id;	/* output - XIBIS file unit */
int *status;
{
   *status = IBISFileUnitOpen( *xunit_id );
...
void FTN_NAME(ibis_file_close)( int *xunit_id, char *close_option,
...
int FTN_NAME(ibis_file_get)( int *ibis_id, char *name, void *value, int *sval,
		int *maxval, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
   char c_name[MAX_VALUE_NAME+1];
   char *countString=(char *)0;
...
void FTN_NAME(ibis_file_set)( int *ibis_id, char *name, void *value,
		int *status, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
   XIBIS *ibis=IBIS_FETCH_ID(*ibis_id);
   ...
   switch (key)
   {
	...
	case VALUE_NR:
	case VALUE_NC:
		valptr = (char *)(*(int *)value); /* pass by value */ /* 64-bit okay(?) rgd 1-99 */
		break;
...
void FTN_NAME(ibis_file_clear)( xunit_id, status)
int *xunit_id;	/* input - XIBIS file unit */
int *status;
{
...
XGROUP *group;
{
	...
	for(ent=group->columns->next;ent;ent=ent->next)
	{
		col = (XCOL *)ent->value;
		if (col->unit)
			_i_delete_value(col->unit->columns, col);
		col->unit = group;
	}
}

int IBISGroupNew( ibis_id,type, name, cols, ncols, expr )
int ibis_id;
char *type;		/* "group" "unit" or "local"  */
char *name;
int *cols;
int ncols;
char *expr;
...
int _IBISGroupNew(ibis, type, name, cols, ncols, expr)
XIBIS *ibis;
char *type;		/* "group" "unit" or "local"  */
char *name;
int *cols;
...
char *expr;
{
   ...
   _ibis_current_module="IBISGroupNew";
	...
	if (!*listptr)
	{
		*listptr = _i_new_list(_i_free_group);
...
int IBISGroupDelete( ibis_id,type, name )
int ibis_id;
char *type;		/* "group", "unit" , "local", or all */
char *name;
{
	XIBIS *ibis=IBIS_FETCH_ID(ibis_id);
	XGROUP *grp=(XGROUP *)0;
...
int IBISGroupModify( ibis_id,type, name, mod, cols, ncols )
int ibis_id;
char *type;		/* "group", "unit" , "local", or null */
char *name;
char *mod;		/* "add" or "remove" */
int *cols;		/* list of columns */
int ncols;		/* number of columns in list */
{
	XIBIS *ibis=IBIS_FETCH_ID(ibis_id);
	XGROUP *grp=(XGROUP *)0;
	List *grplist=(List *)0;
   ...
   _ibis_current_module="IBISGroupModify";
	...
	switch (_i_keymatch(mod, ModifyList))
	{
		...
		case MODIFY_REMOVE:
			...
			if (!grp->columns->next)
				_i_delete_value( grplist, grp); /* empty group */
			break;
		default:
			return IBIS_INVALID_PARM;
			break;
...
int IBISGroupFind( ibis_id,type, column, namelist, sgroup, maxgroups, length)
int ibis_id;
   ...
   _ibis_current_module="IBISGroupFind";
	...
	for (listptr=lists;*listptr;listptr++,index++)
	{
		...
		for (ent=grplist->next; ent; ent=ent->next)
		{
			...
			if (_i_find_value(grp->columns, col))
			{
				numgroups++;
...
int IBISGroupTransfer( in_id, out_id, type, incols, outcols, nc )
int in_id;
int out_id;
char *type;		/* "group", "unit" , or null */
int *incols;		/* list of input columns or null if 1,2,.. */
int *outcols;		/* list of output columns or null if 1,2,.. */
int nc;			/* number of columns to process */
{
...
int FTN_NAME(ibis_group_new)(int *ibis_id, char *type, char *name, int *cols,
		int *ncols, char *expr, ZFORSTR_PARAM)
#if 0
...
void FTN_NAME(ibis_group_delete)( int *ibis_id, char *type, char *name,
		int *status, ZFORSTR_PARAM )
#if 0
char *type;		/* "group", or "unit", etc */
#endif
...
void FTN_NAME(ibis_group_modify)( int *ibis_id, char *type, char *name, char *mod, int *cols, int *ncols, int *status, ZFORSTR_PARAM )
#if 0
char *type;		/* "group", or "unit", etc */
#endif
{
...
int FTN_NAME(ibis_group_find)( int *ibis_id, char *type, int *column,
		char *namelist, int *sgroup, int *maxgroups, ZFORSTR_PARAM )
#if 0
int *ibis_id;
char *type;		  /* input: "format" "group" "unit" "local" or null=ALL */
int *column;      /* input: column number to search for */
...
int FTN_NAME(ibis_group_transfer)( int *in_id, int *out_id, char *type, int *incols, int *outcols, int *nc, ZFORSTR_PARAM )
#if 0
char *type;		/* "group", or "unit", etc */
#endif
{
...
_HandleNotice( record, notice, sval, endval )
XREC *record;
notice_type notice;
void * sval;	/* value attached to notice */
int endval;	/* auxiliary value */
{
	int status = 1;
	...
	switch (notice)
	{
		...
		case NOTICE_FLUSHING_ROWS: 
			...
			if ((record->top_row + record->num_rows -1) < ((int)sval) ) return 1;
		...
		case NOTICE_OPEN:
			_i_insert_value( record->collaborators, (XREC *)sval );
			break;
			
		case NOTICE_CLOSING:
			_i_delete_value( record->collaborators, (XREC *)sval );
			break;
	}
...
XREC *record;
{
	...
	status = _NotifyCollaborators( record, NOTICE_FLUSHING_ROWS,
				(void *)mod_row, (mod_row + num_mods -1) );
	if (status != 1) return status;
...
XREC *record;
{
	...
	status=_NotifyCollaborators( record, NOTICE_NEED_ROWS, (void *)srow, (srow + nrows -1) );
	if (status!=1) return status;
 		...
 		for (ent=ibis->record->next; ent; ent=ent->next)
 		{
 			...
 			for (col=0, found_col=0; col<rec->num_cols && !found_col; col++)
 			{
 				if (sharecols[ rec_col[col]->id ])
 				{
 					 ...
 					 */
 					_i_insert_value( record->collaborators, rec );
 					found_col=1;
 				}
 			}
 		}
...
int IBISRecordOpen( ibis_id, record_id,  group, columns, ncols, u_format)
int ibis_id;
int *record_id;
int *columns;
int ncols;
...
char *u_format;
{
	...
	*newrec->method = *ibis->recmethod;
	...
	if (!ibis->record)
	{
		ibis->record = _i_new_list( _i_free_record );
		if (!ibis->record)
		{
			status = IBIS_MEMORY_FAILURE;
			goto failure;
...
int IBISRecordSet( record_id, name, value )
int record_id;
char *name;
int value;
{
	...
	switch(_i_keymatch(name,ValueList))
	{
			...
			if (sizeof(int) != sizeof(char *))
	        	 ...
	        	 */
	        	fmt = _i_IBISFormatCode( (char *)value );
...
int IBISRecordGet( record_id, name, value, sval, nvals )
int record_id;
char *name;
char *value;
int sval;
...
int IBISRecordRead(record_id, buffer, row)
int record_id;
char *buffer;
int row; /* record to read, or 0 */
{
	XREC *record=IBIS_FETCH_REC_ID(record_id);
	int status=1;
...
int IBISRecordWrite(record_id,buffer,row)
int record_id;
char *buffer;
int row; /* record to write, or 0 */
{
	XREC *record=IBIS_FETCH_REC_ID(record_id);
	int status=1;
...
int IBISRecordClear(record_id,row,nrows)
int record_id;
int row;
int nrows;
{
...
void FTN_NAME(ibis_record_open)( int *ibis_id, int *record_id, char *group,
	int *columns, int *ncol, char *u_format, int *status, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
...
void FTN_NAME(ibis_record_close)( record_id, status)
int *record_id;
int *status;
{
...
void FTN_NAME(ibis_record_set)( int *record_id, char *name, void *value,
		int *status, ZFORSTR_PARAM )
{
   ZFORSTR_BLOCK
   char c_name[MAX_VALUE_NAME+1];
   ...
   *status = IBISRecordSet( *record_id, c_name, valptr );
...
int FTN_NAME(ibis_record_get)( int *record_id, char *name, char *value,
		int *sval, int *nvals, ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
   char c_name[MAX_VALUE_NAME+1];
   char vname[MAX_VALUE_NAME+1];
...
void FTN_NAME(ibis_record_read)(int *record_id, char *buffer, int *row,
		int *status, ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
   XREC *record=IBIS_FETCH_REC_ID(*record_id);
...
void FTN_NAME(ibis_record_write)(int *record_id, char *buffer, int *row,
		int *status, ZFORSTR_PARAM)
{
   ZFORSTR_BLOCK
   XREC *record=IBIS_FETCH_REC_ID(*record_id);
...
void FTN_NAME(ibis_record_clear)(record_id, row, nrows, status)
int *record_id;
int *row;
int *nrows;
int *status;
...
int unit;	/* input: VICAR file unit */
{
	...
	{
	    /* Get next keyword */
           status = zlninfo(unit,key,format,&maxlen,&nelts,0);
           if ((status == END_OF_LABEL) || (strcmp(key,"TASK") == 0) ||
                (strcmp(key,"PROPERTY") == 0)) break;
	   else if (status != 1) break;
	
	   status=zldel( unit, "property",key, "property","ibis",0);
	   if (status != 1) break;
	}

	/* now zap the IBIS label */
	status=zldel( unit, "property","property", "property","ibis",0);
...
void FTN_NAME(ibis_label_remove)( unit , status)
int *unit;	/* input: VICAR file unit */
int *status;
{
...
int IBISRowClear(ibis_id,srow,nrows)
int ibis_id;
int srow;
int nrows;
{
...
int IBISRowNew(ibis_id,srow,nrows)
int ibis_id;
int srow;
int nrows;
{
...
int IBISRowDelete(ibis_id,srow,nrows)
int ibis_id;
int srow;
int nrows;
{
...
void FTN_NAME(ibis_row_clear)(ibis_id, row, nrows, status)
int *ibis_id;
int *row;
int *nrows;
int *status;
...
void FTN_NAME(ibis_row_new)(ibis_id, row, nrows, status)
int *ibis_id;
int *row;
int *nrows;
int *status;
...
void FTN_NAME(ibis_row_delete)(ibis_id, row, nrows, status)
int *ibis_id;
int *row;
int *nrows;
int *status;
...
void FTN_NAME(ibis_signal)(ibis_id, status, abend_flag)
...
void FTN_NAME(ibis_signal_u)(unit, status, abend_flag)
...
XIBIS *ibis;
{
	return (zvopen( ibis->unit, "op", "read", "convert", "off", 0 ) );
}

static int open_update( ibis )
XIBIS *ibis;
{
	return (zvopen( ibis->unit, "op", "update", "convert", "off", 0 ));
}
...
void _i_ibis1_install_methods( ibis )
XIBIS *ibis;
{
...
XIBIS *ibis;
{
	return (zvopen( ibis->unit, "op", "read", "cond", "binary","lab_act","  ", 0 ) );
}

static int open_update( ibis )
XIBIS *ibis;
{
	return (zvopen( ibis->unit, "op", "update", "cond", "binary","lab_act","  ", 0 ));
}
...
void _i_ibis2_install_methods( ibis )
XIBIS *ibis;
{
...
static 	int grow_segments( ibis,  newsegment)
XIBIS *ibis;
int newsegment;
{
	int status=1;
...
static int _column_dofile( ibis, function, buffer,
				 column, srow, nrows,  inc)
XIBIS *ibis;
int (*function)(); /* zvwrit or zvread */
char *buffer;
int column;
int srow;
int nrows;
...
static int _columnnew(ibis,column,ncols,size)
XIBIS *ibis;
int column;
int ncols;
int size;
...
static int _columndelete(ibis,column,ncols)
XIBIS *ibis;
int column;
int ncols;
{
...
static int _record_dofile( record, function, mod_row, nrows )
XREC *record;
int (*function)(); /* zvwrit or zvread */
int mod_row;
int nrows;
...
static int _row_dofile(ibis, function, buffer, srow, nrows, inc)
XIBIS *ibis;
int (*function)();
char *buffer;
int srow;
...
static int _rowdelete(ibis,nrows)
XIBIS *ibis;
int nrows;
{
...
void _i_install_cfile_methods( ibis )
XIBIS *ibis;
{
...
static int move_columns( ibis, srccol,destcol, ncols )
XIBIS *ibis;
int srccol;
int destcol;
int ncols;
...
static 	int shrink_segments( ibis,  newsegment)
XIBIS *ibis;
int newsegment;
{
	int status=1;
...
int size;
{
	...
	int status;
	int new_segment;
	int dummy=size; /* so compilers won't complain */
...
int _i_install_grfile_methods( ibis )
XIBIS *ibis;
{
	ibis->colmethod->dofile = _column_dofile;
	ibis->colmethod->new = _columnnew;
...
int ncols;
{
	...
	{
		status = zvread( unit, blockbuf, "line", srcblock, 0);
		if (status !=1) break;
		status = zvwrit( unit, blockbuf, "line", destblock, 0);
		if (status !=1) break;
		srcblock += inc;
		destblock += inc;
	}
...
int size;
{
	...
	int newnc = ibis->nc+ncols;
	int blocks_per_col=(ibis->segment*4)/ibis->blocksize;
	int dummy=size; /* so compilers wont complain */
...
int _i_install_ofile_methods( ibis )
XIBIS *ibis;
{
	ibis->colmethod->dofile = _column_dofile;
	ibis->colmethod->new = _columnnew;
...
void _i_install_rfile_methods( ibis )
XIBIS *ibis;
{
	ibis->colmethod->dofile = _column_dofile;
	ibis->colmethod->new = _columnnew;
...
#define GET_IBIS( vname, pname )  \
   do { \
   		status = zlget( ibis->unit, "property",(pname),(vname), \
				"property","ibis",   0); \
		if (status!=1) goto end; \
   } while (0)
...
#define PUT_IBIS( pname, pvalue, pformat, pcount ) \
   do { \
		zldel( ibis->unit, "property",(pname),"property","ibis",0); \
		status=zladd( ibis->unit, "property",(pname),(pvalue), "property","ibis", \
		              "format",(pformat),"nelement",(pcount), 0); \
		if (status!=1) goto failure; \
   } while (0)


static int _flush_write();	/* forward ref to shut up compiler */
...
static int fetch_property_array( unit, prop, cptr, numelts )
int unit;
char *prop;
int *cptr;
int *numelts;
{
	...
	status=zlinfo( unit, "property", prop,  format,
	         &maxlen, numelts, "property","ibis", 0);
    ...
    status = zlget( unit, "property",prop,cptr, "property","ibis",
    			"nelement", *numelts,  0);
...
static int fetch_column_formats( ibis, fmt, cols, ncols )
XIBIS *ibis;
int fmt;
int *cols;
int ncols;
...
static int fetch_file_properties( ibis )
XIBIS *ibis;
{
	int status=1;
...
static int fetch_column_groups( ibis, type)
XIBIS *ibis;
char *type; /* UNIT, GROUP, etc */
{
	...
	status=zlinfo( unit, "property", listname,  format,
	          &grouplen, &numgroups, "property","ibis", 0);
    ...
    status = zlget( unit, "property",listname, grouplist, "property","ibis",
    			"nelement", numgroups,"ulen",grouplen,  0);
    if (status!=1) goto end;
...
static int fetch_column_properties( ibis )
XIBIS *ibis;
{
	int status=1;
	int buffer[MAX_COL];
...
static int put_column_groups( ibis, type, grouplist)
XIBIS *ibis;
char *type; 	/* "UNIT", "GROUP", etc */
List *grouplist;  	/* ibis->units, ibis->groups, etc */
{
	...
	status=zladd( ibis->unit, "property",listname,groupnames, "property","ibis",
	              "format","string","nelement",ngroups,"ulen", grouplen, 0); 
	if (status!=1) goto failure;
...
static int put_column_formats(ibis, format)
XIBIS *ibis;
int format;
{
	int num_fmt = 0;
...
static int _pre_open_write( ibis )
XIBIS *ibis;
{
	int status=1;
	int i;
...
static int _post_open_read( ibis )
XIBIS *ibis;
{
	int status=1;
...
static int _post_open_write( ibis )
XIBIS *ibis;
{
	int status;
...
XIBIS *ibis;
{
	 ...
	 */
	zldel( ibis->unit, "property",IFILE_TYPE, "property","ibis",0);
	for (fmt=FMT_BYTE; fmt<=FMT_ASCII; fmt++)
			zldel( ibis->unit, "property",format_label[fmt],"property","ibis",0);
	...
	if (status!=1) return status;
 	...
 	status=zldel( ibis->unit, "system",ILABEL_HOST,0);
	status=zladd( ibis->unit, "system",ILABEL_HOST,ibis->hostfmt,"format","string", 0);
	if (status!=1) return status;
	status=zldel( ibis->unit, "system",ILABEL_INTFMT,0);
	status=zladd( ibis->unit, "system",ILABEL_INTFMT,ibis->intfmt,"format","string", 0);
	if (status!=1) return status;
	status=zldel( ibis->unit, "system",ILABEL_REALFMT,0);
	status=zladd( ibis->unit, "system",ILABEL_REALFMT,ibis->realfmt,"format","string", 0);
	if (status!=1) return status;
	status=zldel( ibis->unit, "system",ILABEL_BTYPE,0);
	status=zladd( ibis->unit, "system",ILABEL_BTYPE, IBTYPE_IBIS,"format","string", 0);
	if (status!=1) return status;
...
int _i_install_nlabel_methods( ibis )
XIBIS *ibis;
{
	int status=1;
...
XIBIS *ibis;
{
	...
	status = zvread( unit, &nr_value, "line", 1, "nsamps", sizeof(int), 0);
	zvtrans( ibis->trans[FMT_FULL].intrans, &nr_value, &ibis->nr, 1 );
...
XIBIS *ibis;
{
	...
	zvtrans( ibis->trans[FMT_FULL].outtrans, &ibis->nr, &nr_value,  1 );
	status = zvwrit( ibis->unit, &nr_value, "line", 1, "nsamps", size, 0);
	if (status != 1) return status;
	...
	status=zldel( ibis->unit, "system","nl",0);
	status=zladd( ibis->unit, "system","nl",&nl,"format","int", 0);
...
int _i_install_olabel_methods( ibis )
XIBIS *ibis;
{
	int status=1;
...
int _i_install_grlabel_methods( ibis )
XIBIS *ibis;
{
	int status=1;
...
void _i_insert_value(list, value)
register List *list;
list_value value;
{
...
void _i_append_value(list, value)
register List *list;
list_value value;
{
	register List *ent;
...
void _i_delete_value(list, value)
register List *list;
list_value value;
{
...
int _i_insert_column_pointers( ibis, col, ncol )
XIBIS *ibis;
int col;
int ncol;
{
...
int _i_delete_column_pointers( ibis, col, ncol)
XIBIS *ibis;
int col;
int ncol;
{
...
int _i_move_column_pointers( ibis, srccol, destcol, ncol)
XIBIS *ibis;
int srccol;
int destcol;
int ncol;
...
int _i_attach_format_to_column(ibis,fmt,col)
XIBIS *ibis;
int fmt;
int col;
{
...
int _i_attach_column_to_group(list,grp,col)
List *list;
char *grp;
XCOL *col;
{
...
int _i_detach_col_from_format(ibis,col)
XIBIS *ibis;
int col;
{
	int fmt;
	...
	_i_delete_value( fgroup->columns, colm);
	
	return 1; 
}

int _i_detach_column_from_list(ibis,list,col)
XIBIS *ibis;
List *list;
int col;
{
	...
	{
		group = (XGROUP*)ent->value;
		_i_delete_value( group->columns, colm);
	}
	return 1; 
}
...
static XGAP *find_offset( ibis, offset )
XIBIS *ibis;
int offset;
{
	List *elt;
...
static XGAP *find_size( ibis, size )
XIBIS *ibis;
int size;
{
	List *elt=ibis->gaps;
...
_i_allocate_space( ibis, offset, size )
XIBIS *ibis;
int offset;
int size;
{
	...
	if (!pos) return 1; /* not in list */

	if (offset == pos->offset)
	{
		if (offset + size == pos->end)
			_i_delete_value( ibis->gaps, pos );
		else
...
_i_deallocate_space( ibis, offset, size )
XIBIS *ibis;
int offset;
int size;
{
	...
	if (!ibis->gaps) ibis->gaps = _i_new_list( free );
...
int _i_find_space( ibis, offset, size)
XIBIS *ibis;
int *offset;
int size;
{
...
char *group;
{
	...
	newgrp->columns = _i_new_list(0); /* no destruction */
...
char *_i_parse_group_name( ibis, name, list)
XIBIS *ibis;
char *name;
List **list;
{
...
XGROUP* _i_find_group_all( ibis, grp )
XIBIS *ibis;
char *grp;
{
...
XGROUP* _i_request_group( list, grp )
List *list;
char *grp;
{
	XGROUP *group;
...
static XGROUP* group_AND(grp1,grp2)
XGROUP *grp1;
XGROUP *grp2;
{
...
static XGROUP* group_OR(grp1,grp2)
XGROUP *grp1;
XGROUP *grp2;
{
	List *vals;
...
static XGROUP* group_DIFF(grp1,grp2)
XGROUP *grp1;
XGROUP *grp2;
{
	List *vals1=grp1->columns->next;
...
XGROUP* _i_group_construct(ibis, expression)
XIBIS *ibis;
char *expression;
{
	char *expr;
...
int _i_notify_records(ibis, notice, sval, endval)
XIBIS *ibis;
notice_type notice;
int sval;
int endval;
...
int _i_IBISFormatSize( ibis, format, size)
XIBIS *ibis;
int format;
int *size;
{
...
int _i_strcmp_nocase(str1,str2)
char *str1;
char *str2;
{
	char c1,c2;
...
int _i_strncmp_nocase(str1,str2,n)
char *str1;
char *str2;
int n;
{
...
int _i_keymatch(keystr, keys)
char *keystr;  /* input key string */
char **keys;   /* array of pointers to keys */
{
	int keyno;
...
int _i_compute_blocksize(ibis, lower_bound )
XIBIS *ibis;
int lower_bound;
{
...
int _i_useful_segment( lower_bound )
int lower_bound;
{
	int bound=1;
...
void _i_compute_new_offsets(ibis,scol,ncol)
XIBIS *ibis;
{
	int i;
...
int _i_process_contiguous(unit,function,buffer,offset,
				nbytes,blksize,recsize,inc)
int unit;
int (*function)(); /* zvwrit or zvread */
char *buffer;
int offset;
...
int inc; /* increment buffer or not ? */
{
	...
	{
	    if (contiguous)
		status = (*function)( unit, buffer,"line", line, 0);
	    else
...
int _i_clear_ibis_file(ibis, sblock)
XIBIS *ibis;
int sblock;
{
	char *buffer;
	...
	{
		status = zvwrit( unit, buffer, "line", block, "nsamps",blocksize, 0);
		if (status!=1) break;
	}
...
int _i_install_numblocks(ibis, label,numblocks)
XIBIS *ibis;
char *label;  /* "NL" or "NLB" */
{
	int status;
	...
	zldel( ibis->unit, "system",label,0);
...
static void ascii_to_ascii( inlen, outlen, inbuf, outbuf, num, nullend )
int inlen;
int outlen;
char *inbuf;
char *outbuf;
...
int _i_set_trans(ibis,trans,buffmt,filefmt)
XIBIS *ibis;
trans_type *trans;
int buffmt;
int filefmt;
{
	...
	zvtrans_in( trans->intrans, format_name[ filefmt ],
		format_name[buffmt],ibis->intfmt, ibis->realfmt);
	zvtrans_out( trans->outtrans, format_name[buffmt],
...
int _i_trans_buf_to_local( trans, col, inbuf, outbuf, num )
trans_type *trans;
XCOL *col;
char *inbuf;
char *outbuf;
int num;
{
	...
	if (infmt<FMT_ASCII && infmt>FMT_NONE)
		zvtrans( trans->intrans, inbuf, outbuf, num );
	else
	{
...
int _i_trans_buf_from_local( trans, col, inbuf, outbuf, num )
trans_type *trans;
XCOL *col;
char *inbuf;
char *outbuf;
int num;
{
	...
	if (infmt<FMT_ASCII && infmt>FMT_NONE)
		zvtrans( trans->outtrans, inbuf, outbuf, num );
	else
	{
		int format=col->format;
		int filesize=trans->format_size[format];
...
/**
 ** Externals referenced by several routines
 **/
extern int zvwrit();
extern int zvread();
...
XGROUP* _i_group_construct(/* char *expression */);
char *_i_parse_group_name(/* ibis, name, list */);
void *_i_mem_dup( /* data, size */);
...
void _i_free_record(/* rec */ );
int _i_null_method();
int _i_strcmp_nocase();
int _i_strncmp_nocase();
...
#define NEED_TRANS( ibis, col ) \
	((col)->trans != NULL || \
	   ...
	   ((col)->format < FMT_ASCII  && (col)->format > FMT_NONE && \
	   	(ibis)->trans[(col)->format].outtrans[0]) )
...
struct List {
	struct List *next;
	struct List *prev;
	void (*destruct)(); /* destruction method for value */
	list_value value;
};
...
struct trans_type {
	int intrans[12];	/* put these first so they're aligned well */
	int outtrans[12];
