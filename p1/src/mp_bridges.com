...
$! VPACK Version 1.9, Sunday, November 07, 1999, 08:44:39
$!
...
void FTN_NAME(mp_init)(MP *mp, int *fstat )
{
...
void FTN_NAME(mp_get_keywords)( MP *mp,
...
void FTN_NAME(mp_get_value)( MP *mp, char *fkey, double *val, int *fstat,
	ZFORSTR_PARAM)
{
  ZFORSTR_BLOCK
  char key[41];
  ...
  *fstat = mpGetValues( *mp, key, val, 0 );
}
...
void FTN_NAME(mp_get_value_str)( MP *mp, char *fkey, char *fval, int *fstat,
		ZFORSTR_PARAM)
{
  ZFORSTR_BLOCK
...
void FTN_NAME(mp_ll2xy)( mp, y, x, lat, lon, ll_type, status)
MP *mp;
double *x;
double *y;
double *lat;
...
void FTN_NAME(mp_set_debug_flag)( flag, status)
int *flag;
int *status;
{
...
/*****************************************************************/
	 ...
	 *
	 */
void FTN_NAME(mp_set_value)( MP *mp, char *fkey, double *val, int *fstat,
		ZFORSTR_PARAM)
{
  ZFORSTR_BLOCK
  char key[31];
  ...
  *fstat = mpSetValues( *mp, key, *val, 0 );
}
...
void FTN_NAME(mp_set_value_str)( MP *mp, char *fkey, char *fval, int *fstat,
		ZFORSTR_PARAM)
{
  ZFORSTR_BLOCK
  char key[mpMAX_KEYWD_LENGTH+1], val[mpMAX_KEYWD_LENGTH+1];
...
void FTN_NAME(mp_xy2ll)( mp, y, x, lat, lon, ll_type, status )
MP *mp;
double *x;
double *y;
double *lat;
...
void FTN_NAME(mp_label_write)( MP *mp, int *unit, char *flabtyp, int *fstat,
		ZFORSTR_PARAM)
#if 0
MP *mp;					/* IN:  user's MP structure */
int *unit;				/* IN:  VICAR file unit number */
...
void FTN_NAME(mp_buf2mpo)( buf, mp, fstat)
 void *buf;
 MP *mp;
 int *fstat;
{
...
void FTN_NAME(mp_mpo2buf)( mp, buf, fstat)
 MP *mp;
 void *buf;
 int *fstat;
{
