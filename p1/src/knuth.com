...
$! VPACK Version 1.9, Sunday, February 11, 2007, 10:31:49
$!
...
void FTN_NAME(knuth)(char *string, int *buf, int *status, ZFORSTR_PARAM)
#if 0
char *string;        /* input: string to parse                */
int *buf;            /* output: compiled code & registers     */
...
void FTN_NAME(knuth_var)(char *fname, int *value, ZFORSTR_PARAM)
#if 0
...
void FTN_NAME(xkn65_exe)(buf,table,count,num_input,flags)
...
void FTN_NAME(knuth_lookup)(buf,table,num_input,flags)
float *buf;          /* input:  variables,constants,operators,operands  */
unsigned char *table;/* output: the 256x256 combinations of IN1 and IN2 */
int *num_input;      /* input:  the number of input files               */
int *flags;          /* input:  numbers are rounded or truncated ?      */
...
void FTN_NAME(xknuth)(buf,result)
...
void FTN_NAME(knuth_complex)(char *string, complex_type *buf,int *status,
			ZFORSTR_PARAM)
#if 0
char *string;        /* input: string to parse                */
...
void FTN_NAME(xknuth_complex)(buf,result)
...
int zknuth_complex(string,buf)
char *string;      /* input: string to parse                */
complex_type *buf; /* output: compiled code & registers     */
{
	int status=zknuth(string,buf);
