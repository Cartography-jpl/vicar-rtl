...
$! VPACK Version 1.8, Thursday, February 16, 1995, 11:29:34
$!
...
int zrdgr(instance,num,dimension)
int instance;
int num;
int dimension;
{
...
int zrdgr_unit(unit,num,dimension)
int unit;
int num;
int dimension;
{
...
int zwrgr(instance,num,dimension)
int instance;
int num;
int dimension;
{
	...
	status = zvunit(&unit,"out",instance,0);
	if (status !=1) return status;
...
int zwrgr_unit(unit,num,dimension)
int unit;
int num;
int dimension;
{
...
int zupdategr(unit,num,dimension)
int unit;
int num;
int dimension;
{
...
int zgetgr(num,zero1,eof,first_c,second_c,other_c)
int num;
int *zero1;
int *eof;
float *first_c;
...
int znextgr(num,eof,first_c,second_c,other_c)
int num;
int *eof;
float *first_c;
float *second_c;
...
int zputgr(num,first_c,second_c,other_c)
int num;
float first_c;
float second_c;
float *other_c;
{
	...
	if (graph->write_row > graph->size)
	{
		...
		/* Extend file */
		status = IBISFileSet(graph->ibis,IFILE_NR,(char *)newsize,0);
		if (status != 1) return status;
		graph->size = newsize;
	}
...
int num;
{
	...
	if (graph.flags & FLAG_FILE_NEW)
	{
		...
		status = IBISFileSet(graph.ibis,IFILE_NR,(char *)graph.write_row-1,0);
		if (status != 1) status2=status;  /* but continue */
	}
...
int dimension;
{
	...
	status = IBISFileSet(ibis,IFILE_NR,ROW_INC,0);
	if (status != 1) return status;


 	status=zldel( unit, "system","type",0);
	if (status != 1) return status;
	status=zladd( unit,"system","type","GRAPH1","format","string", 0);
	if (status != 1) return status;
...
int FTN_NAME(rdgr)(instance,num,dimension)
int *instance;
int *num;
int *dimension;
{
...
int FTN_NAME(rdgr_unit)(unit,num,dimension)
int *unit;
int *num;
int *dimension;
{
...
int FTN_NAME(wrgr)(instance,num,dimension)
int *instance;
int *num;
int *dimension;
{
...
int FTN_NAME(wrgr_unit)(unit,num,dimension)
int *unit;
int *num;
int *dimension;
{
...
int FTN_NAME(updategr)(unit,num,dimension)
int *unit;
int *num;
int *dimension;
{
...
int FTN_NAME(getgr)(num,zero,eof,first_c,second_c,other_c)
int *num;
int *zero;
int *eof;
float *first_c;
...
int FTN_NAME(nextgr)(num,eof,first_c,second_c,other_c)
int *num;
int *eof;
float *first_c;
float *second_c;
...
int FTN_NAME(putgr)(num,first_c,second_c,other_c)
int *num;
float *first_c;
float *second_c;
float *other_c;
...
int FTN_NAME(endgr)(num)
int *num;
{
	return zendgr(*num);
}

int FTN_NAME(setgr)(num, row)
int *num;
int *row;
{
...
int FTN_NAME(clgr)(num)
int *num;
{
	return zclgr(*num);
}

void FTN_NAME(signalgr)(num,status,abortflag)
int *num;
int *status;
int *abortflag;
{
