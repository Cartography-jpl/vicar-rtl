...
FUNCTION  static  CODE  taegetline
(
...
(
 ...
 TEXT field[]	/* OUT: associated text */
    ...
    do
	{
	code = taegetline(db, FALSE, record);		/* read next record */ 
...

/*
 * 	taegetline.  Get the next non-comment line. 
...
(
 ...
 TEXT		record[]	/* OUT: text record */
    ...
    code = taegetline(db, TRUE, record);		/* read next non-comment line*/
...
(
 ...
 TEXT		record[]	/* OUT: text record 		*/
    ...
    code = taegetline(db, FALSE, record);		/* read next non-comment line*/
    if (code != SUCCESS) 
