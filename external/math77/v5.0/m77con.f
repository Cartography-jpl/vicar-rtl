...
c (or at least some precsion greater than double), Integer, Complex,
...
c this operand has the value true, and othewise has the value false.
...
c
c There are integer, logical, and string types.  An integer constant is
c defined as in Fortran, the logical constants are .T., .F., and .U.
c (.U. indicates an undefined value, with T=.T., F=.F., and U=.U., we
c have, U|F=.U., U|T=.T., U&F=.F., U&T=.U., ~U=.U.  If U is on either
c side of a relational operator, the result is .U.)
...
c present, then the following line is to be converted usintg Li',
...
c types I, P, X, C, Z,  and W, if we have not encountered a "C-- T4 Next
c line special..." line referencing the current type.
c DATEND Set =.true. when data staement processing is to end, see LDATA.
c I      Temporary index.
...
c              pointes to the end of the string.
...
c        Also KVERS is negated when procesing a "C++ Current has..."
...
c LDATA  Flag to tell state of processing special data statments.
...
c LENHED Number fo characters in the heading text.
...
c LTYIN  Current index as for LTYUI.
c LTYOUT As for LTYIN, but for the current output type.
c LTYUO  Default index for the output type, 1 for S, 2 for D, 3 for Q.
c LTYUI  Default index for the input type, 1 for S, 2 for D, 3 for Q.
...
c NAMDAT Name of array in modified data statment.
...
c NERR   Numnber of errors encountered.
...
c MXSTR  Maximum number of strings for replacement.
...
c Unit numbers
c   8   For reading the control information from m77job
c   9   For reading the current file.
c  10   For writing the generated file.
c  11   A scratch file used to save lines prior to getting a "C--"
c  12   For reading the comparison file.
c TEMPC  Character string used for temporary storage.
c
c ********************** Specifications ********************************
c
...
c
     ...
     5   LSTAT, LTYIN, LTYOUT, LTYUI, LTYUO, LOOK, LOP, MAP(128),
     6   MAPPED(LENLIN), MOREPR, NAMBEG, NAMEND, NERR, NFILE, NUM0,
     7   NUMNAM, NUMSTR, LSTOLD(0:MXSTR), LSTNEW(0:MXSTR),
     8   NAMMRK(MXNAME), NAMREF(MXNAME)
      logical CHGLIN, CONVOK
      common /M77C1/ BUF1, BUF2, BUF3, C, CNTRL, HEADER, INFUNC, KEYWRD,
     2   LETFUP, LIN, LINLO, LINU, LINUP, LOUT, LOUTLO, LOUTU, LOUTUP,
     3   NAMES, PATH, STROLD, STRNEW, STRSAV
     ...
     4   LSTNEW, LTYIN, LTYOUT, LTYUI, LTYUO, LOOK, LOP, MAP, MAPPED,
     5   MOREPR, NAMBEG, NAMEND, NERR, NFILE, NUM0, NUMNAM, NUMSTR,
     6   NAMMRK, NAMREF, CHGLIN, CONVOK
c End of declarations required for common
      logical SCRIN, DATEND
      integer I, I1, I2, II, J, K, KXCERR, L, LDAT, LENHED, LINEI,
     1   LNAMDA
     ...
     4   MAPPLU= 20, MAPMIN =21, MAPSTR =22, MAPCAT =23 )
      integer MXCERR
      parameter (MXCERR = 10)
      character BUFDAT*72, NAMDAT*20, LETTYP*9, TEMPC*30
      data LETTYP / 'SDQICZWPX' /
c
c ******************** Formats *****************************************
c
 1010 format(' Finished processing, ', I4, ' files processed, ', I4,
     1   ' errors.')
c
c
         ...
         if (BUF1(2:3) .eq. '>>') then
            if (BUF1(4:) .ne. ' ') then
               ...
               do 110 LENHED = LENLIN, 2, -1
                  if (HEADER(LENHED:LENHED) .ne. ' ') go to 120
  110          continue
...
c
      if ((BUF2(1:4).eq.'MAKE') .and. (KPT(4) .eq. 4)) then
         LOOK = 5
         call GETMAK(LOUTUP, LOUTLO, LOUT, LTYUO)
         LOUTU = LOUTUP
     ...
     1         'Confused in "from" part of a "MAKE..." line')
            LOOK = LOOK + 4
            call GETMAK(LINUP, LINLO, LIN, LTYUI)
         end if
         LINU = LINUP
         LTYOUT = LTYUO
         LTYIN = LTYUI
...
c
  ...
  240       continue
      ...
      K = 4
  300 K = K + 1
      if (BUF1(K:K) .eq. ' ') go to 300
      if (L .ne. 0) BUF1(L:L) = LIN
      if (J .eq. 0) then
         I = K
  310    I = I + 1
         if (BUF1(I:I) .ne. ' ') go to 310
         I = I - 1
      else
         I = J
  320    I = I - 1
         if (BUF1(I:I) .eq. ' ') go to 320
      end if
      BUF3 = BUF1(K:I) // ' ==> '
      II = I - K + 7
      if (L+J .eq. 0) then
...
c Set up letter for the output file.
         ...
         if (J .ne. 0) then
  330       J = J + 1
            if ((BUF1(J:J) .eq. ' ') .and. (J .lt. LENLIN)) go to 330
            C = BUF1(J:J)
            if ((C .ne. '.') .and. (BUF1(J+1:J+1) .ne. '.')) then
               ...
               L = J
  340          L = L + 1
               if (BUF1(L:L) .ne. ' ') go to 340
  350          I = I - 1
               if (BUF1(I:I) .ne. C) then
                  if (I .ge. K) go to 350
                  stop 'No processing -- bad "ext" in "FILE" statment.'
               end if
...
c                         Take care of fixing data statements
         if (abs(LDATA) .eq. 2) then
            call pack
            DATEND = BUF1(1:5) .ne. '     '
            if (DATEND) then
               DATEND = (BUF1(6:6) .eq. ' ') .or. (BUF1(6:6).eq.'0')
               if (DATEND) DATEND = BUF2(1:4+LNAMDA) .ne.
     1            'DATA'//NAMDAT(1:LNAMDA)
            end if
            if ((LDATA .eq. 2) .or. DATEND) then
c                         End line with ' /'
               BUFDAT(LDAT+1:LDAT+2) = ' /'
               LDAT = LDAT + 1
               if (DATEND) LDATA = 0
            else
               BUFDAT(LDAT+1:LDAT+1) = ','
            end if
            write(10, '(A)') BUFDAT(1:LDAT+1)
         else if (BUF1(1:1) .ne. ' ') then
            go to 407
         else
            call pack
            if (BUF2(1:4) .ne. 'DATA') go to 407
            LNAMDA = index(BUF2(6:LAST), '(')
            if (LNAMDA .eq. 0) then
               LNAMDA = index(BUF2(6:LAST), '/')
               if (LNAMDA .eq. 0) then
...
c                                Get start of line.
               ...
               II = KDATA
               if (II .lt. 0) II = -II
  402          TEMPC(K:K) = char(mod(II, 10) + NUM0)
               K = K - 1
               II = II / 10
               if (II .ne. 0) go to 402
               if (KDATA .lt. 0) then
                  TEMPC(K:K) = '-'
                  K = K - 1
               end if
               II = K - LNAMDA - 5
               TEMPC(II:K) = 'DATA '//NAMDAT(1:LNAMDA)//'('
               KDATA = KDATA + 1
               BUF1(6:6) = ' '
            else if (LDATA .eq. - 1) then
               II = 13 - LNAMDA
               TEMPC(II:20) = 'DATA ' // NAMDAT(1:LNAMDA) // ' / '
               LDATA = -2
            else
               II = 20
               TEMPC(20:20) = ' '
            ...
            if (BUF2(1:4) .eq. 'DATA') then
               K = index(BUF1(1: KPT(LAST)), '/')
               if (K .eq. 0) then
                  call ERROUT('Missing "/" in DATA statement')
                  LDATA = 0
                  go to 407
                  ...
                  K = K + 1
                  if (BUF1(K:K) .eq. ' ') K = K + 1
               end if
            else
               K = KPT(2) - 1
            end if
            call COPY(TEMPC(II:20), 7, 21 - II, K - 7, 0)
            LDAT = KPT(LAST)
  405       LDAT = LDAT - 1
            if (BUF1(LDAT:LDAT) .eq. ' ') go to 405
            BUFDAT(1:LDAT) = BUF1(1:LDAT)
            go to 408
         end if
      end if
  407 write(10, '(A)') BUF1(1:KPT(LAST))
  408 if (BUF2(1:3) .eq. 'END') then
     ...
     1      (BUF2(4:4) .eq. '!')) then
            ...
            if (LSTAT .gt. 0) then
               ...
               LTYOUT = LTYUO
               LTYIN = LTYUI
               if (LTYOUT * LTYIN .eq. 0) then
                  INFDIF = 0
               else
                  INFDIF = LOCFUN(LTYOUT) - LOCFUN(LTYIN)
               end if
            end if
      ...
      if (KXCERR .gt. 0) then
c                             Comparing files.
  410    read(12, '(A)', END=950) BUF3
         if (BUF3(2:3) .eq. '--') then
            if ((BUF1(2:3).ne.'--') .and. (BUF3(4:4).ne.'-')) go to 410
         end if
  420    if (BUF1 .ne. BUF3) then
...
c is skip the diagnostic and one read of BUF3.
  ...
  430       I = I + 1
            if (I .le. LENLIN) then
               if (BUF1(I:I) .eq. ' ') go to 430
            end if
  440       J = J +1
            if (J .le. LENLIN) then
               if (BUF3(J:J) .eq. ' ') go to 440
            end if
            if ((I .le. LENLIN) .and. (J .le. LENLIN))then
               if (BUF1(I:I) .eq. BUF3(J:J)) go to 430
            else if (I .eq. J) then
               go to 460
            end if
  ...
  460 LINE = LINE + 1
      CHGLIN = .false.
      if (SCRIN) then
         read(11, END = 470) BUF1
         go to 480
  470    SCRIN = .false.
      end if
      read(9, '(A)', END = 900) BUF1
  480 if (BUF1(2:3) .eq. '++') then
         if (BUF1(4:4) .ne. '+') then
            if (SCRIN) go to 360
            call PROCPP
...
c              1    2    3    4    5    6    7    8
  490 go to (560, 560, 520, 500, 510, 530, 540, 550), KVERS
c Turning a block of actives off.
  500 BUF2 = BUF1
      if (BUF2(LENLIN:LENLIN) .ne. ' ') then
         call ERROUT(
     1      'About to shift character off right end of line')
      end if
      BUF1 = 'C' // BUF2(1:LENLIN-1)
      if (BUF1 .eq. 'C ') BUF1(1:1) = ' '
  510 if (BUF1(3:4) .eq. '--') go to 570
      go to 560
c Turning a block if inactives on.
  520 BUF2 = BUF1
      BUF1 = BUF2(2:LENLIN)
      go to 560
...
c
  560 if (LSTAT .lt. 0) then
         if (LSTAT .lt. -2) go to 770
         if (LSTAT .eq. -2) then
            if (BUF1(2:3) .eq. '--') then
               if (LOUTUP .eq. ' ') then
...
c                   Reset the state, starting the scratch file.
      ...
      if (BUF1(2:3) .ne. '--') go to 580
c                    Do nothing for one of the initial "C--..." lines
  570 if (index(BUF1, '?') .ne. 0) go to 360
      call PACK
      if (BUF2(4:4) .ne. '-') call PROCMM
      go to 380
  580 call PACK
      if (INFDIF .le. 0) then
         if (INFDIF .eq. 0) go to 630
         if (INFDIF .eq. -1000) go to 740
      end if
c
c                                Take care of type conversion
c                           First the Keywords.
      L = LENKEY(LTYIN)
      K = index(BUF2(1:LAST), KEYWRD(LTYIN)(1:L))
      if (K .ne. 0) then
         if (index(BUF2(1:LAST), '=') .eq. 0) then
            if (KPT(K) .lt. 7) go to 600
            if ((K .gt. 8) .and. (KPT(K-1) .gt. 6)) then
               if (index(BUF2(1:LAST), 'IMPLICIT') .ne. K - 8)
     1            go to 600
            else
               do 590 I = 2, K-1
                  if (MAPPED(I) .ne. MAPDIG) go to 600
  590          continue
            end if
c                                               Treat as a keyword
            if (KEYBLK(LTYIN) .ne. 0) L = L + 1
            call COPY(KEYWRD(LTYOUT), KPT(K), LENKEY(LTYOUT),
     1         L, KEYBLK(LTYOUT))
            BUF2(K:K+LENKEY(LTYIN)-1) = ' '
         end if
      end if
c          Take care of the processor defined functions.
  600 do 620 I = LOCFUN(LTYIN), LOCFUN(LTYIN+1) - 1
         J = 1
  610    K = index(BUF2(J:LAST), INFUNC(I)(1:LENFUN(I)))
         if (K .ne. 0) then
            K = K + J - 1
            J = K + 1
            I1 = KPT(K)
...
c                     Think we have an intrinsic function name.
                  if (I + INFDIF .ge. LOCFUN(LTYOUT+1)) then
                     call ERROUT(
     1     'No output intrinsic corresponds to the input intrinsic')
                  else
     ...
     1                  LENFUN(I+INFDIF), LENFUN(I), 0)
            ...
            if (J .lt. LAST) go to 610
         end if
  620 continue
c
c        Take care of the user names
  630 do 660 I = 1, NUMNAM
         J = 1
  640    K = index(BUF2(J:LAST), NAMES(I)(1:LENNAM(I)))
         if (K .ne. 0) then
            K = K + J - 1
            J = K + LENNAM(I)
            I1 = KPT(K)
            if (I1 .gt. 1) then
               if (MAP(ichar(BUF1(I1-1:I1-1))) .gt. MAPDIG) then
                  ...
                  if (I1 .lt. LENLIN) then
                     if (MAP(ichar(BUF1(I1:I1))).le.MAPDIG) go to 650
                  end if
                  NAMREF(I) = NAMREF(I) + 1
                  K = K + NAMMRK(I)
                  II = KPT(K)
  ...
  650       if (J .le. LAST) go to 640
         end if
  660 continue
c
  670 if (NUMSTR .ne. 0) then
c                 Take care of string replacements
         ...
         do 700 I = 1, NUMSTR
            ...
            J = LSTOLD(I)
            L = 1
  680       K = index(BUF1(L:), STROLD(II:J-1))
            if (K .ne. 0) then
               K = K + L - 1
               I1 = J - II
               L = K + I1 - 1
               if (BUF3(K:L) .ne. ' ') go to 680
               BUF1(K:L) = ' '
               I2 = LSTNEW(I) - LSTNEW(I-1)
               if (I2 .gt. I1) then
c When new is bigger try to sop up extra blanks.
                  I1 = min(KPT(LAST), I2 + K + 1)
  690             L = L + 1
                  if ((BUF1(L:L).eq.' ') .and. (L.lt.I1)) go to 690
                  L = L - 1
                  I1 = L - K
               end if
  ...
  700    continue
         if (LSTAT .lt. -2) go to 790
         if (LSTAT .eq. -2) go to 360
      end if
c                       Look for floating point constants
      I1 = 1
  710 K = index(BUF2(I1:LAST), '.')
      if (K * LTYIN .ne. 0) then
         K = I1 + K - 1
         if (MAPPED(K-1) .gt. MAPUC) then
            do 720 K = K + 1, LAST - 1
               if (MAPPED(K) .ne. MAPDIG) then
                  if (BUF2(K:K) .eq. LETFUP(LTYIN:LTYIN)) then
                     if ((MAPPED(K+1) .eq. MAPDIG) .or.
     1                  (MAPPED(K+1) .ge. MAPPLU)) then
                        J = KPT(K)
                        C = LETFUP(LTYOUT:LTYOUT)
  ...
  730    I1 = K + 1
         if (I1 .lt. LAST) go to 710
      end if
  740 if (LSTAT .eq. 0) go to 380
      LSTAT = 0
c
c          Set indices for type conversion
  750 LTYOUT = LTYUO
      ...
      if (LINUP .ne. LINU) then
         LINUP = LINU
         do 760 I = 1, min(LNAMU, NUMNAM)
            K = NAMMRK(I) + 1
            NAMES(I)(K:K) = LINUP
  760    continue
      end if
      if (LTYIN .ne. LTYUI) then
         LTYIN = LTYUI
         C = LETTYP(LTYIN:LTYIN)
         do 765 I = LNAMU+1, NUMNAM
            K = NAMMRK(I) + 1
            NAMES(I)(K:K) = C
  765    continue
      end if
...
c                              In the initial state
  770 if (BUF1(2:3) .eq. '--') go to 800
  790 if (LSTAT .eq. -3) then
...
c
      C = BUF2(4:4)
      if (index(BUF1, '?') .eq. 0) then
         if (index(BUF2, 'MASKCODECHANGE') .ne. 0) go to 790
         if (BUF2(5:20) .eq. 'NEXTLINESPECIAL:') go to 790
c    Following test is temporary until we get rid of these lines.
         if (BUF2(5:30) .eq. 'USEDFORUSUALPRECISIONBELOW') go to 870
c                 Take care of old form "C--..." lines.
         LOOK = 0
         call GETNAM
         if (KNT .eq. 1) go to 460
...
c            Should be processing continuations.
  ...
  850 SCRIN = .true.
      LINE = LINEI
  860 write(11) BUF1
  870 if (.not. SCRIN) go to 460
      rewind(11)
      go to 750
c                             Got an end of file.
  900 if (LSTAT .ge. -2) then
c             At the end of a program unit as well as EOF.
     ...
     1         'No output file -- no special lines for the output type')
         ...
         if (LPATH .ne. 0) then
  910       read(12, '(A)', END=930) BUF1
            if (BUF1 .eq. ' ') go to 910
...
c                         End of the comparison file
  950 if (BUF1 .eq. ' ') go to 460
      call ERROUT('Verify file ended prematurely')
      KXCERR = 0
      go to 460
c
c                         End of the control file.
 1000 close (11, STATUS = 'DELETE')
      print 1010, NFILE, NERR
      stop
      end
...
c                       J = place in BUF1 where text in BUF3 goes.
               J = K + KK
               JJ = LENLIN - J + 1
               if (BUF3(JJ+1:) .ne. ' ') then
                  call ERROUT('Shifting off text at end of line')
               else if (BUF3(JJ:JJ) .eq. ' ') then
                  BUF1(J:J) = ' '
                  J = J + 1
               end if
               BUF1(J:LENLIN) = BUF3(1:)
               II = J - I
               do 90 I1 = LAST, 1, -1
...
c                                    May be logical constant
               if (BUF2(LOOK+1:LOOK+1) .eq. 'T') then
                  ITYPE = -1
               else if (BUF2(LOOK+1:LOOK+1) .eq. 'F') then
                  ITYPE = -3
               else if (BUF2(LOOK+1:LOOK+1) .eq. 'U') then
                  ITYPE = -2
               else if (BUF2(LOOK+1:LOOK+1) .eq. 'X') then
                  ITYPE = LOOK + 1
                  IVAL = ITYPE
                  BUF2(IVAL:IVAL) = LETFUP(LTYOUT:LTYOUT)
               else if (BUF2(LOOK+1:LOOK+1) .eq. 'N') then
                  ITYPE = LOOK + 1
                  IVAL = ITYPE
                  BUF2(IVAL:IVAL) = LOUTUP
               else if (BUF2(LOOK+1:LOOK+1) .eq. 'Y') then
                  ITYPE = LOOK + 1
                  IVAL = ITYPE
                  BUF2(IVAL:IVAL) = LETTYP(LTYUO:LTYUO)
               else
                  go to 210
               end if
               LOOK = LOOK + 3
...
c   End of GETEXP
      ...
      subroutine GETMAK(LUP, LLO, LC, LTYU)
...
c
     ...
     1   'Letter required to define file')
      ...
      LC = BUF1(LEND:LEND)
      LTYU = index(LETTYP, LUP)
      LOOK = LOOK + 1
      if (BUF2(LOOK:LOOK+7) .eq. 'WITHTYPE') then
         LTYU = index(LETTYP, BUF2(LOOK+8:LOOK+8))
         if ((LTYU .eq. 0) .or. (LTYU .gt. 7)) call ERROUT(
...
c               call ERROUT('Unknown type of "C--..." line')
  ...
  100          continue
     ...
     1               LETTYP(LTYIN:LTYIN)
                  ...
                  call ERROUT(
     1      'When converting old "C--..." lines, names do not match up')
               end if
               LOOK = LEND + 2
               go to 20
...
c
      ...
      if (K .eq. 0) then
         if (BUF2(4:23) .eq. 'BEGINMASKCODECHANGES') then
            ...
            call ERROUT(
     1        'Invalid letter specifying precision -- line ignored')
         end if
         return
      end if
      if (LOUT .eq. ' ') return
...
c This particular "if block" could probably be deleted.
  ...
  100 call ERROUT('Badly formed "C--..." line ignored')
      return
  200 if (LENKEY(LTYOUT) .eq. 0) then
         call ERROUT('No change -- Output type not defined.')
         LTYOUT = LTYUO
         LOUTUP = LOUTU
      else if (LENKEY(LTYIN) .eq. 0) then
...
c Got a logical type -- Use old state from L and ITYPE to get new state.
            ...
            call ERROUT(
     1       'Line ignored, expressions must be of type logical')
            return
      end if
...
c                  Need to cover up "," that was part of text.
            ...
            call ERROUT(
     1        'Line ignored -- couldn''t get proper line counts')
            return
      end if
...
c                           Convert old form "C++ SET instruction".
   ...
   80       call ERROUT('Badly formed "C++...", line ignored')
         ...
         if (K .gt. 4) then
            if (index(BUF2(4:K-1), LOUTUP) .eq. 0) return
         end if
         LOOK = K + 7
      end if
      call getasg
...
c
      ...
      call pack
      BUF2 = BUF3
   20 L = index(BUF2(LOOK:), ',')
      if (L .ne. 0) then
         LEND = LOOK + L - 2
         J = index(BUF1, BUF2(LOOK:LEND))
         if (J .ne. 0) then
            if (MAP(ichar(BUF1(J-1:J-1))) .gt. MAPDIG) then
               J = J + L - 2
   40          J = J + 1
               if (BUF1(J:J) .eq. ' ') go to 40
               if (BUF1(J:J) .eq. '=') then
   60             J = J + 1
                  if (BUF1(J:J) .eq. ' ') go to 60
                  call GETAND
                  if (ITYPE .eq. -4) then
