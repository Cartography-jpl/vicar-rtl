...
c--S replaces "?": ?LGAMA, ?GAMMA, ?ERM1, ?ERV1
C
C     Designed and programmed by W.J.CODY, Argonne National Lab, 1982.
C     Minor changes for use in the JPL MATH77 library by C.L.LAWSON &   00001200
C     S.CHAN, JPL, 1983.
...
C     functions that approximate LOG(GAMMA) to least 18                 00002200
...
C           number.
C
C  EPS      The smallest positive floating-point number such            00003200
...
C
C-- Begin mask code changes                                             00004200
...
c  2**1070 = 0.126e323    177.78  0.1501e320   CDC/7600 SP              00005200
...
C
c     ------------------------------------------------------------------00006200
      external         SGAMMA, R1MACH, SERM1, SERV1
      real             SGAMMA, R1MACH
      real             ALN4, C,CORR,D1,D2,D4, DEL
      real             EPS, FIVE, FRTBIG,FOUR,HALF
      real             LOMEGA, ONE, P1
      real             P2,P4, P65, Q1,Q2,Q4,RES,SQRTPI
      real             T1, T5, THRHAL,TWELVE, U, X,XLBIG,XDEN
      real             XINF,XM1,XM2,XM4,XNUM
      real             Y, Y1, Y2, YSQ, ZERO
      integer I, J                                                      00007200
...
C
      ...
      DATA P65 / 0.65E0 /
C                                           T5 = LN(SQRT(4*PI)) - HALF  00008200
...
c ----------------------------------------------------------------------
     ...
     *        2.637748787624195437963534E4,7.225813979700288197698961E3/
      DATA Q1/6.748212550303777196073036E1,1.113332393857199323513008E3,00009500
...
c ----------------------------------------------------------------------
C  NUMERATOR AND DENOMINATOR COEFFICIENTS FOR RATIONAL MINIMAX
C     APPROXIMATION OVER (4.0,12.0)                                     00011400
c ----------------------------------------------------------------------
...
c        0.0121 when XINF = 2**127 to 0.000176 when XINF = 2**8191.     00014400
c
      ...
      DO 20004 J=1,7
            ...
            DEL = Y2 - Y1
            if(abs(DEL) .lt. 0.5e-5 * Y2) go to 37
            Y1 = Y2
20004 CONTINUE
   37    continue                                                       00015400
...
C
     ...
     *             X,',')
      ...
      IF(Y .GT. FIVE)THEN
          ...
          GO TO 900
      END IF
      IF (Y .GT. FOUR) GO TO 300                                        00017600
...
C     LGAM(Y) = LGAM(2Y) - LGAM(Y+ 1/2) - Y*LOG(4) + LOG(SQRT(4*PI))    00018600
C
        XM1 = Y - HALF
...
20016 IF (.NOT.(Y .GT. EPS)) GO TO 20019
        XM1 = Y
      ASSIGN 20020 TO NPR001                                            00019600
      GO TO 30001
20020   RES = U - log(Y)
      GO TO 20013
20019   RES = -log(Y)
