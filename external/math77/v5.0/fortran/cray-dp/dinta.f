...
630   CONTINUE
      ...
      IF (.NOT.DID1.OR.XJP.LE.0.1e0) GO TO 20067
               ISTOP(2,1)=18
...
C
670   CONTINUE
      IF(XJ.GT.0.0e0.AND.NSUB.LT.NSUBMX.AND.(INIT.OR.LENDT-IC2.GE.2))THE
     *N
...
920      I=6-NSUB/2
         ...
         IF (NSUB.EQ.0)                                                 
     *      DELTA=MIN(DELTA,ABS(XT(6)-ALOCAL)+2.0e0*ABS(XT(6)-XT(5)))
         DELTA=Z(1)*DELTA
...
C
      ...
      IF (ABS(X2-X1).LE.8.0e0*ABS(X1-BLOCAL)) GO TO 20148
      END IF
...
C
      IF(PART.NE.1)THEN
            I=2
c##         EPSR=EPSR+ERRT(2)
      IF (STEP(1)*STEP(2).LE.0.0e0) GO TO 20157
      END IF
         END(2)=ALOCAL
         I=1
...
C        TA=0.5e0*(X1+X2)
      ...
      IF(.NOT.FAIL)THEN
         ...
         IF (XJP.GT.0.0e0) CALL DINTNS (NSIB)
      END IF
      ABSDIF=0.5e0*DELTA
      ERRT(2)=0.0e0
      RESULT(2)=0.0e0
...
20172    ABSCIS=XT(J)+0.25e0*(XT(I)-XT(J))
         GO TO 1490
1410     CONTINUE
...
2060     CONTINUE
     ...
     *.LE.1.0e0) GO TO 20234
      ELSEIF(ERRI.LT.100.0e0*EPSMIN)THEN
      IF (4.0e0*ERR.GT.EPSMIN) GO TO 20236
                  IF (.NOT.INIT.AND.DISCHK.LE.0) GO TO 2250
                  IF (ENDPTS-2) 2750,2760,2250
      END IF
      IF (REPROD.GE.1.0E-4) GO TO 20234
      IF (K.LT.KAIMT.AND.ERR.GT.100.0e0*EPSMIN) GO TO 20234
      IF (ERRI*REPROD.GT.0.01e0*EPNOIZ) GO TO 20234
...
2120        RNDC=4.0e0*RNDC*(ERR/EPSMIN)
            IF (IPRINT.GT.0.and.100.0e0*err.gt.eps) CALL DINTO (8,WORK)
            EPS=ERR
            ROUNDF=.TRUE.
            DELTA=DELTA+DELTA
            GO TO 2250
...
2350  FATA=FATB
      ...
      IF (PHISUM.GE.16.0e0*PHTSUM) GO TO 20270
      IF (PHISUM.GE.PHTSUM*0.5e0**(KAIMT-K+1)) GO TO 20270
      GO TO 20272
      END IF
            TEND=END(PART)
            KMAX=KMAXF
...
C           1 .LE.  I  .LE. 7
      IF(I.GE.3)THEN
      IF (I.EQ.3.AND.ERRI.GE.EPSMIN.AND.REP.GT.0.3e0) GO TO 20278
      IF (ERR.EQ.0.0e0) GO TO 20278
      IF (ERR.GT.EPSMIN) GO TO 20278
      IF (ERRI.GT.10.0e0*EPSMIN) GO TO 20278
