* File: SRN2GB.for       Subrs used by the
*       David Gay & Linda Kaufman nonlinear LS package.
*       Needed for versions that allow Bounded variables.
*       SRN2GB is called by DNLAFB, DNLAFB, & DRNSGB.
*       The other subrs in this file are needed by SRN2GB.
*
c>> 1994-11-02 SRN2GB Krogh  Changes to use M77CON
c>> 1993-03-10 SRN2GB CLL Moved stmt NN = ... to follow IF (IV1 ...
...
c--& ?D7MLP,?L7MST,?L7ITV,?L7IVM,?R7MDC,?V7SHF
C
...
C  LEAST SQUARES PROBLEMS.  IT IS SIMILAR TO  SRN2G, EXCEPT THAT
...
C
      REAL             SD7TPR, SV2NRM
      EXTERNAL SIVSET, SD7TPR,SD7UPD, SG7ITB,SITSUM,SL7VML, SQ7APL,
     1        SQ7RAD, SR7TVM,SV7CPY, SV7SCP, SV2NRM
c     ------------------------------------------------------------------
...
C SG7ITB... PERFORMS BASIC MINIMIZATION ALGORITHM.
...
C SR7TVM... MULT. VECTOR BY TRANS. OF UPPER TRIANG. MATRIX FROM QR FACT.
...
C
 ...
 20   CALL SG7ITB(B, D, X, IV, LIV, LV, P, P, V, X, X)
...
C
      G1 = IV(G)
      Y1 = G1 + P
      CALL SG7ITB(B, D, V(G1), IV, LIV, LV, P, P, V, X, V(Y1))
...
C
 ...
 80   CALL SQ7APL(ND, N, P, DR, RD, 0)
      CALL SR7TVM(ND, P, V(Y1), V(RD1), DR, RD)
      IV(REGD) = 0
      GO TO 110
C
 90   IV(1) = -2
...
C
 999  RETURN
C  ***  LAST CARD OF SRN2GB FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SG7ITB(B, D, G, IV, LIV, LV, P, PS, V, X, Y)
...
C  ***  DISCUSSION  ***
C
C        SG7ITB IS SIMILAR TO DG7LIT, EXCEPT FOR THE EXTRA PARAMETER B
C     -- SG7ITB ENFORCES THE BOUNDS  B(1,I) .LE. X(I) .LE. B(2,I),
C     I = 1(1)P.
C        SG7ITB PERFORMS NL2SOL-LIKE ITERATIONS FOR A VARIETY OF
...
C     COMPACTLY BY ROWS), AND SG7ITB BUILDS AN APPROXIMATION, S, TO THE
...
C     SG7ITB DECIDES DYNAMICALLY WHETHER OR NOT TO USE S WHEN CHOOSING
...
C     1, OR 2).
C
C        FOR UPDATING S, SG7ITB ASSUMES THAT THE GRADIENT HAS THE FORM
...
C     RHO(I,X)*GRAD(R(I,X0)), WHEN CALLING SG7ITB WITH IV(1) = 2 AND
...
C      DN2GB (AND  DN2FB), ARE NOT REFERENCED BY SG7ITB OR THE
C     SUBROUTINES IT CALLS.
C
C        WHEN SG7ITB IS FIRST CALLED, I.E., WHEN SG7ITB IS CALLED WITH
C     IV(1) = 0 OR 12, V(F), G, AND HC NEED NOT BE INITIALIZED.  TO
C     OBTAIN THESE STARTING VALUES, SG7ITB RETURNS FIRST WITH IV(1) = 1,
...
C     IN COMPUTING A COVARIANCE MATRIX.  IN THIS CASE SG7ITB WILL MAKE
...
C             FUNCTION VALUE AT X, AND CALL SG7ITB AGAIN, HAVING CHANGED
...
C             CAUSE SG7ITB TO IGNORE V(F) AND TRY A SMALLER STEP.  NOTE
...
C             PUTING G, HC, AND Y THE NEXT TIME SG7ITB RETURNS WITH
...
C             THE CALLER SHOULD THEN CALL SG7ITB AGAIN (WITH IV(1) = 2).
...
C             IV(NFGCAL) TO 0, IN WHICH CASE SG7ITB WILL RETURN WITH
C             IV(1) = 15.
C
C  ***  GENERAL  ***
C
...
c     LOGICAL STOPX
c     external STOPX
      REAL             SD7TPR, SRLDST, SV2NRM
      EXTERNAL SA7SST, SD7TPR, SF7DHB, SG7QSB,I7COPY, I7PNVR, I7SHFT,
     1        SITSUM, SL7MSB, SL7SQR, SL7TVM,SL7VML,SPARCK, SQ7RSH,
     2         SRLDST, SS7DMP, SS7IPR, SS7LUP, SS7LVM,      SV2NRM,
     3        SV2AXY,SV7CPY, SV7IPR, SV7SCP, SV7VMP
c     ------------------------------------------------------------------
C SA7SST.... ASSESSES CANDIDATE STEP.
C SD7TPR... RETURNS INNER PRODUCT OF TWO VECTORS.
C SF7DHB... COMPUTE FINITE-DIFFERENCE HESSIAN (FOR INIT. S MATRIX).
C SG7QSB... COMPUTES GOLDFELD-QUANDT-TROTTER STEP (AUGMENTED MODEL).
...
C SL7MSB... COMPUTES LEVENBERG-MARQUARDT STEP (GAUSS-NEWTON MODEL).
...
C SQ7RSH... SHIFTS A QR FACTORIZATION.
C SRLDST... COMPUTES V(RELDX) = RELATIVE STEP SIZE.
C SS7DMP... MULTIPLIES A SYM. MATRIX FORE AND AFT BY A DIAG. MATRIX.
C SS7IPR... APPLIES PERMUTATION TO (LOWER TRIANG. OF) SYM. MATRIX.
C SS7LUP... PERFORMS QUASI-NEWTON UPDATE ON COMPACTLY STORED LOWER TRI-
C             ANGLE OF A SYMMETRIC MATRIX.
C SS7LVM... MULTIPLIES COMPACTLY STORED SYM. MATRIX TIMES VECTOR.
...
C SV7CPY.... COPIES ONE VECTOR TO ANOTHER.
C SV7IPR... APPLIES A PERMUTATION TO A VECTOR.
C SV7SCP... SETS ALL ELEMENTS OF A VECTOR TO A SCALAR.
C SV7VMP... MULTIPLIES (DIVIDES) VECTORS COMPONENTWISE.
C
C  ***  SUBSCRIPTS FOR IV AND V  ***
C
...
C
 ...
 10   CALL SPARCK(1, D, IV, LIV, LV, P, V)
      I = IV(1) - 2
...
C           *** DISALLOW CONVERGENCE IF X(I1) HAS JUST BEEN FREED ***
 ...
 80      IF (I1 .GE. P1) GO TO 90
            ...
            IF (HAVRM)
     1          CALL SQ7RSH(I1, P1, HAVQTR, V(QTR1), V(RMAT1), V(W1))
 90      P1 = P1 - 1
 100     CONTINUE
      IV(PC) = P1
C
...
C
      ...
      DIG1 = IV(DIG)
      CALL SV7VMP(P, V(DIG1), G, D, -1)
      CALL SV7IPR(P, IV(IPI), V(DIG1))
      V(DGNORM) = SV2NRM(P1, V(DIG1))
...
C
 ...
 130  RMAT1 = IV(RMAT)
      LMAT1 = IV(LMAT)
      CALL SL7SQR(P, V(LMAT1), V(RMAT1))
      IPI = IV(IPIVOT)
      IPIV1 = IV(PERM) + P
      CALL I7PNVR(P, IV(IPIV1), IV(IPI))
      CALL SS7IPR(P, IV(IPIV1), V(LMAT1))
      CALL SV2AXY(PP1O2, V(S1), NEGONE, V(LMAT1), V(H1))
C
...
C
 180  CALL SITSUM(D, G, IV, LIV, LV, P, V, X)
 190  K = IV(NITER)
...
C
         ...
         LMAT1 = IV(LMAT)
         WLM1 = W1 + P
         CALL SL7MSB(B, D, G, IV(IERR), IV(IPIV0), IV(IPIV1),
...
C        *** MAKE TEMPORARY PERMUTATION ARRAY ***
         ...
         IF (J .GT. 0) GO TO 290
            J = H1
            RMAT1 = IV(RMAT)
            CALL SL7SQR(P1, V(H1), V(RMAT1))
            GO TO 300
 290     CALL SV7CPY(P*(P+1)/2, V(H1), V(J))
         CALL SS7IPR(P, IV(IPI), V(H1))
 300     IF (IV(MODEL) .EQ. 1) GO TO 310
            LMAT1 = IV(LMAT)
            S1 = IV(S)
            CALL SV7CPY(P*(P+1)/2, V(LMAT1), V(S1))
            CALL SS7IPR(P, IV(IPI), V(LMAT1))
            CALL SV2AXY(P1LEN, V(H1), ONE, V(LMAT1), V(H1))
 310     CALL SV7CPY(P, V(TD1), D)
         CALL SV7IPR(P, IV(IPI), V(TD1))
         CALL SS7DMP(P1, V(H1), V(H1), V(TD1), -1)
         IV(KAGQT) = -1
C
C  ***  COMPUTE ACTUAL GOLDFELD-QUANDT-TROTTER STEP  ***
C
 320  LMAT1 = IV(LMAT)
      CALL SG7QSB(B, D, V(H1), G, IV(IPI), IV(IPIV1), IV(IPIV2),
...
c     RSTRST = 3
 ...
 380  CALL SV7CPY(P, X, V(X01))
      GO TO 410
 390   CALL SV7CPY(P, V(LSTGST), V(STEP1))
       GO TO 410
 400     CALL SV7CPY(P, V(STEP1), V(LSTGST))
         CALL SV2AXY(P, X, ONE, V(STEP1), V(X01))
         V(RELDX) = SRLDST(P, D, X, V(X01))
C
...
C
 ...
 420  CALL SV2AXY(P, V(STEP1), NEGONE, V(X01), X)
      L = IV(IRC) - 4
      STPMOD = IV(MODEL)
...
C
      E = V(PREDUC) - V(FDIF)
      S1 = IV(S)
      CALL SS7LVM(PS, Y, V(S1), V(STEP1))
      STTSST = HALF * SD7TPR(PS, V(STEP1), Y)
...
C
 ...
 480     RMAT1 = IV(RMAT)
         IPIV0 = IV(IPIVOT)
         CALL SV7CPY(P, V(TEMP1), V(STEP1))
         CALL SV7IPR(P, IV(IPIV0), V(TEMP1))
         CALL SL7TVM(P, V(TEMP1), V(RMAT1), V(TEMP1))
         CALL SL7VML(P, V(TEMP1), V(RMAT1), V(TEMP1))
         IPIV1 = IV(PERM) + P
         CALL I7PNVR(P, IV(IPIV1), IV(IPIV0))
         CALL SV7IPR(P, IV(IPIV1), V(TEMP1))
C
...
C
 500  IV(NGCALL) = IV(NGCALL) + 1
      G01 = IV(W)
      CALL SV7CPY(P, V(G01), G)
      GO TO 690
C
C  ***  INITIALIZATIONS -- G0 = G - G0, ETC.  ***
C
 510  G01 = IV(W)
      CALL SV2AXY(P, V(G01), NEGONE, V(G01), G)
      STEP1 = IV(STEP)
      TEMP1 = STEP1 + P
      TEMP2 = IV(X0)
...
C        ***  DO GRADIENT TESTS  ***
C
         IF (SV2NRM(P, V(TEMP1)) .LE. V(DGNORM) * V(TUNER4))  GO TO 530
              IF (SD7TPR(P, G, V(STEP1))
     1                  .GE. V(GTSTEP) * V(TUNER5))  GO TO 540
 530               V(RADFAC) = V(INCFAC)
C
C  ***  COMPUTE Y VECTOR NEEDED FOR UPDATING S  ***
C
 540  CALL SV2AXY(PS, Y, NEGONE, Y, G)
C
...
C
 550  RMAT1 = IV(RMAT)
      IPIV0 = IV(IPIVOT)
      CALL SV7CPY(P, V(G01), V(STEP1))
      I = G01 + PS
      IF (PS .LT. P) CALL SV7SCP(P-PS, V(I), ZERO)
      CALL SV7IPR(P, IV(IPIV0), V(G01))
      CALL SL7TVM(P, V(G01), V(RMAT1), V(G01))
      CALL SL7VML(P, V(G01), V(RMAT1), V(G01))
      IPIV1 = IV(PERM) + P
      CALL I7PNVR(P, IV(IPIV1), IV(IPIV0))
      CALL SV7IPR(P, IV(IPIV1), V(G01))
C
 560  CALL SV2AXY(PS, V(G01), ONE, Y, V(G01))
C
C  ***  UPDATE S  ***
C
...
C
 590  IV(RESTOR) = 0
 600  CALL SF7DHB(B, D, G, I, IV, LIV, LV, P, V, X)
      GO TO (610, 620, 630), I
...
C
 999  RETURN
C
C  ***  LAST LINE OF SG7ITB FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SR7TVM(N, P, Y, D, U, X)
C
...
C
      ...
      DO 10 II = 1, PL
         I = PP1 - II
         T = X(I) * D(I)
         IF (I .GT. 1) T = T + SD7TPR(I-1, U(1,I), X)
         Y(I) = T
 10      CONTINUE
C  ***  LAST LINE OF SR7TVM FOLLOWS  ***
      END
      SUBROUTINE SF7DHB(B, D, G, IRT, IV, LIV, LV, P, V, X)
C
C  ***  COMPUTE FINITE-DIFFERENCE HESSIAN, STORE IT IN V STARTING
C  ***  AT V(IV(FDH)) = V(-IV(H)).  HONOR SIMPLE BOUNDS IN B.
C
C  ***  IF IV(COVREQ) .GE. 0 THEN SF7DHB USES GRADIENT DIFFERENCES,
...
C
     ...
     1        NEWM1, PP1O2, STPI, STPM, STP0
      ...
      REAL             HALF, HLIM, NEGPT5, ONE, TWO, ZERO
C
C  ***  EXTERNAL SUBROUTINES  ***
C
      EXTERNAL SV7CPY, SV7SCP
C
C SV7CPY.... COPY ONE VECTOR TO ANOTHER.
C SV7SCP... COPY SCALAR TO ALL COMPONENTS OF A VECTOR.
C
C  ***  SUBSCRIPTS FOR IV AND V  ***
C
...
C
      PARAMETER (HALF=0.5E+0, HLIM=0.1E+0, NEGPT5=-0.5E+0, ONE=1.E+0,
     1     TWO=2.E+0, ZERO=0.E+0)
C
...
C        *** SUPPLY ZEROS IN CASE B(1,I) = B(2,I) FOR SOME I ***
         CALL SV7SCP(P*(P+1)/2, V(HES), ZERO)
...
C
      GSAVE1 = IV(W) + P
      IF (M .GT. 0) GO TO 20
C        ***  FIRST CALL ON SF7DHB.  SET GSAVE = G, TAKE FIRST STEP  ***
         CALL SV7CPY(P, V(GSAVE1), G)
         IV(SWITCH) = IV(NFGCAL)
         GO TO 80
C
 20   DEL = V(DELTA)
...
C
 ...
 350  V(F) = V(FX)
      ...
      IF (KIND .LT. 0) GO TO 999
         IV(NFGCAL) = IV(SWITCH)
         GSAVE1 = IV(W) + P
         CALL SV7CPY(P, G, V(GSAVE1))
         GO TO 999
C
 999  RETURN
C  ***  LAST LINE OF SF7DHB FOLLOWS  ***
      END
      REAL             FUNCTION SH2RFG(A, B, X, Y, Z)
C
C  ***  DETERMINE X, Y, Z SO  I + (1,Z)**T * (X,Y)  IS A 2X2
C  ***  HOUSEHOLDER REFLECTION SENDING (A,B)**T INTO (C,0)**T,
C  ***  WHERE  C = -SIGN(A)*SQRT(A**2 + B**2)  IS THE VALUE SH2RFG
C  ***  RETURNS.
C
c     ------------------------------------------------------------------
      REAL             A, B, X, Y, Z
...
C
 ...
 10   T = abs(A) + abs(B)
      ...
      SH2RFG = T * C
 999  RETURN
C  ***  LAST LINE OF SH2RFG FOLLOWS  ***
      END
      SUBROUTINE SH2RFA(N, A, B, X, Y, Z)
C
...
c     ------------------------------------------------------------------
      ...
      DO 10 I = 1, N
         ...
         B(I) = B(I) + T*Z
 10      CONTINUE
C  ***  LAST LINE OF SH2RFA FOLLOWS  ***
      END
      SUBROUTINE SG7QSB(B, D, DIHDI, G, IPIV, IPIV1, IPIV2, KA, L, LV,
...
C
      ...
      REAL             B(2,P), D(P), DIHDI(*), G(P), L(*),
     1                 STEP(P,2), TD(P), TG(P), V(LV), W(P), X0(P), X(P)
C     DIMENSION DIHDI(P*(P+1)/2), L(P*(P+1)/2)
C
      REAL             SD7TPR
      EXTERNAL SD7TPR,SG7QTS, SS7BQN, SS7IPR,SV7CPY, SV7IPR,
     1         SV7SCP, SV7VMP
C
C  ***  LOCAL VARIABLES  ***
C
...
C
 20   KINIT = -1
      ...
      IF (P1 .GT. 0) GO TO 30
         NRED = ZERO
         DS0 = ZERO
         CALL SV7SCP(P, STEP, ZERO)
         GO TO 60
C
 30   CALL SV7CPY(P, TD, D)
      CALL SV7IPR(P, IPIV, TD)
      CALL SV7VMP(P, TG, G, D, -1)
      CALL SV7IPR(P, IPIV, TG)
 40   K = KINIT
      KINIT = -1
      V(RADIUS) = RAD - V(DSTNRM)
      CALL SG7QTS(TD, TG, DIHDI, K, L, P1, STEP, V, W)
      P0 = P1
      IF (KA .GE. 0) GO TO 50
         NRED = V(NREDUC)
         DS0 = V(DST0)
C
 50   KA = K
      V(RADIUS) = RAD
      P10 = P1
      CALL SS7BQN(B, D, STEP(1,2), IPIV, IPIV1, IPIV2, KB, L, LV,
     1            NS, P, P1, STEP, TD, TG, V, W, X, X0)
      IF (NS .GT. 0) CALL SS7IPR(P10, IPIV1, DIHDI)
      PRED = PRED + V(PREDUC)
      IF (NS .NE. 0) P0 = 0
      IF (KB .LE. 0) GO TO 40
C
 60   V(DST0) = DS0
      V(NREDUC) = NRED
      V(PREDUC) = PRED
      V(GTSTEP) = SD7TPR(P, G, STEP)
C
C  ***  LAST LINE OF SG7QSB FOLLOWS  ***
      END
      SUBROUTINE SL7MSB(B, D, G, IERR, IPIV, IPIV1, IPIV2, KA, LMAT,
...
C
      ...
      INTEGER IPIV(P), IPIV1(P), IPIV2(P)
      REAL             B(2,P), D(P), G(P), LMAT(*), QTR(P), RMAT(*),
     1                 STEP(P,3), TD(P), TG(P), V(LV), W(P), WLM(*),
     2                 X0(P), X(P)
C     DIMENSION LMAT(P*(P+1)/2), RMAT(P*(P+1)/2), WLM(P*(P+5)/2 + 4)
C
      REAL             SD7TPR
      EXTERNAL SD7MLP, SD7TPR, SL7MST, SL7TVM, SQ7RSH, SS7BQN,
     1        SV2AXY,SV7CPY, SV7IPR, SV7SCP, SV7VMP
C
C  ***  LOCAL VARIABLES  ***
C
...
C     *** USE STEP(1,3) AS TEMP. COPY OF QTR ***
      ...
      IF (P1 .GT. 0) GO TO 30
         ...
         GO TO 90
C
 30   CALL SV7VMP(P, TG, G, D, -1)
      CALL SV7IPR(P, IPIV, TG)
      P10 = P1
 40   K = KINIT
      KINIT = -1
      V(RADIUS) = RAD - V(DSTNRM)
      CALL SV7VMP(P1, TG, TG, TD, 1)
      DO 50 I = 1, P1
 50      IPIV1(I) = I
      K0 = max(0, K)
      CALL SL7MST(TD, TG, IERR, IPIV1, K, P1, STEP(1,3), RMAT, STEP,
     1            V, WLM)
      CALL SV7VMP(P1, TG, TG, TD, -1)
      P0 = P1
      IF (KA .GE. 0) GO TO 60
...
C
 60   KA = K
      V(RADIUS) = RAD
      L = P1 + 5
      IF (K .LE. K0) CALL SD7MLP(P1, LMAT, TD, RMAT, -1)
      IF (K .GT. K0) CALL SD7MLP(P1, LMAT, TD, WLM(L), -1)
      CALL SS7BQN(B, D, STEP(1,2), IPIV, IPIV1, IPIV2, KB, LMAT,
...
C
      ...
      DO 70 K = P11, P10
         J = L - K
         I = IPIV2(J)
         IF (I .LT. J) CALL SQ7RSH(I, J, .TRUE., QTR, RMAT, W)
 70      CONTINUE
C
...
C
      CALL SV7VMP(P10, W, STEP(1,2), TD, -1)
      CALL SL7TVM(P10, W, LMAT, W)
      CALL SV2AXY(P10, STEP(1,3), ONE, W, QTR)
      GO TO 40
C
 90   V(DST0) = DS0
      V(NREDUC) = NRED
      ...
      V(GTSTEP) = SD7TPR(P, G, STEP)
C
C  ***  LAST LINE OF SL7MSB FOLLOWS  ***
      END
      SUBROUTINE SS7BQN(B, D, DST, IPIV, IPIV1, IPIV2, KB, L, LV, NS,
...
C
      ...
      INTEGER IPIV(P), IPIV1(P), IPIV2(P)
      REAL             B(2,P), D(P), DST(P), L(*),
     1                 STEP(P), TD(P), TG(P), V(LV), W(P), X(P),
     2                 X0(P)
C     DIMENSION L(P*(P+1)/2)
C
      REAL             SD7TPR, SR7MDC, SV2NRM
      EXTERNAL SD7TPR, I7SHFT, SL7ITV, SL7IVM, SQ7RSH, SR7MDC, SV2NRM,
     1        SV2AXY,SV7CPY, SV7IPR, SV7SCP, SV7SHF
C
C  ***  LOCAL VARIABLES  ***
C
...
C
      ...
      DST1 = ZERO
      IF (MEPS2 .LE. ZERO) MEPS2 = TWO * SR7MDC(3)
      P0 = P1
      NS = 0
      DO 10 I = 1, P
         IPIV1(I) = I
 ...
 20      W(I) = -STEP(I) * TD(I)
      ...
      V(PREDUC) = ZERO
      GTS = -V(GTSTEP)
      IF (KB .LT. 0) CALL SV7SCP(P, DST, ZERO)
      KB = 1
C
...
* DNSGB (8 of 10)
 ...
 60      CONTINUE
C
      IF (P .GT. P1) CALL SV7CPY(P-P1, STEP(P1+1), DST(P1+1))
      CALL SV2AXY(P1, STEP, -T, W, DST)
      DST0 = DST1
      DST1 = SV2NRM(P, STEP)
C
...
C
 80   V(DSTNRM) = DST1
      CALL SV7CPY(P1, DST, STEP)
      T1 = ONE - T
      DO 90 I = 1, P1
 90      TG(I) = T1 * TG(I)
      IF (ALPHA .GT. ZERO) CALL SV2AXY(P1, TG, T*ALPHA, W, TG)
      V(PREDUC) = V(PREDUC) + T*((ONE - HALF*T)*GTS +
     1                        HALF*ALPHA*T*SD7TPR(P1,W,W))
      IF (K .EQ. 0) GO TO 110
C
...
C
      ...
      IF (J .EQ. P1) GO TO 100
         NS = NS + 1
         IPIV2(P1) = J
         CALL SQ7RSH(J, P1, .FALSE., TG, L, W)
         CALL I7SHFT(P1, J, IPIV)
         CALL I7SHFT(P1, J, IPIV1)
         CALL SV7SHF(P1, J, TG)
         CALL SV7SHF(P1, J, DST)
 100  IF (K .LT. 0) IPIV(P1) = -IPIV(P1)
      P1 = P1M1
      IF (P1 .LE. 0) GO TO 110
      CALL SL7IVM(P1, W, L, TG)
      GTS = SD7TPR(P1, W, W)
      CALL SL7ITV(P1, W, L, W)
      GO TO 30
C
C     ***  UNSCALE STEP  ***
C
...
C
 ...
 140     CONTINUE
C
 150  CALL SV2AXY(P, X, ONE, STEP, X0)
      IF (NS .GT. 0) CALL SV7IPR(P0, IPIV1, TD)
C  ***  LAST LINE OF SS7BQN FOLLOWS  ***
      END
      SUBROUTINE SQ7RSH(K, P, HAVQTR, QTR, R, W)
C
...
C
      ...
      DO 50 J = K, PM1
         JM1 = J - 1
         JP1 = J + 1
         IF (JM1 .GT. 0) CALL SV7CPY(JM1, R(K1+1), R(J1+2))
         J1 = J1 + JP1
         K1 = K1 + J
         A = R(J1)
         B = R(J1+1)
         IF (B .NE. ZERO) GO TO 10
              ...
              Z = ZERO
              GO TO 40
 10      R(K1) = SH2RFG(A, B, X, Y, Z)
         IF (J .EQ. PM1) GO TO 30
         I1 = J1
         DO 20 I = JP1, PM1
              I1 = I1 + I
              CALL SH2RFA(1, R(I1), R(I1+1), X, Y, Z)
 20           CONTINUE
 30      IF (HAVQTR) CALL SH2RFA(1, QTR(J), QTR(JP1), X, Y, Z)
 40      T = X * WJ
         W(J) = WJ + T
         WJ = T * Z
 50      CONTINUE
      W(P) = WJ
      CALL SV7CPY(P, R(K1+1), W)
 999  RETURN
      END
      SUBROUTINE SV7VMP(N, X, Y, Z, K)
C
...
C
 ...
 30      X(I) = Y(I) * Z(I)
 999  RETURN
C  ***  LAST CARD OF SV7VMP FOLLOWS  ***
      END
      SUBROUTINE SV7IPR(N, IP, X)
C
...
C
 ...
 20      X(K) = X(J)
         ...
         X(K) = T
 30      CONTINUE
C  ***  LAST LINE OF SV7IPR FOLLOWS  ***
      END
      SUBROUTINE SV7SHF(N, K, X)
C
...
C
 ...
 80         K = J
            ...
            IF (J .GT. I) GO TO 10
 90      CONTINUE
C  ***  LAST LINE OF SS7IPR FOLLOWS  ***
      END
      SUBROUTINE SD7MLP(N, X, Y, Z, K)
C
C ***  SET X = DIAG(Y)**K * Z
...
C
 ...
 50      CONTINUE
 999  RETURN
C  ***  LAST CARD OF SD7MLP FOLLOWS  ***
      END
      SUBROUTINE SS7DMP(N, X, Y, Z, K)
C
