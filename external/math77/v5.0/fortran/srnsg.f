      SUBROUTINE  SRNSG(A, ALF, C, DA, IN, IV, L, L1, LA, LIV, LV,
     1                  N, NDA, P, V, Y)
c>> 1994-10-20 SRNSG Krogh  Changes to use M77CON
c>> 1990-06-12 SRNSG CLL @ JPL
c>> 1990-04-23 CLL (Recent revision by DMG)
*** from netlib, Mon Apr 23 20:37:24 EDT 1990 ***
c>> 1990-02-20 CLL @ JPL
C   (Calls SN2RDP with 4, rather than 6, arguments. -- CLL)
C
...
C T(1)...T(N),  SRNSG ATTEMPTS TO COMPUTE A LEAST SQUARES FIT
...
C     IN (IN)  WHEN  SRNSG IS CALLED WITH IV(1) = 2 OR -2, THEN FOR
...
C                  SRNSG SHOULD RETURN FOR THEM.
C     IV (I/O) INTEGER PARAMETER AND SCRATCH VECTOR.   SRNSG RETURNS
...
C                 (AFTER A RETURN WITH IV(1) = 2),  SRNSG RETURNS
...
C
      REAL             SD7TPR, SL7SVX, SL7SVN, SR7MDC
      EXTERNAL SC7VFN,SIVSET, SD7TPR,SITSUM, SL7ITV,SL7SRT, SL7SVX,
     1         SL7SVN, SN2CVP, SN2LRD, SN2RDP,  SRN2G, SQ7APL,SQ7RAD,
     2        SQ7RFH, SR7MDC, SS7CPR,SV2AXY,SV7CPY,SV7PRM, SV7SCL,
     3         SV7SCP
C
c--S replaces "?": ?RNSG ,?C7VFN,?D7TPR,?ITSUM,?IVSET,?L7ITV,?L7SRT
...
C SL7SRT.... COMPUTES (PARTIAL) CHOLESKY FACTORIZATION.
...
C SN2LRD... COMPUTES COVARIANCE AND REGRESSION DIAGNOSTICS.
C SN2RDP... PRINTS REGRESSION DIAGNOSTICS.
C  SRN2G... UNDERLYING NONLINEAR LEAST-SQUARES SOLVER.
...
C
 ...
 10   IF (IV(PERM) .LE. AR) IV(PERM) = AR + 1
      CALL  SRN2G(V, V, IV, LIV, LV, N, N, N1, NML, P, V, V, V, ALF)
...
C  ***  DIAGNOSTICS), HAVE  SRN2G COMPUTE ONLY THE PART CORRESP.
...
C
 30   CALL  SRN2G(V(D1), V(DR1L), IV, LIV, LV, NML, N, N1, NML, P,
     1            V(R1L), V(RD1), V, ALF)
      IF (abs(IV(RESTOR)-2) .EQ. 1 .AND. L .GT. 0)
     1        CALL SV7CPY(L, C, V(CSAVE1))
      IV1 = IV(1)
      IF (IV1-2) 40, 150, 230
C
...
C
 40   IV(IV1SAV) = IV(1)
      IV(1) = abs(IV1)
      IF (IV(RESTOR) .EQ. 2 .AND. L .GT. 0) CALL SV7CPY(L, V(CSAVE1), C)
      GO TO 999
C
...
C
      ...
      K = P * (P + 1) / 2
      I1 = IV(LMAT)
      CALL SV7CPY(K, V(I), V(I1))
      I = I + K
      CALL SV7SCP(PP*(PP+1)/2 - K, V(I), ZERO)
      IV(NFCOV) = IV(NFCOV) + 1
...
C
 240  I = DR1 + N*P
      DO 250 I1 = 1, L
         CALL SV7SCL(N, V(I), NEGONE, A(1,I1))
         I = I + N
 250     CONTINUE
      PP = L + P
      HSAVE = IV(H)
      ...
      CALL SV7SCP(LH, V(K), ZERO)
      CALL SQ7RAD(N, N, PP, V, .FALSE., V(K), V(DR1), V)
      IV(MODE) = I + 8
      IV(1) = 2
      IV(NGCALL) = IV(NGCALL) + 1
      IV(NGCOV) = IV(NGCOV) + 1
...
C
 270  PP1 = P + 1
      ...
      DO 290 I = PP1, PP
         DRI1 = DR1
         DO 280 I1 = 1, I
            V(LI) = V(LI) + SD7TPR(N, V(DRI), V(DRI1))
            LI = LI + 1
            DRI1 = DRI1 + N
 280        CONTINUE
         DRI = DRI + N
 290     CONTINUE
      CALL SL7SRT(PP1, PP, V(K), V(K), I)
      IF (I .NE. 0) GO TO 310
 300  TEMP1 = K + LH
      T = SL7SVN(PP, V(K), V(TEMP1), V(TEMP1))
      IF (T .LE. ZERO) GO TO 310
      T = T / SL7SVX(PP, V(K), V(TEMP1), V(TEMP1))
      V(RCOND) = T
      IF (T .GT. SR7MDC(4)) GO TO 320
 310     IV(REGD) = -1
         IV(COVMAT) = -1
         IV(FDH) = -1
         GO TO 340
 320  IV(H) = TEMP1
      ...
      IF (IV(MODE) - PP .LT. 2) GO TO 330
         I = IV(H)
         CALL SV7SCP(LH, V(I), ZERO)
 330  CALL SN2LRD(V(DR1), IV, V(K), LH, LIV, LV, N, N, PP, V(R1),
     1            V(RD1), V)
 340  CALL SC7VFN(IV, V(K), LH, LIV, LV, N, PP, V)
      IV(H) = HSAVE
C
 350  IF (IV(REGD) .EQ. 1) IV(REGD) = RD1
 360  IF (IV(1) .LE. 11) CALL SS7CPR(C, IV, L, LIV)
      IF (IV(1) .GT. 6) GO TO 999
         CALL SN2CVP(IV, LIV, LV, P+L, V)
