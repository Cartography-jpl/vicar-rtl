...
c     ------------------------------------------------------------------
      ...
      integer IPTOL, ITERC, K, LDA, M, MAX, MEQ, MEQL, MP, MSAT
...
C
     ...
     *   IPLAST .ne. 0 .or. IPTOL .ne. 0) then
         print'(/6x,a/6x,a,i4,a,i4,a,i4,a,g11.3/6x,a,g11.3,a,g11.3)',
     *   'Beginning subroutine SMLC02, called from SMLC01:',
     *   'N =',N,',  M =',M,',  MEQ  =',MEQ,',  ACC =',ACC,
     *   'RELACC =',RELACC,',  TOL = ',TOL
      endif
C
...
c
*     print'(/a,i6,a,i6)','SMLC05.. Debug.. On entry, INFO = ',INFO,
*    * ',  ITERC =',ITERC
...
*         print'(a,2g23.15/(19x,2g23.15))',
*    *   ' SMLC05 798.. X()=',(X(I),I=1,N)
*         print'(a,g23.15)',' .. F =',F
*         print'(a,2g23.15/(19x,2g23.15))',
*    *   ' ............ G()=',(G(I),I=1,N)
...
c                         debug printing.  CLL 3/26/92
      DIFF = 1.0e0
      IF (IPTOL .ne. 0) THEN
          print'(/a,g13.5)',' New value of TOL = ',TOL
          DOPRNT = .true.
...
C
*     print'(/a/a,3g13.5/a,3g13.5/a,g13.5/a,2g13.5,i5)',
*    *   ' SMLC05 849..',
*    *   '          ACC**2, SSQKT, DDOTG=',ACC**2, SSQKT, DDOTG,
*    *   '             F, FPREV, FPREV-F=',F, FPREV, FPREV-F,
*    *   '                          DIFF=',DIFF,
*    *   '             TOL, RELACC, NACT=',TOL, RELACC, NACT
...
C
      IF (F .GE. FPREV) THEN
          ...
          END IF
*         print'(/a,2g13.5)',
*    *       ' SMLC05 869.. DDOTG, EPS*F=',DDOTG, EPS*F
...
c        print'(a/a,g13.5,a,g13.5,a,i5)',
c    *   ' SMLC05.. Debug.. return to reduce TOL.',
c    *   ' ........ TOL=',TOL,',   RELACC=',RELACC,',  INFO=',INFO
         GOTO 70
      endif
c
...
*            print'(/a,2g13.5/a,2g13.5)',
*    *         ' SMLC05 890..      DDOTG, SUM=',DDOTG,SUM,
*    *         ' RELACC*SUM, DDOTG+RELACC*SUM=',
*    *          RELACC*SUM,DDOTG+RELACC*SUM
...
C     Initialization.
c
*     print'(/a,g13.5/a,3g13.5/(22x,3g13.5))',
*    *   ' SMLC09 1290.. STEPCB=',STEPCB,
*    *   '               .. D()=',(D(I),I=1,N)
      RELINT=0.9E0
      ICOUNT=0
      RATIO=-1.0E0
   ...
   10 CONTINUE
      ...
      DGOPT=abs(DDOTG)
      NOBACK = .false.
*     print'(a,2g13.5/a,g13.5)',
*    *   ' SMLC09 1318.. RATIO,STPMIN=',RATIO,STPMIN,
*    *   '                     DDOTG =',DDOTG
C
...
*        print*,' SMLC09 1355.. Ending SMLC09 on NOBACK.'
   ...
   30    X(I)=XS(I)+STEP*D(I)
      ...
      ICOUNT=ICOUNT+1
*         print'(/a,i3/a,2g23.15)',
*    *       ' SMLC09 1321..  ICOUNT=',ICOUNT,
*    *   '   .. STEP,STPOPT =',STEP,STPOPT
*         print'(a,2g23.15/(19x,2g23.15))',
*    *   '            .. X()=',(X(I),I=1,N)
*         print'(a,g23.15)','             .. F =',F
*         print'(a,2g23.15/(19x,2g23.15))',
*    *   '            .. G()=',(G(I),I=1,N)
   ...
   40 continue
      SIZE = SIZE * RELACC
*     print'(/a,g13.5/a,g13.5/a,g13.5/a,g13.5)',
*    *   ' SMLC09 1335..       FOPT - F =', FOPT - F,
*    *   '            ..          DGMID =', DGMID,
*    *   '            ..           SIZE =', SIZE,
*    *   '        .. DGOPT - abs(DGMID) =', DGOPT - abs(DGMID)
      IF (F .LE. FOPT) THEN
...
*           print*,' SMLC09 1381.. Setting NOBACK on new test.'
         ...
         if(KEEP) then
              STPOPT=STEP
*             print'(/a,g13.5)',' SMLC09 1384.. New STPOPT=',STPOPT
              FOPT=F
              DO 50 I=1,N
   50         GOPT(I)=G(I)
              DGOPT=abs(DGMID)
...
C
*     print'(/a,i23,g23.15/a,2g23.15)',
*    *   ' SMLC09 1366.. ICOUNT, STEP=',ICOUNT,STEP,
*    *   '             STPLOW, STPHGH=',STPLOW, STPHGH
      IF (STPHGH .EQ. 0.0E0) THEN
          IF (STEP .EQ. STEPCB) GOTO 70
          TEMP=10.0E0
          IF (DGMID .GT. 0.9E0*DDOTG) TEMP=DDOTG/(DDOTG-DGMID)
          STEP=min(TEMP*STEP,STEPCB)
*         print'(a,g23.15)',' SMLC09 1374.. To 20 with STEP=',STEP
          GOTO 20
...
*         print'(a,g23.15)',' SMLC09 1384.. To 20 with STEP=',STEP
          GOTO 20
      ELSE
          STEP=0.1E0*STEP
          if (STEP .GE. STPMIN) then
*         print'(a,g23.15)',' SMLC09 1389.. To 20 with STEP=',STEP
             GOTO 20
          endif
      END IF
C
...
c     ------------------------------------------------------------------
   ...
   10 RESKT(I)=G(I)
*     print'(/a,2g23.15/(19x,2g23.15))',
*    *   ' SMLC16 1993.. G()=',(G(I),I=1,N)
*     print'(a,3g23.15/(8x,3g23.15))',
*    *   ' .. U()=',(U(I),I=1,3)
*     print'(a,2g23.15/(8x,2g23.15))',
*    *   ' .. Z()=',(Z(I),I=1,16)
*     print'(a,2g23.15/(9x,2g23.15))',
*    *   ' .. A(,)=',((A(I,J),J=1,4),I=1,3)
      IF (NACT .GT. 0) THEN
          ICASE=0
   20     DO 50 KK=1,NACT
...
*       print'(a,3g13.5)',' SMLC16 1991.. Z(),RESKT(),TEMP=',
*    *      Z(IZ),RESKT(I),TEMP
   30     IZ=IZ+N
          TEMP=TEMP*U(K)
*       print'(a,2g13.5)',' SMLC16 1995.. U(K),TEMP=',U(K),TEMP
          IF (ICASE .EQ. 0) PAR(K)=0.0E0
          IF (K .LE. MEQL .OR. PAR(K)+TEMP .LT. 0.0E0) THEN
              PAR(K)=PAR(K)+TEMP
*           print'(a,2g13.5)',' SMLC16 1999.. TEMP,PAR(K)=',
*    *         TEMP,PAR(K)
          ELSE
              TEMP=-PAR(K)
              PAR(K)=0.0E0
...
*          print'(a,2g13.5)',
*    *       ' SMLC16 2018.. TEMP,RESKT(JM)=',
*    *       TEMP,RESKT(JM)
                  ELSE
                      RESKT(JM-N)=RESKT(JM-N)-TEMP
*          print'(a,2g13.5)',
*    *       ' SMLC16 2022.. TEMP,RESKT(JM-N)=',
*    *       TEMP,RESKT(JM-N)
                  END IF
              END IF
          END IF
...
*         print'(/a,4g12.4/(23x,4g12.4))',
*    *       ' SMLC16 2014.. RESKT()=',(RESKT(I),I=1,N)
          DO 60 I=1,N
   60     SSQKT=SSQKT+RESKT(I)**2
*        print'(a,g13.5)',' SMLC16 2018.. SSQKT=',SSQKT
C
...
c
      ...
      logical HAVEG, MODE1, NOPP, LNOPP
...
c                             Set up base locations in W()
 ...
 1000    format ('0Gradient #=', I3, '  F=', 1pe24.17, '  DXN=',
     1      1pe15.8 / '  I', 11x, 'X', 14X, 'DX', 10x, 'G', 10x, 'ERR',
     2      9X, 'H', 9X, 'dG/dV')
         KDEBUG = KDEBUG + 1
      end if
      do 40 I = 1, LXL
   ...
   20    if ((W(LPP+I) .lt. 0.E0) .or. (mod(KOUNT, 10) .eq. 0)) then
            if ((XI + HB .le. XU(I)) .and. (XI - HB .ge. XL(I))) then
               MODE1 = .false.
               H = sign(min(100.E0 * HB, XU(I) - XI, XI - XL(I)), D)
               go to 30
            end if
         end if
         MODE1 = .true.
...
c                                 Adjust H to keep XNEW within bounds.
     ...
     1             W(LPP+I) = -W(LPP+I)
         ...
         if (KDEBUG .ne. 0) then
            print '(I3, 1PE22.14, E10.2, E12.4, E12.4, E11.3, E10.2)',
...
c     ------------------------------------------------------------------
      ...
      logical ALL
c     ------------------------------------------------------------------
      print'(/1x,i6,a,i6,a,g13.5/1x,a,g13.5,a,g13.5)',
     * ITERC,' iterations. ',NFVALS,' function evals.    F = ',F,
     * '  TOL =',TOL,'   Norm of RESKT =',ENRMKT
      print'(a,5g13.5/(8x,5g13.5))','     X =',(X(I),I=1,N)
      ...
      if (IPMORE .gt. 0) then
         if(ALL) then
            ...
            if (NACT .eq. N) then
               ...
               print'(a,5g13.5/(8x,5g13.5))',' RESKT =',(RESKT(I),I=1,N)
            end if
         end if
         ...
         if (NACT .eq. 0) then
            ...
            if(ALL)
     *      print'(a,5g13.5/(8x,5g13.5))','   PAR =',(PAR(I),I=1,NACT)
         end if
      end if
      return
      end
