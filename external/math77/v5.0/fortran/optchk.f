...
c ERRSEV Flags error/diagnostic level.  = 0 none; 07 is used to get
c        diagnostics only; and ERRBAD otherwise.
...
c        ERRSEV is set from NERBAD(INTCHK(0)+4).
c 
...
c Declarations for error messages.
     ...
     1   LTXTEE, LTXEND
      ...
      parameter (METEXT =53)
      parameter (MEIMAT =58)
      integer MACT(16), ERRBAD, ERRSEV, NERBAD(0:7)
      equivalence (MACT(2), ERRSEV)
c
...
cAE Space for ETEXT.$E
      ...
      parameter (LTXTAA=  1,LTXTAB=  9,LTXTAC=169,LTXTAD=202,LTXTAE=230)
      character MTXTAA(1) * (247)
c
      parameter (LTXTEE = LTXTAE-2)
      parameter (LTXEND = 247)
C
      data MTXTAA/'OPTCHK$B"Option #" is negated if option needs attenti
     *on.$N"First Loc." is negated if user did not set value.$NSpace ava
     *il. = $I; all options must have first loc. > $I$EOption #$HFirst L
     *oc.$HLast Loc.$EFrom subprogram/argument: $BSpace for ETEXT.$E'/
c                      1 2 3      4       5  6       7      8       9
...
c++ CODE for VERSION = F is active
         MTXTAA(1)(LTXTEE:LTXEND)=ETEXT(1:)
c++ CODE for VERSION = C is inactive
c%%       strcpy(&mtxtaa[0][LTXTEE-1], etext);
c++ END
         call MESS(MACT, MTXTAA, INTCHK(1))
         if (ERRSEV .gt. 10) INTCHK(1) = -LAST
