...
c     -----------------------------------------------------------------
      ...
      REAL             FN, FOUR, ONE
      REAL             SUM, T, T1, TP, TPI, TS, TS1, TWO
      REAL             WI, WR, ZERO
...
c transform being computed.
      DO 10 K = 1, NDD
         ...
         IF (MODE(K:K) .eq. 'A' .or. MODE(K:K) .eq. 'a') THEN
            NDIV = NDIV * N
         ELSEIF (MODE(K:K) .eq. 'S' .or. MODE(K:K) .eq. 's') THEN
            NDIV = 2 * NDIV
         else
            CERROR = MODE(K:K)
            CALL IERM1 ('STCST',2,2,
     1         'MODE(K:K) = '// CERROR, 'for K =', K, '.')
            MS = -2
            return
         END IF
         ...
         if(ITCSK .eq. 0) then
            CERROR = TCS(K:K)
            CALL IERM1 ('STCST', 3, 2,
     1         'TCS(K:K) = '// CERROR, 'for K =', K, '.')
            return
         endif
         NF(1) = 1
         IF (ITCSK .GE. 2) THEN
...
c
c                               Fatal error, default is to stop in IERM1
  200 CALL IERM1 ('STCST', 4, 2, 'Require 0 .le. max(M(K)) .le. 21',
     1  'M', MM, '.')
      MS = -2
      RETURN
c
