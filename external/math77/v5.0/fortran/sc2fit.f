...
c     ------------------------------------------------------------------
      ...
      real             XI(NXY), YI(NXY), SDI(NXY), B(NB), W(NW, 5)
...
20012 IF(SDI(1) .lt. ZERO)THEN
      ...
      ELSE
         IERR1 = 1100
         call ERMSG('SC2FIT',IERR1,0,'Require SD(1) .ne. Zero','.')     00018000
         return
      END IF
C
...
20019 IF ((N20018-I).LT.0) GO TO 20018
     ...
     *    'I',I,',')
          call SERV1('X(I-1)',XI(I-1),',')                              00019200
          call SERV1('X(I)',XI(I),'.')
...
c
      GO TO 20026
20027 call SBACC(W, NW, NBAND, IRNOW, KSIZE, ISEG, JTPREV, IERR2)
...
C
20044 call SBSOL(1,W, NW, NBAND, IRNOW, JTPREV, W(1,NBAND1),            
     *           NPARAM, RNORM, IERR3)
...
c
30003 call SC2BAS(XI(JPOINT), 0, ISEG,NEWSEG,B,NB,P)
      IF( USEWT1)THEN
         WT = WT1
      ELSE
         sdijp = sdi(jpoint)
...
30002    call ERMSG('SC2FIT',IERR1,0,                                   
