...
c>> 1994-10-20 DRFT Krogh  Changes to use M77CON
c>> 1990-01-23 DRFT CLL Removed extraneous label 80.
...
c
      ...
      elseif( MODE .eq. 'S' .or. MODE .eq. 's') then
         ...
         call ERMSG('DRFT',2,2,'Bad MODE.  MODE = '//MODE,'.')
         MS = -2
         return
      endif
      NDA = ND
...
c                               Fatal error, default is to stop in IERM1
         CALL IERM1 ('DRFT', 1, 2, 'BAD ND', 'ND', ND, '.')
         MS = -2
         RETURN
      END IF
      MA = 0
...
c
c               Fatal error, default is to stop in IERM1
  400 call ERMSG('DRFT',2,2,'M() specified improperly','.')
      MS = -2
      RETURN
c
      END
