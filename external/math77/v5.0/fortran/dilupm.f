...
C>> 1995-12-13 DILUPM  Krogh  Fixed bad Y value when getting derivative.
C>> 1995-12-04 DILUPM  Krogh  Set so DILUP errors keep stop/print level.
C>> 1995-11-10 DILUPM  Krogh  Fixed so char. data at col. 72 is not ' '.
C>> 1994-11-11 DILUPM  Krogh   Declared all vars.
c>> 1994-10-20 DILUPM  Krogh  Changes to use M77CON
c>> 1994-09-12 DILUPM  Krogh  Added CHGTYP code.
c>> 1993-04-28 DILUPM  Krogh  Additions for Conversion to C.
...
c ID4    = 4 defined in data.  To avoid passing literal to DILUP.
...
c   On an error with IIFLG .lt. 0, in DILUP this is set to -10 * stop
...
c
     ...
     1 INTCHK(0:20), IOP2N, IOPTI(3), IPRAG, IRAG, IX, IYBAS, IYDSAV,
     2 IYSAV, IYT, J, K, KC(MAXDIM), KPTD(0:MAXDIM), L, LERTMP, LI, LT,
...
cAM Previous error from DILUP occurred in dimension $I.$E
c   $
cAN LUP(1:$M):$B
c   $
cAO NDEG(1:$M):$B
...
cAR X((1:$M):$B
     ...
     * LTXTAP=  1,LTXTAQ=  1,LTXTAR=  1)
      ...
      data MTXTAA/'DILUPM$BEstimated error = $F, Requested error = $F.$E
     ...
     *tive order in [0, $I], but it is = $I.$EPrevious error from DILUP$
...
c                  IOPTI(2) = 0, if variable order and need more points.
  ...
  216       continue
            ID = ID +1
            KGOC = abs(KGOC)
            call DILUP(X(IDIM), EOPT(IYDSAV), NT, XT(IXT), EOPT(IYT),
...
c Error Processing
  ...
  410 IOPT(1) = IIFLG
      MACT(3) = -IIFLG
      call DMESS (MACT, MTXTAA, INTCHK(1), ERRDAT)
      if (IIFLG .lt. -2) then
         MACT(7) = NDIMI
         MACT(10) = NDIMI
         ...
         MACT(9) = MEFVEC
         call DMESS (MACT(6), MTXTAF, NDEG, X)
         MACT(9) = MEIVEC
         MACT(7) = LT
         MACT(10) = LT
