...
C  ***  LAST LINE OF  SRN2G FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SG7LIT(D, G, IV, LIV, LV, P, PS, V, X, Y)
...
C             PUTING G, HC, AND Y THE NEXT TIME SG7LIT RETURNS WITH
...
C
 110  CALL SITSUM(D, G, IV, LIV, LV, P, V, X)
 120  K = IV(NITER)
...
C
         ...
         IPIV1 = IV(IPIVOT)
         CALL SL7MST(D, G, IV(IERR), IV(IPIV1), IV(KALM), P, V(QTR1),
...
C
 470  CALL SV2AXY(PS, Y, NEGONE, Y, G)
C
...
C
 490  CALL SV2AXY(PS, V(G01), ONE, Y, V(G01))
C
C  ***  UPDATE S  ***
C
...
C
 520  IV(RESTOR) = 0
 530  CALL SF7HES(D, G, I, IV, LIV, LV, P, V, X)
      GO TO (540, 550, 580), I
...
C
 590  IV(FDH) = -1
      ...
      T = T / SL7SVX(P, V(LMAT1), V(STEP1), V(STEP1))
...
C  ***  LAST LINE OF SC7VFN FOLLOWS  ***
      END
      SUBROUTINE SF7HES(D, G, IRT, IV, LIV, LV, P, V, X)
C
...
C
 130  I = abs(IV(COVREQ))
      IF (I .LE. 1) WRITE(PU,'(1x/a/a/1x)')
     *        ' COVARIANCE = VARFAC * H**-1 * (J**T * J) * H**-1',
     *        ' WHERE H = F.D. HESSIAN'
      IF (I .EQ. 2) WRITE(PU,'(1x/a/1x)')
     * ' COVARIANCE = VARFAC * H**-1,'//
     * ' WHERE H = FINITE-DIFFERENCE HESSIAN'
      IF (I .GT. 2) WRITE(PU,'(1x/a/1x)')
 ...
 170     CONTINUE
 180  FORMAT(' ROW',I3,2X,5g12.3/(9X,5g12.3))
C
 999  RETURN
...
C  ***  LAST CARD OF SN2RDP FOLLOWS  ***
      END
      SUBROUTINE SO7PRD(L, LS, P, S, W, Y, Z)
C
