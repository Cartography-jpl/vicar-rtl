...
C DERM1   Displays an error message.                                    00003100
...
C
      ...
      DOUBLE PRECISION RK, RM, RN, S0, S1, S2, S3, S4, SGN, SI, SJ, SK  00004100
...
C     PI2 = PI/2
C
      DATA LN2 /0.693147180559945309417232121458176568075D0/            00005100
...
C
      ...
      IF(ABS(K) .GT. 1.0d0)THEN
         CALL DERM1 ('DELEFI',1,0,'ABS(K) .GT. 1.0','K',K,'.')          00007200
         IERR = 2
         RETURN
      END IF
      IERR = 0
...
C
         ...
         TS = SN*CN
20012 CONTINUE
            AN = (PN*AN - TS)/QN                                        00010200
            R  = K2*HN/QN
            S2 = S2 + R*AN
            HN = PN*R
            S0 = S1
            S1 = S1 + HN*AN
      IF (ABS(TR) .LT. ABS(AN)) GO TO 20013
      IF (ABS(S1) .LE. ABS(S0)) GO TO 20013
            PN = QN + 1.0d0
            QN = PN + 1.0d0
            TR = SS*TR                                                  00011200
            TS = SS*TS
