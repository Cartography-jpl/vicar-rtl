...
c
*     print'(/a,i6,a,i6)','DMLC05.. Debug.. On entry, INFO = ',INFO,
*    * ',  ITERC =',ITERC
...
C
      IF (F .GE. FPREV) THEN
          ...
          END IF
*         print'(/a,2g13.5)',
*    *       ' DMLC05 869.. DDOTG, EPS*F=',DDOTG, EPS*F
...
c    *   ' DMLC05.. Debug.. return to reduce TOL.',
...
*    *         ' DMLC05 890..      DDOTG, SUM=',DDOTG,SUM,
...
*    *   ' DMLC09 1318.. RATIO,STPMIN=',RATIO,STPMIN,
*    *   '                     DDOTG =',DDOTG
C
...
*           print*,' DMLC09 1381.. Setting NOBACK on new test.'
         ...
         if(KEEP) then
              STPOPT=STEP
*             print'(/a,g13.5)',' DMLC09 1384.. New STPOPT=',STPOPT
              FOPT=F
              DO 50 I=1,N
   50         GOPT(I)=G(I)
              DGOPT=abs(DGMID)
...
*    *   ' DMLC09 1366.. ICOUNT, STEP=',ICOUNT,STEP,
...
*         print'(a,g23.15)',' DMLC09 1374.. To 20 with STEP=',STEP
          GOTO 20
...
*         print'(a,g23.15)',' DMLC09 1384.. To 20 with STEP=',STEP
          GOTO 20
      ELSE
          STEP=0.1D0*STEP
          if (STEP .GE. STPMIN) then
*         print'(a,g23.15)',' DMLC09 1389.. To 20 with STEP=',STEP
             GOTO 20
          endif
      END IF
C
...
*       print'(a,3g13.5)',' DMLC16 1991.. Z(),RESKT(),TEMP=',
*    *      Z(IZ),RESKT(I),TEMP
   30     IZ=IZ+N
          TEMP=TEMP*U(K)
*       print'(a,2g13.5)',' DMLC16 1995.. U(K),TEMP=',U(K),TEMP
          IF (ICASE .EQ. 0) PAR(K)=0.0D0
          IF (K .LE. MEQL .OR. PAR(K)+TEMP .LT. 0.0D0) THEN
              PAR(K)=PAR(K)+TEMP
*           print'(a,2g13.5)',' DMLC16 1999.. TEMP,PAR(K)=',
*    *         TEMP,PAR(K)
          ELSE
              TEMP=-PAR(K)
              PAR(K)=0.0D0
...
*          print'(a,2g13.5)',
*    *       ' DMLC16 2018.. TEMP,RESKT(JM)=',
*    *       TEMP,RESKT(JM)
                  ELSE
                      RESKT(JM-N)=RESKT(JM-N)-TEMP
*          print'(a,2g13.5)',
*    *       ' DMLC16 2022.. TEMP,RESKT(JM-N)=',
*    *       TEMP,RESKT(JM-N)
                  END IF
              END IF
          END IF
...
*    *       ' DMLC16 2014.. RESKT()=',(RESKT(I),I=1,N)
          DO 60 I=1,N
   60     SSQKT=SSQKT+RESKT(I)**2
*        print'(a,g13.5)',' DMLC16 2018.. SSQKT=',SSQKT
C
 ...
 1000    format ('0Gradient #=', I3, '  F=', 1pe24.17, '  DXN=',
   ...
   20    if ((W(LPP+I) .lt. 0.D0) .or. (mod(KOUNT, 10) .eq. 0)) then
            if ((XI + HB .le. XU(I)) .and. (XI - HB .ge. XL(I))) then
               MODE1 = .false.
               H = sign(min(100.D0 * HB, XU(I) - XI, XI - XL(I)), D)
               go to 30
            end if
         end if
         MODE1 = .true.
