      subroutine SSFIT(XI,YI,SDI,NXY, KORDER, NC, TKNOTS, BCOEF,        
     *                  SIGFAC, IERR1, LDW, W)
c>> 1994-10-19 SSFIT Krogh  Changes to use M77CON
c>> 1994-01-31 SSFIT CLL  Add test for SDI(i) .le. 0 when SDI(1) > 0.
c>> 1993-01-12 CLL  Using SSBASD in place of _SBAS.
...
c     ------------------------------------------------------------------
c--S replaces "?": ?SFIT, ?BACC, ?BSOL, ?SBASD, ?ERV1
...
c     ------------------------------------------------------------------
      ...
      parameter(KMAX=20)
      real             BCOEF(NC), DOF, ONE, P(KMAX), RNORM
      real             SDI(NXY), sdijp, SIGFAC
      real             TKNOTS(NC+KORDER), W(LDW, KORDER+1), WT, WT1
      real             XI(NXY), YI(NXY), ZERO
...
20003 IF (.NOT.(KORDER .gt. KMAX)) GO TO 20006
         IERR1 = 150
         call IERM1('SSFIT',IERR1,0,'Require KORDER .le. KMAX.',        
     *   'KORDER',KORDER,',')
...
20015 IF (.NOT.(LDW .lt. NC+2)) GO TO 20021
        IERR1 = 300
        call IERM1('SSFIT',IERR1,0,'Require LDW .ge. NC+2',             
     *             'LDW',LDW,',')
        call IERV1('NC',NC,'.')
...
20021 IF(SDI(1) .lt. ZERO)THEN
      ...
      ELSE
         IERR1 = 1100
         call ERMSG('SSFIT',IERR1,0,'Require SD(1) .ne. Zero','.')
         return
      END IF
C
...
20042 IF ((N20041-JPT ).LT.0) GO TO 20041
      ...
      IF (.NOT.( K .gt. LDW )) GO TO 20046
            call SBACC(W, LDW, NBAND, IRNOW, KSIZE, ISEG, JTPREV, IERR2)
...
c
      GO TO 20040
20041 call SBACC(W, LDW, NBAND, IRNOW, KSIZE, ISEG, JTPREV, IERR2)
...
C
20061 CONTINUE
20060 call SBSOL(1,W, LDW, NBAND, IRNOW, JTPREV, BCOEF,                 
     *           NC, RNORM, IERR3)
      IF (.NOT.(IERR3 .ne. 0)) GO TO 20063
               IERR1 = 1000 + IERR2
               call ERMSG('SSFIT',IERR1,0,                              
...
c
30003 call SSBASD(KORDER, LEFT, TKNOTS, XI(JPOINT), 0, P)
      IF( USEWT1)THEN
         WT = WT1
      ELSE
...
30002    call ERMSG('SSFIT',IERR1,0,                                    00035800
