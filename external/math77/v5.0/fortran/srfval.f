...
C
C     CHECK ARGUMENT RANGES AND CALL SRFVLX TO DO THE COMPUTATION       00001100
...
C
     ...
     *     x,',')
      ...
      IF(MIN(X+Y,MIN(X+Z,Y+Z)) .LT. LOLIM)THEN
         IERR = 2
         CALL SERM1 ('SRFVAL',2,0,'One of X+Y, X+Z or Y+Z < LOLIM','X', 00007400
     *     x,',')
         CALL SERV1 ('Y',Y,',')
         CALL SERV1 ('Z',Z,',')
...
C ----------------------------------------------------------------------
      ...
      PARAMETER (C1 = 10.0e0 / 24.0e0)                                  00013600
