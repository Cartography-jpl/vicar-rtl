...
C
C--S replaces "?": ?CI, ?CIN, ?CPVAL, ?ERM1
C
      INTEGER ERRLEV
      PARAMETER (ERRLEV=0)
      REAL             C(23), SCIN, SCPVAL, F(13), G(13), GAMMA,X, Z,ZW
      EXTERNAL SCPVAL
...
C        Provide a value in case the error message processor returns.
      ...
      ELSE IF (X.LT.16.0) THEN
         Z = X/16.0
         ZW = Z*Z
         SCI = LOG(X) - ZW*SCPVAL(C,20,ZW) + GAMMA
      ELSE
         Z = 16.0/X
         ZW = Z*Z
         SCI = Z*(SCPVAL(F,10,ZW)*SIN(X) - Z*SCPVAL(G,10,ZW)*COS(X))
      END IF
      RETURN
C
...
C
      ...
      IF (ABS(X).LT.16.0) THEN
         Z = X/16.0
         ZW = Z*Z
         SCIN = ZW*SCPVAL(C,20,ZW)
      ELSE
         Z = 16.0/X
         ZW = Z*Z
         SCIN = LOG(ABS(X)) + GAMMA -
