...
c     facilitate replacement of use of this package by Fortran intrinsics
...
c     RANDOM_SEED of Fortran 90.  This relates to setting or fetching
c     the seed.
...
c     ------------------------------------------------------------------
      integer I, J, JUMP, K, KSEED(2), KSIZE, MODE, MODE1, N
      real ASP, BSP, USP(N)
      real             AFACSP, BFACSP, C2SP, MDIVSP, QSP, RSP, SCALSP
      real             X1SP, XCURSP, XTSTSP
      double precision ADP, BDP, UDP(N)
      double precision AFACDP, BFACDP, C2DP, DIFF
      double precision MDIVDP, QDP, RDP, SCALDP
      double precision TEST(0:3,0:1), X1DP, XCURDP, XTSTDP

      parameter( AFACDP = 612662.0D0, MDIVDP = 68719476503.0D0)
      parameter( X1DP = 123456789.0D0, SCALDP = 100000.0D0)
      parameter( BFACDP = 112165.0d0, C2DP = 243273.0d0)

      parameter( AFACSP = 612662.0e0, MDIVSP = 68719476503.0e0)
      parameter( X1SP = 123456789.0e0, SCALSP = 100000.0e0)
      parameter( BFACSP = 112165.0e0, C2SP = 243273.0e0)

      save MODE, XCURDP, XCURSP
      data TEST / 24997965550.0d0, 68719476502.0d0,
     *            68718863841.0d0, 36962132774.0d0,
     *            43721510953.0d0,           1.0d0,
     *                 612662.0d0, 31757343729.0d0/
      data XCURDP / X1DP /, XCURSP / X1SP /
      data MODE / 1 /
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      entry SRANUA(USP, N)
c
  300 go to (310, 320, 330, 340), MODE
  310   JUMP = 3
        go to 800
c                                         Mode 2.
  320   continue
...
c     ------------------------------------------------------------------
      entry DRANUA(UDP, N)
c
  400 go to (410, 420, 430, 440), MODE
  410   JUMP = 4
        go to 800
c                                         Mode 2.
  420   continue
...
c                                         Mode 4.
  ...
  450 continue
      return
c     ------------------------------------------------------------------
      entry SRANUS(USP, N, ASP, BSP)
c
  500 go to (510, 520, 530, 540), MODE
  510   JUMP = 5
        go to 800
c                                         Mode 2.
  520   continue
...
c                                         Mode 4.
  ...
  550 continue
      return
c     ------------------------------------------------------------------
      entry DRANUS(UDP, N, ADP, BDP)
c
  600 go to (610, 620, 630, 640), MODE
  610   JUMP = 6
        go to 800
c                                         Mode 2.
  620   continue
...
c                            Following line ends MODE loop.
  880   continue
c
c        Here the computations were unsuccessful for all values of MODE.
...
c
     ...
     *   '.')
  885 continue
      go to (100, 200, 300, 400, 500, 600), JUMP
      end
