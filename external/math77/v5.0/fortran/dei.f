...
c     ------------------------------------------------------------------
      ...
      DOUBLE PRECISION D1MACH,DCSEVL,DEI
...
C
      ...
      DATA FAC / 0.999D0 /
C
      DATA MSG1(1) / 'X SO SMALL DE1 OVERFLOWS' /
      DATA MSG1(2) / 'X SO LARGE DEI OVERFLOWS' /
C
      DATA MSG2(1) / 'X = ZERO, DE1(0) = + INFINITY' /
      DATA MSG2(2) / 'X = ZERO, DEI(0) = - INFINITY' /
C
      DATA MSG3(1) / 'X SO LARGE DE1 UNDERFLOWS' /
      DATA MSG3(2) / 'X SO SMALL DEI UNDERFLOWS' /
C
      DATA NAME / 'DE1','DEI' /
C
...
C
      ...
      IF (X .LT. XMIN) THEN
        CALL DERM1( NAME(MODE),1,0,MSG1(MODE),'X',XIN,',' )
        CALL DERV1('Limit',sign(XMIN,XIN),'.')
        DE1 = -BIGNUM
      ELSE
...
C
 70   IF (X.GT.XMAX) GO TO 80
      ...
      GO TO 90
C
  80  CALL DERM1( NAME(MODE),3,0,MSG3(MODE),'X',XIN,',' )
      CALL DERV1('Limit',sign(XMAX,XIN),'.')
      DE1 = ZERO
      GO TO 90
C
  90  CONTINUE
