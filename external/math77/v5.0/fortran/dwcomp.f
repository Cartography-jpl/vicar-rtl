...
c     entry      DWASIN (N16,U16,Y16)         DWSQRT,DWPWRI,DWPRO1,DWCHN
c     entry      DWACOS (N16,U16,Y16)         DWSQRT,DWPWRI,DWPRO1,DWCHN
c     entry      DWATN2 (N9,Y9,X9,A9)      DWSUM,DWDIF,DWPRO,DWQUO,DWCHN
c
c     subroutine DWSUM  (NDERIV,U,V,W)        None
c     entry      DWDIF  (N2,U2,V2,W2)         None
c     entry      DWSQRT (N15,U15,Y15)         DWCHN
c     entry      DWEXP  (N4,U4,Y4)            DWCHN
c     entry      DWSIN  (N7,U7,Y7)            DWCHN
c     entry      DWCOS  (N7,U7,Y7)            DWCHN
c     entry      DWTAN  (N7,U7,Y7)            DWCHN
c     entry      DWSINH (N7,U7,Y7)            DWCHN
c     entry      DWCOSH (N7,U7,Y7)            DWCHN
c     entry      DWTANH (N7,U7,Y7)            DWCHN, DWPRO, DWQUO
c     entry      DWSET  (N10,UVAL,UDER,U10)   None
c     entry      DWSUM1 (N11,C11,U11,Y11)     None
c     entry      DWDIF1 (N12,C12,U12,Y12)     None
c     entry      DWPRO1 (N13,C13,U13,Y13)     None
c     entry      DWQUO1 (N14,C14,U14,Y14)     DWQUO
c
c     subroutine DWLOG  (NDERIV,U,Y)          DWCHN
c     entry      DWPWRI (NDERIV,MPWR,U,Y)     DWCHN
c
c     subroutine DWCHN  (NDERIV,U,F)          DWPRO
c
c     subroutine DWPRO  (NDERIV,U,V,W)        DPASCL
c     entry      DWQUO  (NDERIV,U,V,W)        DPASCL
c
...
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      double precision TWO, ZERO, ONE, BIG, XX, YY
      parameter(TWO=2.0D0, ZERO=0.0D0, ONE=1.0D0)
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C
...
c
   66 continue
      ...
      if(ACOSIN) call DWPRO1(NS,-ONE,S2,S2)
c
...
C
      ...
      double precision             U11(*),Y11(*)
      double precision             U12(*),Y12(*)
      double precision             U13(*),Y13(*)
      double precision             U14(*),Y14(*)
      double precision             U15(*),Y15(*)
      double precision  C11, C12, C13, C14, UVAL, UDER, ZERO
      double precision  FAC, SCALE, SQRTU, HALF, ONE, SH, CH, SN, CS
...
C
      ...
      logical          SHORT
      double precision ZERO, ONE, D, FAC, UINV
...
C     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      double precision    ZERO, TWO, THREE, FOUR, SIX, ONE, FAC, TEMP
      parameter(ZERO = 0.0D0, TWO = 2.0D0, THREE = 3.0D0)
      parameter(FOUR = 4.0D0, SIX = 6.0D0, ONE = 1.0D0)
      logical FIRST
      save FIRST, C
      data FIRST /.TRUE. /
C     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
