...
c
c--S replaces "?": ?IVA,?IVAA,?IVABU,?IVACO,?IVACR,?IVAEV,?IVAHC,?IVAG
...
c++  End
 2220 KORD1I = 2
      ...
      if (KIS .ge. 1000) then
         if (LDIS .eq. 2) then
            HH = HH*min(min(TP, TP**2), (CP25 * EXR/EMAX)**.333333333E0)
            if (KQMAXS .le. 3) then
               LDIS = 0
               EREP = abs(EREP)
               TSPECS(2) = HH
...
cAB Error in IOPT() specifications: IOPT =$E
      ...
      data MTXTAA/'SIVAOP$BError in IOPT() specifications: IOPT =$E'/
c                      1   2  3   4       5  6      7
