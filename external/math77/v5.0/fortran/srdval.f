...
C     INCOMPLETE ELLIPTIC INTEGRALS, ACM TRANSACTIONS ON MATHEMA-       00002100
...
C     CHECK: RD(X,Y,Z) + RD(Y,Z,X) + RD(Z,X,Y)                          00003100
...
C ----------------------------------------------------------------------
      ...
      PARAMETER (C3 = 9.0e0 / 22.0e0)                                   00006100
...
C          EXPENSE OF ROBUSTNESS.                                       00008200
C ----------------------------------------------------------------------
C
      RD = 0.0
...
C
      ...
      POWER4 = 1.0e0
C
20010 CONTINUE                                                          00012500
         MU = (XN + YN + 3.0e0 * ZN) / 5.0e0
         XNDEV = (MU - XN) / MU
         YNDEV = (MU - YN) / MU
         ZNDEV = (MU - ZN) / MU
         EPSLON = MAX(ABS(XNDEV),MAX(ABS(YNDEV),ABS(ZNDEV)))
      IF (EPSLON .LT. ERRTOL) GO TO 20011
         XNROOT = SQRT(XN)
         YNROOT = SQRT(YN)
         ZNROOT = SQRT(ZN)
         LAMDA = XNROOT * (YNROOT + ZNROOT) + YNROOT * ZNROOT           00013500
