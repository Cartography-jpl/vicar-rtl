...
C
C--D replaces "?": ?CI, ?CIN, ?CPVAL, ?ERM1
C
      INTEGER ERRLEV
      PARAMETER (ERRLEV=0)
      DOUBLE PRECISION C(23), DCIN, DCPVAL, F(13), G(13), GAMMA,X, Z,ZW
      EXTERNAL DCPVAL
...
C        Provide a value in case the error message processor returns.
      ...
      ELSE IF (X.LT.16.0) THEN
         Z = X/16.0
         ZW = Z*Z
         DCI = LOG(X) - ZW*DCPVAL(C,20,ZW) + GAMMA
      ELSE
         Z = 16.0/X
         ZW = Z*Z
         DCI = Z*(DCPVAL(F,10,ZW)*SIN(X) - Z*DCPVAL(G,10,ZW)*COS(X))
      END IF
      RETURN
C
...
C
      ...
      IF (ABS(X).LT.16.0) THEN
         Z = X/16.0
         ZW = Z*Z
         DCIN = ZW*DCPVAL(C,20,ZW)
      ELSE
         Z = 16.0/X
         ZW = Z*Z
         DCIN = LOG(ABS(X)) + GAMMA -
