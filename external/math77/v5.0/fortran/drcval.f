...
C     INVERSE HYPERBOLIC FUNCTIONS ARE EXPRESSED IN TERMS OF RC         00002100
...
C     LAWRENCE LIVERMORE NATIONAL LABORATORY, LIVERMORE, CA 94550.      00003100
...
C
      DOUBLE PRECISION X, Y, RC                                         00006100
      INTEGER IERR
C
...
C                                                                       00007100
      ...
      DATA LOLIM /-1.0/
C
C     ERRTOL IS SET TO THE DESIRED ERROR TOLERANCE.                     00008100
...
C                                                                       00010200
      ...
      IF(X .GT. UPLIM .OR. YN .GT. UPLIM)THEN
       ...
       CALL DERM1 ('DRCVAL',3,0,'X > UPLIM or ABS(Y) > UPLIM','X',X,',')
         CALL DERV1 ('Y',Y,',')
         CALL DERV1 ('UPLIM',UPLIM,'.')                                 00012200
         IERR = 3
         RETURN
      END IF
      ...
      IF(X + YN .GT. UPLIM)THEN
         ...
         RETURN
      END IF
      IF(Y.LT.Y2236L .AND. X.GT.0.0d0 .AND. X.LT.XLU225)THEN            00014200
         RC = 0.0d0
         CALL DERM1 ('DRCVAL',4,0,                                      
     *      'Y < -2.236/SQRT(LOLIM) AND 0 < X < (LOLIM*UPLIM)**2/25',   
     *      'X',X,',')
         CALL DERV1 ('Y',Y,',')
         CALL DERV1 ('LOLIM',LOLIM,',')
         CALL DERV1 ('UPLIM',UPLIM,'.')
...
C        TRANSFORM TO POSITIVE Y
      ...
      END IF
C
20016 CONTINUE                                                          00016400
         MU = (XN + YN + YN) / 3.0d0
         SN = (YN + MU) / MU - 2.0d0
      IF (ABS(SN) .LT. ERRTOL) GO TO 20017
...
20017 CONTINUE
C
      RC = W*(1.0d0 + SN*SN*(C1 + SN*(C2 + SN*(C3 + SN*C4)))) / SQRT(MU)00017300
      RETURN
C
      END        
