...
c     entry INSRTX (DATAOP,N,LIST,LIST1)
c     5 February 1990.
c
...
c
c     If INSRTX is invoked, DATAOP is a 4-argument subroutine, invoked
c     by CALL DATAOP (8,I,J,IORDER).  IORDER should be negative if the
...
c     end while
c
      integer COMPAR, LIST(*), LIST1, N
      external COMPAR, DATAOP
c
c     *****     Local Variables     ************************************
c
...
c
c     Set up assigned GO TO's depending on which entry is used.
c
      assign 60 to jump1
      assign 320 to jump2
      go to 20
...
c     *****     Entry INSRTX     ***************************************
c
      entry INSRTX (DATAOP,N,LIST,LIST1)
c
      assign 70 to jump1
      assign 330 to jump2
c
...
c
60    ncomp=compar(i,ip1)
      go to 80
70    call dataop(8,i,ip1,ncomp)
80    if (ncomp) 100,90,120
...
c     Indicate end of chain
160   list(i)=0
C     Compute log base 2 of IABS(direct).
170   direct=iabs(direct)+1
180   m=1
      phase=-phase  
      do 190 i=1,mxsort
...
310   heads(m)=0
      nchain=0
      go to jump2, (320,330)
320   ncomp=compar(j1,j2)
      go to 340 
330   call dataop(8,j1,j2,ncomp)
340   if (ncomp.gt.0) then
c
...
380      nchain=-1
         i2=j2
         j2=list(i2)
         if (j2.ne.0) go to jump2, (320,330)
         list(I2)=j1
      else
c
...
420      nchain=1
         i1=j1
         j1=list(i1)
         if (j1.ne.0) go to jump2, (320,330)
         list(i1)=j2
      end if
