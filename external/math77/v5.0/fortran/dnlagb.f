      SUBROUTINE   DNLAGB(N, P, X, B, DCALCR, DCALCJ, IV, LIV, LV, V)
c>> 1994-10-20 DNLAGB Krogh  Changes to use M77CON
c>> 1990-07-02 DNLAGB CLL @ JPL
...
C
      ...
      INTEGER N, P, LIV, LV
      INTEGER IV(LIV)
      DOUBLE PRECISION X(P), B(2,P), V(LV)
C
C  ***  DISCUSSION  ***
C
...
C     IF NOT, THEN DNLAGB RETURNS WITH IV(1) = 15.  When DNLAGB returns,
...
C     THEN DNLAGB RETURNS WITH IV(1) = 16.  WHEN DNLAGB RETURNS, THE
...
C     SETS IV(1) TO 15 OR 16.  CALLING DNLAGB WITH IV(1) = 0 CAUSES ALL
C     DEFAULT VALUES TO BE USED FOR THE INPUT COMPONENTS OF IV AND V.
C        IF YOU FIRST CALL DIVSET, THEN SET IV(1) TO 13 AND CALL DNLAGB,
...
C     ARE LARGE ENOUGH.  IF SO, THEN DNLAGB RETURNS WITH IV(1) = 14.
C     WHEN CALLED WITH IV(1) = 14, DNLAGB ASSUMES THAT STORAGE HAS
...
c--D replaces "?": ?NLAGB, ?IVSET, ?RN2GB, ?CALCR, ?CALCJ
...
C
 50   CALL DCALCJ(N, P, X, IV(NFGCAL), V(DR1))
      IF (IV(NFGCAL) .EQ. 0) IV(TOOBIG) = 1
      GO TO 20
C
 999  RETURN
C
C  ***  LAST CARD OF   DNLAGB FOLLOWS  ***
      END
