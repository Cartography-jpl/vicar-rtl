...
C     VALUE IS COMPUTED BY USING A PRELIMINARY TRANSFORMATION           00002100
...
C     1-16.
C     AUTHORS: B. C. CARLSON AND ELAINE M. NOTIS, AMES LABORATORY-      00003100
...
C        = Z * W,  A = P * P * (X+Y+Z+W),  B = P * (P+X) * (P+Y),       00004100
...
C
      DOUBLE PRECISION X, Y, Z, R, RJ                                   00006100
      INTEGER IERR
C
...
C     DRFVLX IS INSIDE SRFVAL.                                          00007100
C
...
C
      ...
      DOUBLE PRECISION ZN,ZNDEV,ZNROOT,Z1,Z2,Z3,Z4                      00008100
      ...
      PARAMETER (Z4 = 9.0d0 / 22.0d0)                                   00009100
...
C
C ----------------------------------------------------------------------
C     WARNING. CHANGES IN THE PROGRAM MAY IMPROVE SPEED AT THE          00011100
...
C
     ...
     *      'ONE OF X, Y, or Z is negative, or R is zero','X',X,',')
         CALL DERV1 ('Y',Y,',')
         CALL DERV1 ('Z',Z,',')
         CALL DERV1 ('R',R,'.')                                         00012300
         RETURN
      END IF
     ...
     *      'ONE OF X+Y, X+Z, Y+Z or ABS(R) < LOLIM','X',X,',')
         CALL DERV1 ('Y',Y,',')
         CALL DERV1 ('Z',Z,',')
         CALL DERV1 ('R',R,',')
...
20014 CONTINUE
         ...
         XNDEV = (MU - XN) / MU                                         00018500
         YNDEV = (MU - YN) / MU
         ZNDEV = (MU - ZN) / MU
         RNDEV = (MU - RN) / MU
         EPSLON = MAX(ABS(XNDEV),MAX(ABS(YNDEV),MAX(ABS(ZNDEV),         
     *                ABS(RNDEV))))
      IF (EPSLON .LT. ERRTOL) GO TO 20015
         XNROOT = SQRT(XN)
         YNROOT = SQRT(YN)
         ZNROOT = SQRT(ZN)
         LAMDA = XNROOT * (YNROOT + ZNROOT) + YNROOT * ZNROOT
         ALFA = RN * (XNROOT + YNROOT + ZNROOT) + XNROOT*YNROOT*ZNROOT  00019600
         ALFA = ALFA * ALFA
...
20016 CONTINUE
      ...
      IF (ABS(SN) .LT. ETOLRC) GO TO 20017                              00020600
