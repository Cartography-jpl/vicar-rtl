* File: SIVSET.for      28 subrs used by all main subrs of the
*       David Gay & Linda Kaufman nonlinear LS package.
c>> 1994-11-02 SIVSET Krogh  Changes to use M77CON
c>> 1992-04-27 SIVSET CLL Removed unreferenced stmt labels.
...
c>> 1990-02-16 CLL JPL
      SUBROUTINE SIVSET(ALG, IV, LIV, LV, V)
C
...
C
      INTEGER  I1MACH
      EXTERNAL SV7DFL, I1MACH
c--S replaces "?": ?IVSET,?V7DFL,?R7MDC,?L7SVN,?D7TPR,?V2AXY,?V2NRM
...
C SV7DFL.... PROVIDES DEFAULT VALUES TO V.
C
      INTEGER ALG1, MIV, MV
...
C
 999  RETURN
C  ***  LAST CARD OF SIVSET FOLLOWS  ***
      END
      SUBROUTINE SV7DFL(ALG, LV, V)
C
...
C
 999  RETURN
C  ***  LAST CARD OF SV7DFL FOLLOWS  ***
      END
      REAL             FUNCTION SR7MDC(K)
C
...
C
 10   SR7MDC = ETA
      GO TO 999
C
 20   SR7MDC = sqrt(256.E+0*ETA)/16.E+0
      GO TO 999
C
 30   SR7MDC = MACHEP
      GO TO 999
C
 40   SR7MDC = sqrt(MACHEP)
      GO TO 999
C
 50   SR7MDC = sqrt(BIG/256.E+0)*16.E+0
      GO TO 999
C
 60   SR7MDC = BIG
C
 999  RETURN
C  ***  LAST CARD OF SR7MDC FOLLOWS  ***
      END
      REAL             FUNCTION SL7SVN(P, L, X, Y)
C
...
C
C        SV2NRM - FUNCTION, RETURNS THE 2-NORM OF A VECTOR.
C
C  ***  REFERENCES  ***
C
...
C
      ...
      PM1 = P - 1
C
C  ***  FIRST CHECK WHETHER TO RETURN SL7SVN = 0 AND INITIALIZE X  ***
C
      II = 0
      J0 = P*PM1/2
      JJ = J0 + P
...
C       ***  UPDATE PARTIAL SUMS  ***
         IF (JM1 .GT. 0) CALL SV2AXY(JM1, X, XPLUS, L(J0+1), X)
 50      CONTINUE
C
C  ***  NORMALIZE X  ***
C
 ...
 70      X(I) = T*X(I)
C
C  ***  SOLVE L*Y = X AND RETURN SL7SVN = 1/TWONORM(Y)  ***
C
      DO 100 J = 1, P
         JM1 = J - 1
         J0 = J*JM1/2
         JJ = J0 + J
         T = ZERO
         IF (JM1 .GT. 0) T = SD7TPR(JM1, L(J0+1), Y)
         Y(J) = (X(J) - T) / L(JJ)
 100     CONTINUE
C
      SL7SVN = ONE/SV2NRM(P, Y)
      GO TO 999
C
 110  SL7SVN = ZERO
 999  RETURN
C  ***  LAST CARD OF SL7SVN FOLLOWS  ***
      END
      SUBROUTINE SQ7APL(NN, N, P, J, R, IERR)
C     *****PARAMETERS.
      INTEGER NN, N, P, IERR
      REAL             J(NN,P), R(N)
C
...
C     *****SUBROUTINES AND FUNCTIONS CALLED.
C
C     SD7TPR - FUNCTION, RETURNS THE INNER PRODUCT OF VECTORS
C
C     *****REFERENCES.
...
C     CALL ON SV2AXY SUBSTITUTED FOR DO LOOP, FALL 1983.
C
C     *****GENERAL.
C
C     THIS SUBROUTINE WAS WRITTEN IN CONNECTION WITH RESEARCH
...
C
      DO 20 L = 1, K
         NL1 = N - L + 1
         CALL SV2AXY(NL1, R(L), -SD7TPR(NL1,J(L,L),R(L)), J(L,L), R(L))
 20   CONTINUE
C
 999  RETURN
C  ***  LAST LINE OF SQ7APL FOLLOWS  ***
      END
      REAL             FUNCTION SV2NRM(P, X)
C
...
C
 30   SCALE = abs(X(I))
      IF (I .LT. P) GO TO 40
         ...
         GO TO 999
 40   T = ONE
      IF (SQTETA .EQ. ZERO) SQTETA = SR7MDC(2)
C
...
C
      SV2NRM = SCALE * sqrt(T)
 999  RETURN
C  ***  LAST LINE OF SV2NRM FOLLOWS  ***
      END
      REAL             FUNCTION SD7TPR(P, X, Y)
C
...
C
      ...
      EXTERNAL SR7MDC
C
C  ***  SR7MDC(2) RETURNS A MACHINE-DEPENDENT CONSTANT, SQTETA, WHICH
...
C
 ...
 10      SD7TPR = SD7TPR + X(I)*Y(I)
 20   CONTINUE
C
 999  RETURN
C  ***  LAST LINE OF SD7TPR FOLLOWS  ***
      END
      SUBROUTINE SD7UPD(D, DR, IV, LIV, LV, N, ND, NN, N2, P, V)
C
...
C
 999  RETURN
C  ***  LAST CARD OF SD7UPD FOLLOWS  ***
      END
      SUBROUTINE SQ7RAD(N, NN, P, QTR, QTRSET, RMAT, W, Y)
C
...
C
      LOGICAL QTRSET
      INTEGER N, NN, P
      REAL             QTR(P), RMAT(*), W(NN,P), Y(N)
C     DIMENSION RMAT(P*(P+1)/2)
      REAL             SD7TPR, SR7MDC, SV2NRM
      EXTERNAL SD7TPR, SR7MDC,SV2AXY, SV7SCL, SV2NRM
C
C  ***  LOCAL VARIABLES  ***
C
...
C
 ...
 60         T = -T
            ...
            S = S * sqrt(-WI)
 70         W(K,I) = WI
            CALL SV7SCL(NK, W(K,I), ONE/S, W(K,I))
            RMAT(II) = -T
            IF (.NOT. QTRSET) GO TO 80
            CALL SV2AXY(NK, Y(K), -SD7TPR(NK,Y(K),W(K,I)), W(K,I), Y(K))
            QTR(I) = Y(K)
 80         IF (IP1 .GT. P) GO TO 999
            DO 90 J = IP1, P
               CALL SV2AXY(NK, W(K,J), -SD7TPR(NK,W(K,J),W(K,I)),
     1                    W(K,I), W(K,J))
               RMAT(IJ) = W(K,J)
               IJ = IJ + J
 90            CONTINUE
...
C
 ...
 120     IF (RI .LT. ZERO) T = -T
         ...
         CALL SV7SCL(NK, W(K,I), ONE/RI, W(K,I))
         IF (.NOT. QTRSET) GO TO 130
            QRI = QTR(I)
            T = S * ( QRI  +  SD7TPR(NK, Y(K), W(K,I)) )
            QTR(I) = QRI + T
 130     IF (IP1 .GT. P) GO TO 999
         IF (QTRSET) CALL SV2AXY(NK, Y(K), T, W(K,I), Y(K))
         DO 140 J = IP1, P
            RI = RMAT(IJ)
            T = S * ( RI  +  SD7TPR(NK, W(K,J), W(K,I)) )
            CALL SV2AXY(NK, W(K,J), T, W(K,I), W(K,J))
            RMAT(IJ) = RI + T
            IJ = IJ + J
 140        CONTINUE
         GO TO 180
...
C
 999  RETURN
C  ***  LAST LINE OF SQ7RAD FOLLOWS  ***
      END
      SUBROUTINE SPARCK(ALG, D, IV, LIV, LV, N, V)
C
...
C
      REAL             SR7MDC
      EXTERNAL SIVSET, SR7MDC,SV7CPY,SV7DFL
C SIVSET  -- SUPPLIES DEFAULT VALUES TO BOTH IV AND V.
C SR7MDC -- RETURNS MACHINE-DEPENDENT CONSTANTS.
C SV7CPY  -- COPIES ONE VECTOR TO ANOTHER.
C SV7DFL  -- SUPPLIES DEFAULT PARAMETER VALUES TO V ALONE.
C
C  ***  LOCAL VARIABLES  ***
C
...
C
 ...
 10      FORMAT(/' THE FIRST PARAMETER TO SIVSET SHOULD BE',I3,
     1          ' RATHER THAN',I3)
         IV(1) = 67
         GO TO 999
 20   IF (ALG .LT. 1 .OR. ALG .GT. 4) GO TO 340
      ...
      ALG1 = MOD(ALG-1,2) + 1
      IF (IV(1) .EQ. 0) CALL SIVSET(ALG, IV, LIV, LV, V)
      IV1 = IV(1)
...
C
 ...
 220     FORMAT(/' NONDEFAULT VALUES....'/' INIT',A1,'..... IV(25) =',
     1          I3)
...
C
      IV(DTYPE0) = IV(DTYPE)
      PARSV1 = IV(PARSAV)
      CALL SV7CPY(IV(NVDFLT), V(PARSV1), V(EPSLON))
      GO TO 999
C
 300  IV(1) = 15
      IF (PU .EQ. 0) GO TO 999
...
C
 999  RETURN
C  ***  LAST LINE OF SPARCK FOLLOWS  ***
      END
      SUBROUTINE SS7LVM(P, Y, S, X)
C
...
C
 999  RETURN
C  ***  LAST CARD OF SS7LVM FOLLOWS  ***
      END
      SUBROUTINE SS7LUP(A, COSMIN, P, SIZE, STEP, U, W, WCHMTD, WSCALE,
     1                  Y)
C
...
C
      REAL             SD7TPR, SV2NRM
      EXTERNAL SD7TPR, SS7LVM, SV2NRM
      PARAMETER (HALF=0.5E+0, ONE=1.E+0, ZERO=0.E+0)
C     ------------------------------------------------------------------
C
      SDOTWM = SD7TPR(P, STEP, WCHMTD)
      DENMIN = COSMIN * SV2NRM(P,STEP) * SV2NRM(P,WCHMTD)
      WSCALE = ONE
      ...
      DO 10 I = 1, P
 10      W(I) = T * WCHMTD(I)
      CALL SS7LVM(P, U, A, STEP)
      T = HALF * (SIZE * SD7TPR(P, STEP, U)  -  SD7TPR(P, STEP, Y))
      DO 20 I = 1, P
...
C
 ...
 40      CONTINUE
C
C  ***  LAST CARD OF SS7LUP FOLLOWS  ***
      END
      SUBROUTINE SL7MST(D, G, IERR, IPIVOT, KA, P, QTR, R, STEP, V, W)
C
...
C             SL7MST FOR THE CURRENT R AND QTR.  ON OUTPUT KA CON-
...
C SL7ITV - APPLY INVERSE-TRANSPOSE OF COMPACT LOWER TRIANG. MATRIX.
C SL7IVM - APPLY INVERSE OF COMPACT LOWER TRIANG. MATRIX.
C SV7CPY  - COPIES ONE VECTOR TO ANOTHER.
C SV2NRM - RETURNS 2-NORM OF A VECTOR.
C
C  ***  REFERENCES  ***
C
...
C  ***  EXTERNAL FUNCTIONS AND SUBROUTINES  ***
C
      REAL             SD7TPR, SL7SVN, SR7MDC, SV2NRM
      EXTERNAL SD7TPR, SL7ITV, SL7IVM, SL7SVN, SR7MDC,SV7CPY, SV2NRM
C
C  ***  SUBSCRIPTS FOR V  ***
C
...
C
 10   KA = 0
      ...
      K = P
      IF (IERR .NE. 0) K = abs(IERR) - 1
      V(NREDUC) = HALF*SD7TPR(K, QTR, QTR)
C
C  ***  SET UP TO TRY INITIAL GAUSS-NEWTON STEP  ***
C
 20   V(DST0) = NEGONE
      IF (IERR .NE. 0) GO TO 90
      T = SL7SVN(P, R, STEP, W(RES))
      IF (T .GE. ONE) GO TO 30
         IF (SV2NRM(P, QTR) .GE. BIG*T) GO TO 90
C
...
C     ***  TREAT IT AS SUCH WHEN USING SL7ITV AND SL7IVM.
 30   CALL SL7ITV(P, W, R, QTR)
...
C
 110  KA = KA + 1
      CALL SV7CPY(PP1O2, W(RMAT), R)
      CALL SV7CPY(P, W(RES), QTR)
C
...
C  ***  COMPUTE STEP  ***
C
 280  CALL SL7ITV(P, W(RES), W(RMAT), W(RES))
...
C
      IF (PHI .GT. ZERO) GO TO 310
         IF (KA .GE. KALIM) GO TO 430
              TWOPSI = ALPHAK*DST*DST - SD7TPR(P, STEP, G)
...
C
 ...
 320  DO 330 I = 1, P
         ...
         STEP(I) = D(J1) * (W(K)/DST)
 330     CONTINUE
      CALL SL7IVM(P, STEP, W(RMAT), STEP)
      DO 340 I = 1, P
...
C
 430  V(STPPAR) = ALPHAK
 440  V(GTSTEP) = min(SD7TPR(P,STEP,G), ZERO)
...
C
C
C  ***  LAST CARD OF SL7MST FOLLOWS  ***
      END
      SUBROUTINE SG7QTS(D, DIG, DIHDI, KA, L, P, STEP, V, W)
C
...
C     (SG7QTS ASSUMES  DIG = D**-1 * G  AND  DIHDI = D**-1 * H * D**-1.)
C
C  ***  PARAMETER DESCRIPTION  ***
C
...
C SL7IVM - APPLIES INVERSE OF COMPACT LOWER TRIANG. MATRIX.
C SL7SRT  - FINDS CHOLESKY FACTOR (OF COMPACTLY STORED LOWER TRIANG.).
C SL7SVN - RETURNS APPROX. TO MIN. SING. VALUE OF LOWER TRIANG. MATRIX.
C SR7MDC - RETURNS MACHINE-DEPENDENT CONSTANTS.
C SV2NRM - RETURNS 2-NORM OF A VECTOR.
C
C  ***  REFERENCES  ***
...
C
      REAL             SD7TPR, SL7SVN, SR7MDC, SV2NRM
      EXTERNAL SD7TPR, SL7ITV, SL7IVM,SL7SRT, SL7SVN, SR7MDC, SV2NRM
C
C  ***  SUBSCRIPTS FOR V  ***
C
...
C  ***  TRY ALPHA = 0  ***
C
 30   CALL SL7SRT(1, P, L, DIHDI, IRC)
...
C     ***  POSITIVE DEFINITE H -- COMPUTE UNMODIFIED NEWTON STEP.  ***
 50   LK = ZERO
      T = SL7SVN(P, L, W(Q), W(Q))
      IF (T .GE. ONE) GO TO 60
         IF (V(DGNORM) .GE. T*T*BIG) GO TO 70
 60   CALL SL7IVM(P, W(Q), L, DIG)
      GTSTA = SD7TPR(P, W(Q), W(Q))
      V(NREDUC) = HALF * GTSTA
      CALL SL7ITV(P, W(Q), L, W(Q))
      DST = SV2NRM(P, W(Q))
      V(DST0) = DST
      PHI = DST - RAD
...
C  ***  FROM SL7SVN TO OBTAIN APPROXIMATE EIGENVECTOR CORRESPONDING
C  ***  TO SMALLEST EIGENVALUE OF (D**-1)*H*(D**-1).  SL7SVN RETURNS
C  ***  X AND W WITH  L*W = X.
C
 340  T = SL7SVN(P, L, W(X), W)
C
C     ***  NORMALIZE W  ***
      DO 350 I = 1, P
 350     W(I) = T*W(I)
...
C     ***  IF NOT YET AVAILABLE, OBTAIN MACHINE DEPENDENT VALUE DGXFAC.
 370  IF (DGXFAC .EQ. ZERO) DGXFAC = EPSFAC * SR7MDC(3)
C
...
C
      T1 = ZERO
      T = SI*(ALPHAK*SW - HALF*SI*(ALPHAK + T*SD7TPR(P,W(X),W)))
...
C
C
C  ***  LAST CARD OF SG7QTS FOLLOWS  ***
      END
      SUBROUTINE SL7IVM(N, X, L, Y)
C
...
C
 ...
 20   J = K*(K+1)/2
      ...
      K = K + 1
      DO 30 I = K, N
         T = SD7TPR(I-1, L(J+1), X)
         J = J + I
         X(I) = (Y(I) - T)/L(J)
 30      CONTINUE
 999  RETURN
C  ***  LAST CARD OF SL7IVM FOLLOWS  ***
      END
      REAL             FUNCTION SL7SVX(P, L, X, Y)
C
...
C  L (IN)  = ARRAY HOLDING THE ELEMENTS OF  L  IN ROW ORDER, I.E.
C             L(1,1), L(2,1), L(2,2), L(3,1), L(3,2), L(3,3), ETC.
C  X (OUT) IF SL7SVX RETURNS A POSITIVE VALUE, THEN X = (L**T)*Y IS AN
...
C  Y (OUT) IF SL7SVX RETURNS A POSITIVE VALUE, THEN Y = L*X IS A
...
C
 999  RETURN
C  ***  LAST CARD OF SL7SVX FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SA7SST(IV, LIV, LV, V)
C
...
C             UNCHANGED SINCE THE PREVIOUS RETURN OF SA7SST.
...
C             IN WHICH CASE SA7SST SETS IV(SWITCH) = 1.
...
C             THAN V(AFCTOL), THEN SA7SST RETURNS WITH IV(IRC) = 10.
...
C            SA7SST RETURNS WITH IV(IRC) = 8 OR 9.
...
C    SA7SST IS DESIGNED TO HANDLE ANY NUMBER OF MODELS.
C
C  ***  USAGE NOTES  ***
C
...
C     CHANGE THE STOPPING TOLERANCES AND CALL SA7SST AGAIN, IN WHICH
...
C
 999  RETURN
C
C  ***  LAST CARD OF SA7SST FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SITSUM(D, G, IV, LIV, LV, P, V, X)
C
...
C
 ...
 380  FORMAT(/' ***** IV(1) =',I5,' *****')
      GO TO 999
C
C  ***  INITIAL CALL ON SITSUM  ***
C
 390  IF (IV(X0PRT) .NE. 0) WRITE(PU,400) (I, X(I), D(I), I = 1, P)
 400  FORMAT(/'     I     INITIAL X(I)',8X,'D(I)'//(1X,I5,g17.6,g14.3))
...
C
 ...
 999  RETURN
C  ***  LAST CARD OF SITSUM FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SL7ITV(N, X, L, Y)
C
...
C
 ...
 30      CONTINUE
 999  RETURN
C  ***  LAST CARD OF SL7ITV FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SL7SQR(N, A, L)
C
...
C
 ...
 30      CONTINUE
      END
c     ==================================================================
      SUBROUTINE SL7SRT(N1, N, L, A, IRC)
C
...
C
 999  RETURN
C
C  ***  LAST CARD OF SL7SRT  ***
      END
c     ==================================================================
      SUBROUTINE SL7TVM(N, X, L, Y)
C
...
C
 ...
 10           CONTINUE
         I0 = I0 + I
 20      CONTINUE
C  ***  LAST CARD OF SL7TVM FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SL7VML(N, X, L, Y)
C
...
C
 ...
 10           CONTINUE
         X(I) = T
 20      CONTINUE
C  ***  LAST CARD OF SL7VML FOLLOWS  ***
      END
c     ==================================================================
      REAL             FUNCTION SRLDST(P, D, X, X0)
C
...
C
 ...
 10      CONTINUE
      SRLDST = ZERO
      IF (XMAX .GT. ZERO) SRLDST = EMAX / XMAX
C  ***  LAST CARD OF SRLDST FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SV2AXY(P, W, A, X, Y)
C
...
C
      DO 10 I = 1, N
 10       X(I) = A * Y(I)
C  ***  LAST LINE OF SV7SCL FOLLOWS  ***
      END
c     ==================================================================
      SUBROUTINE SV7SCP(P, Y, S)
C
