...
C                                                                       00010200
      ...
      IF(X .GT. UPLIM .OR. YN .GT. UPLIM)THEN
       ...
       CALL SERM1 ('SRCVAL',3,0,'X > UPLIM or ABS(Y) > UPLIM','X',X,',')
         CALL SERV1 ('Y',Y,',')
         CALL SERV1 ('UPLIM',UPLIM,'.')                                 00012200
         IERR = 3
         RETURN
      END IF
      ...
      IF(X + YN .GT. UPLIM)THEN
         ...
         RETURN
      END IF
      IF(Y.LT.Y2236L .AND. X.GT.0.0e0 .AND. X.LT.XLU225)THEN            00014200
         RC = 0.0e0
...
20017 CONTINUE
C
      RC = W*(1.0e0 + SN*SN*(C1 + SN*(C2 + SN*(C3 + SN*C4)))) / SQRT(MU)00017300
      RETURN
C
      END        
