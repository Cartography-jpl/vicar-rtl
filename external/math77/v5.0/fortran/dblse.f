...
c     rows of the system in the least squares sense, subject to bounds  00002400
...
c     order M3.
c
c     In anticipation of this usage, this subroutine searches           00004400
...
c     procedure( TERMINATION )                                          00006400
c
...
c
c          The following Sftran procs are used in both Phases 1 & 2:    00008400
c
...
c     procedure( DEBUG )
c     -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
c          BVLS was developed by C. L. Lawson and R. J. Hanson at       00009400
...
c     1988 April CLL & RJH.  Added code to delete constraint rows that  00010400
...
c             where G is an MTRUE by MTRUE matrix generated             00011400
...
c
c     BND(,)  [In] If BND(1,J) .eq. UNBND then X(J) is unbounded below. 00013400
...
c             unboundedness.
c
c     KPRINT  [in]  Larger values give more printing.                   00014400
c           0:       No printing.
...
c           Here EPS denotes the machine precision, such as would be    00016400
...
c              = 2  Inconsistent setting of bounds.                     00017400
...
c             by solving a system of equations, and thus will generally 00018400
c             not be at a bound.
c
...
c             initially and the solution vector at conclusion.  Also    00020400
...
c     ------------------------------------------------------------------
c                 Description of Sets S, F, and H.                      00022400
c
...
c     the currently triangularized subsystem, or by adjusting such a    00024400
...
c     ------------------------------------------------------------------
      ...
      integer II, IIS, IMAX, INDEX(N), IS, ISNEW, IPUT, IPUT1, IPUT2    00026500
      ...
      double precision A1, ABSA1, ALPHA, AMAX                           00027500
      double precision B1, BNDL, BSAVE, BSIZE, BTEMP
      double precision DIFF(MTRUE), FAC, FACTOR
      ...
      parameter(ZERO = 0.0D0, TWO = 2.0D0, FACTOR = 0.01D0)             00028500
...
c           into Set S.  If NSETS .lt. M1 we must build the triangle    00029600
c           further.
c
...
20017 ASSIGN 20019 TO NPR004                                            00032000
      GO TO 30004
20019 CONTINUE
...
c     .        zero is not needed for the functioning of this           00035200
...
c
            X(J) = max(BND(1,J), min(BND(2,J),ZERO))                    00039200
      ELSE
c                                 ****** Error:  IERR = 2 ******
            IERR = 2
            call IERM1('DBLSE',IERR,0,                                  
...
c
      IF(II .gt. NSETS .and. II .le. M1)THEN
            ...
            XTRY = X(JF) + BTEMP / A(II,JF)                             00044200
...
20066 CONTINUE
c
c     .  Quit on error flag, or                                         00050500
c     .  if Set F is empty.
c
...
c        .  inconsistent with preceeding constraint rows.  It also      00053400
...
c     B(NSP1) contains the residual for row NSP1 using the current X(). 00055400
...
c
c     .    We wish to force the maximum number of                       00056400
...
c
      DO 20086 BMODE = 1,2
      if(KPRINT .ge. 2) print '(4x,a,g14.6,a,i2/4x,a,i4/4x,a)',         00058500
...
c
     ...
     *         JF,A1,FREE1,FREE2,FREE
      ELSE
               print '(4x,i4,g14.6,3L2,16x,i4)',                        00062900
...
c
      ...
      IF(BMODE .eq. 1 .and. B1 .ne. ZERO)THEN
         BSIZE = abs(BSAVE)
      DO 20108 J = 1,N
            BSIZE = BSIZE + abs(A(NSP1,J)*X(J))                         00064000
20108 CONTINUE
...
c     .                          Selected index is JSNEW = INDEX(ISNEW)
     ...
     *            X(JSNEW)  .ne.  BND(1,JSNEW)
      FREEJS(2) = BND(2,JSNEW)  .eq.  UNBND   .or.                      00066200
...
c     Do one stage of Gaussian elimination in the upper matrix.         00067300
...
20115 GO TO 20075
c     ------------------------------------------------------------------
C     procedure( MSG FOR IERR = -1 )                                    00069400
...
c     Copy row M to row M1.                                             00071900
...
30013 IF(NSP1 .lt. M1)THEN
         ...
         RT(NSP1) = RT(M1)
      END IF
c     .                              Copy row M to row M1.              00072900
c
...
20124 CONTINUE
c
c        When this loop is entered, NSETS will be > M1.                 00077300
...
20126 ASSIGN 20127 TO NPR012
      GO TO 30012
c
c           .  The above proc will set IERR nonzero if                  00078300
...
C     procedure( BVLS: SELECT COEF TO SOLVE FOR )
c
c     .    Search thru Set F for a new coef to solve for.               00080100
...
c     .             nonzero and if X(J) is at a bound, the sign of W(J) 00081100
...
c     .    If a coef is selected, set FIND = true.                      00082100
c     The index of the selected coef is JSNEW = INDEX(ISNEW).
c     .    If no coef is selected, set FIND = false.
c
30015 FIND = .FALSE.
      ...
      DO 20130 I = 1,M
               print '(1x,i3,2x,5g13.6/(6x,5g13.6))',                   00083100
     *          I,(A(I,J),J=1,N), B(I)
20130 CONTINUE
      END IF
c
...
c     .                                 Begin loop through Set F.
      DO 20133 IF0=IF1,IF2
         J=INDEX(IF0)
c
c        .  Set FREE1 = true if X(J) is not at the left end-point of    00084300
c        .  its constraint region.
...
c        .  have JSTAT(J) = -1.
c        .     For the ok columns the norm is computed and stored in    00086300
...
c        .     find one having largest abs(W(J)).                       00090500
c
      WMAX = ZERO
...
c     is in the array Z().
c     If X + Delta_x satisfies the bounds, this proc returns with       00097000
...
c        .                             ****** Error: IERR = 3 ******
     ...
     *   'Iteration count ITER exceeds ITMAX.','ITER',ITER,',')
         call IERV1('ITMAX',ITMAX,'.')                                  00099200
      GO TO 31012
      END IF
c
      GO TO 30017
...
c
      DO 20177 IS=1,NSETS
            L=INDEX(IS)
            X(L) = X(L) + ALPHA * Z(IS)
            if(KPRINT .ge. 2) print '(1x,a,i4,g14.6)',                  00101400
...
20192 CONTINUE
20191 CONTINUE
31012 GO TO NPR012,(20076,20127)                                        00103600
c     ------------------------------------------------------------------
...
c     Then ALPHA will satisfy  0. .le. ALPHA .lt. 1.                    00104600
c
30017 ALPHA=TWO
      DO 20193 IS=1,NSETS
          L=INDEX(IS)
...
c            .                               XTRY crosses upper bound.  00105600
             LBOUND=2
      ELSE
         LBOUND = 0
      END IF
c 
      IF( LBOUND  .ne.  0 )THEN
         ...
         T = (BNDL - X(L)) / (XTRY - X(L))
      IF(KPRINT .ge. 2)THEN
            print'(1x,a,i4,a,i4,a,g15.7/1x,a,g15.7,a,g15.7,a,g15.7)',   00106600
...
c              .  Similarly, if the tests of L and FREEJS() are         00108200
...
c
               if(KPRINT .ge. 1) print '(4x,a,a/4x,a,i4,a,i4,a,g16.8)', 00109200
...
c                 .  cycling, we make a random selection between the    00110600
...
c     This proc is ref'd by Proc CONTROL CONSTRAINT TESTING, which is   00117000
...
c
      ...
      IF(A(NSP1, JS) .ne. ZERO)THEN
               ...
               A(NSP1,JS) = ZERO
               B(NSP1) = B(NSP1) + FAC * B(NSETS)                       00121000
...
20236    IGIV1 = IFNEW
         IGIV2 = NSETS
      ASSIGN 20238 TO NPR022                                            00122100
      GO TO 30022
20238 CONTINUE
20228 CONTINUE
20224 GO TO 20183
...
c     .  last element of each of these columns.
c
c        .  Find element of largest magnitude in row M1 among column    00123200
...
c
*        do for IS = IGIV1, IGIV2                                       00128400
*           JS=INDEX(IS)
...
*              call DROT (1, B(IS),1, B(IS+1),1, CC,SS)                 00129400
*              if(KPRINT .ge. 3)
...
c
      DO 20256 IR = IGIV1, min(L-1,IGIV2)
            TEMP =          CC(IR)*A(IR,JCOL) + SS(IR)*A(IR+1,JCOL)     00131400
...
20261 CONTINUE
c
c        . Apply previous transformations to RT() for code testing.     00133400
c
      IF(KPRINT  .ge. 3)THEN
...
c
30020    if(KPRINT .ge. 2) print '(1x,a,2i4)',                          
     *   'Gauss elim in pairs of rows.  IGAUS1, IGAUS2 =',IGAUS1,IGAUS2
      DO 20269 IS = IGAUS1, IGAUS2
            JS=INDEX(IS)
      IF(abs(A(IS+1,JS)) .gt. abs(A(IS,JS)))THEN                        00135500
      ...
      END IF
               ...
               call DAXPY(N, FAC, A(IS,1),LDA, A(IS+1,1),LDA)           00136500
               A(IS+1,JS) = ZERO
...
C     procedure( PREPARE ROW NSP1 TO BE OBJECTIVE FUNCTION )            00139800
c
...
c
30008 BSAVE = B(NSP1)
      B(NSP1) = B(NSP1) - DDOT(N, A(NSP1,1), LDA, X, 1)                 00140800
      DO 20284 IS = 1,NSETS
         JS = INDEX(IS)
      IF(IS .gt. 1)THEN
...
c     previously done.  Now eliminate in columns IG1 thru M1 of the     00142800
c     lower matrix, i.e. in rows M1+1 through M.
c
30005 if(KPRINT .ge. 3) print '(1x/1x,a,2i4)',                          
...
20292 CONTINUE
      ...
      ASSIGN 20308 TO NPR004                                            00145800
      GO TO 30004
...
c
30014 DO 20309 IIS = NSETS, 1, -1                                       00146800
...
c
c                     Compute the norm of the final residual vector.    00147800
c                     Clean up W().
30007 RNORM = ZERO
...
20323 CONTINUE
         ...
         print '(1x/1x,a)', 'Consistency test. DIFF()='                 00150800
