...
c                                      Index search, then exit
 1300    continue
         ...
         TP1 = 1.D0 + (XI - XT(1)) / H
         ILI = min(max(1, int(TP1+0.5D0)), NTABI)
         XU = (TP1 - dble(ILI)) * H
         XL = XU
         DX(0) = XU
         N = ILI + ILI
         if (TP1 .gt. dble(ILI))  N = N + 1
         if (TP1 .lt. 1.D0) go to 1240
         if (TP1 .le. dble(NTABI)) go to 260
         go to 1240
c
c                                 Already set up
 1400    KGO = KGOC
...
c                                     Set for automatic order selection.
 1950    continue
            I = I + 2
            LNDEG = I
            EBND = max(0.D0, EOPT(IOPT(I-1)))
            EBNDR = max(0.D0, EOPT(IOPT(I-1)+1))
            KGO = 3
            KAOS = 1
...
c Take care of error messages, IIFLG < -2 should stop.
      ...
      MACT(7) = -IIFLG
      MACT(8) = MLOC(-IIFLG)
      call DMESS(MACT, MTXTAA, IOPT, ERRDAT)
      MACT(9) = MERET
      return
